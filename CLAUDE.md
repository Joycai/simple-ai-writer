...
- `src/lib/` — Core logic, grouped by domain:
  - `src/lib/ai/` — streaming client (`index.ts` dispatch, `openai.ts`/`gemini.ts`/`anthropic.ts` adapters, `types.ts`), per-protocol JSON-mode shaping (`jsonMode.ts`), provider config storage (`configDb.ts`), Gemini safety settings (`safety.ts`), remote probing (`providerProbe.ts`), endpoint limit probing (`endpointProbe.ts` HTTP + `probeAnalysis.ts` pure judgement — measures a model's real context window / output cap; see `docs/architecture.md` → Endpoint probing), multi-draft output vocabulary (`drafts.ts`), token/cost accounting read side (`usage.ts` — the `token_usage` rollups behind Settings → 用量), `apiLog.ts`, `tokenEstimate.ts`
