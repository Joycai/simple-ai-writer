...
- `server/` — **not part of the app.** A standalone Rust/axum binary: the knowledge-base backup server the app can push a project's lore tree to and pull it back down on another machine (named knowledge bases, one-way whole-tree sync, per-entry content hashes). It stores zips and reports hashes — nothing in it parses markdown or knows what a facet is, so the lore format can keep moving without it. Its own crate, its own CI job, its own `server/README.md` (what it is + API) and `server/DEPLOY.md` (编译 / 生成密钥 / systemd / Docker / TLS / 轮换 / 排错); design in `docs/remote-knowledge-base-feasibility.md` §13–§18. The client side lives in `src/lib/sync/` + `src/components/sync/` + Settings → 知识库同步
