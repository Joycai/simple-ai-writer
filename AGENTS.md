...
- `src/lib/theme/` — scheme, contract, validator, registry, install, export, typography themes, downloadable font packs
...
- **[`docs/feature/billing/01-fee-groups.md`](docs/feature/billing/01-fee-groups.md)** — 计费组与两本用量账：九条不变量（含「六段 + 未分项 ≡ `cost_usd`」）、迁移为什么用 `fee_migrated`、备份 v3。Read before touching `src/lib/ai/feeGroup*`, `usageRow`, `usage`, `usageBackfill`, `usageMeter`, or anything that prices a request.
