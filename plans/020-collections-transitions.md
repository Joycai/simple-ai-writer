...
1. **`.railRowOn` 的 `padding-left` 不进过渡。** `.railRow` 是
   `padding: 9px 18px` + `border-left: 3px solid transparent`；`.railRowOn` 把
   `padding-left` 改成 `15px` 并让左边框显色。3 + 15 = 18，**内容位置完全不变**，
   padding 只是给边框让位。过渡 `border-left-color` 和 `background` 即可，
   加 padding 反而会制造一次不存在的位移。
...
- 不动 `.railRowWrap:hover .railSetScope { display: block; }`（:310）——`display`
  ...
  **那不在本方案范围内**，也不确定是否该做（悬停才出现的「设为取材范围」刻意
  「不该随手可点」，见该处注释）。原样留着。
- 不动 `.railRowOn` 的 `padding-left`，不动 `.scopeButton` 的 `border-style`，
