...
>
> **为什么全部保留**（2026-09-12 机械核对，别照着「有人引用」这一条去删）：
> 源码注释与 `docs/` 以「方案 NNN」/`plans/NNN-*.md` 引用的只有 **16 份**
> （003 004 005 009 010 017 020 031 044 045 047 049 050 051 052 053，分布在 14 个
> 文件里共 27 处）——所以「每份都被外部引用」并不成立，靠这一条只能删掉三分之二。
> 真正拦住删除的是**本文件自己**：下面各批的执行记录与验收笔记里有 **78 条**
> 指向 `NNN-*.md` 的链接，且按批次穿插在正文中。删文件要么打断这些链接，要么
> 回头改写那些带日期的执行记录——而后者正是 `docs/reference/terminology.md`
> 「`design/` 与 `plans/` 整体不动」说的「会毁掉时间线」。要清理就连这份 README
> 一起重做，不要只挑文件删。
