...
**仍未做的目检**（需要真窗口 + 真项目，代码验证覆盖不到）——上面三条已用实测
替代，剩下三条只能靠手：

1. **021** 真窗口 Performance 录一次拖拽（先打开一个上千行的长文档，短文档压不出
   这条路径）：帧率不掉 60fps、松手不飘。
2. **024** 滚到距底部约 40px 处用触控板小幅惯性滚动来回蹭：「回到最新」气泡的
   出现/消失必须是干净的开关，**不许有重复浮起**。这是 024 存在的全部理由。
3. **025** 换取材范围整墙淡入一次、卡片微旋转全程不摆正；搜索框连打 10 个字符
   **零动画**。
