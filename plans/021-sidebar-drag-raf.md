...
> ⚠ **实测更正（2026-08-26）：下面对代价的描述说重了。** 用 CDP 的
> `Performance.getMetrics` 做 A/B（同样 60 次宽度变化，一种每次写 DOM、一种合并
> 到一帧），读 `RecalcStyleCount` / `LayoutCount` 的增量：
>
> ```
> 每次写 DOM（改动前）  recalc 0–3   layout 0–3
> 合并到一帧（改动后）  recalc 0     layout 0
> ```
>
> **浏览器本来就会批处理样式失效与布局**——60 次连续 `setProperty` 不会产生 60 次
> 回流，因为期间没有任何强制同步布局的读操作。所以「一帧之内可能重复做好几次
> 样式重算 + 编辑器整列回流」是夸大的：真正省下的是每帧多余的那 1–2 次失效
> （120Hz 鼠标约 2 事件/帧，1000Hz 轮询更多），以及高刷输入下的鲁棒性。
>
> 合并本身仍然是对的做法（也是标准做法），只是**不要指望肉眼可见的提速**。
> 拖拽手感的真实收益来自方案 001 的 `data-resizing`（关掉 320ms 过渡），
> 不是本方案。真窗口里的 Performance 录制仍值得做一次，但预期是「一帧一次
> Layout」，不是「次数大幅下降」。
