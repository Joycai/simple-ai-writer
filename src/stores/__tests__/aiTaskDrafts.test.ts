...
vi.mock("../../lib/agent/plan", async (importOriginal) => ({
  ...(await importOriginal<typeof import("../../lib/agent/plan")>()),
  createPlanGate: () => ({}),
}));

// Context assembly is someone else's test; here it just has to be cheap and
// identical for every draft.
vi.mock("../../lib/context/rag", () => ({
...
vi.mock("../../lib/context/budget", async (importOriginal) => ({
  ...(await importOriginal<typeof import("../../lib/context/budget")>()),
  fixedContextChars: () => 0,
...
vi.mock("../../lib/context/memory", async (importOriginal) => ({
  ...(await importOriginal<typeof import("../../lib/context/memory")>()),
  loadMemory: vi.fn(async () => null),
}));
vi.mock("../../lib/context/bookContext", () => ({
...
vi.mock("../../lib/profile/active", async (importOriginal) => ({
  ...(await importOriginal<typeof import("../../lib/profile/active")>()),
...
import { MAX_DRAFTS, draftCountFor, totalUsage, type Draft } from "../../lib/ai/drafts";
...
async function instructionOf(taskId: string, ask?: string): Promise<string> {
  const rag = await import("../../lib/context/rag");
