...
vi.mock("../../lib/configsync/client", async () => {
  const actual = await vi.importActual<typeof import("../../lib/configsync/client")>(
    "../../lib/configsync/client",
  );
  return { ...actual, connectedConfigClient: async () => fakeClient };
});

const remembered = new Map<string, string>();
vi.mock("../../lib/configsync/password", () => ({
...
vi.mock("../../lib/sync/config", async () => {
  const actual = await vi.importActual<typeof import("../../lib/sync/config")>("../../lib/sync/config");
  return { ...actual, getServerUrl: () => "http://box:8787" };
});
vi.mock("../../lib/sync/local", () => ({ deviceLabel: async () => "REINE-DESKTOP" }));
...
vi.mock("../../lib/ai/configDb", () => ({ listProviders: async () => [] }));
vi.mock("../../lib/project", () => ({ getGlobalDb: async () => ({}) }));
let refreshed = 0;
vi.mock("../configImportRefresh", () => ({
  refreshAfterConfigImport: async () => void refreshed++,
}));

const { useConfigSyncStore } = await import("../configSyncStore");
const { sealBundle } = await import("../../lib/configsync/envelope");
...
describe("push", () => {
  ...
  it("reports a 412 as a conflict the author can act on", async () => {
    const { SyncConflictError } = await import("../../lib/sync/client");
