...
export const useAiTaskStore = create<AiTaskState>((set, get) => ({
  ...
  runTask: async (kind, customInstruction, continueLength, extras, opts) => {
    ...
    // 全仓只有 agentStore 传）。于是「写沈舟的潜入场景」这句话既召不来〈沈舟〉，也激活
    // 不了她那条 keys 里写着「潜入」的特征，而作者看到的只是一个空的【知识库】块。
    //
    ...
    //
    // 上面那一步让「写沈舟的潜入场景」能召来〈沈舟〉、能激活「潜入」；这一步补的是
    // 「青铜罗盘」——那只罗盘这场戏要用，但正文里还没有它，因为写它就是这次的活儿。
    // 扩展词走的仍是 substring 匹配，所以注入报告照旧说得出「由「青铜罗盘」命中」。
    //
