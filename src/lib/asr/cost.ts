/**
 * 上传之前能知道多少：WAV 的时长和据此估的费用。纯函数。
 *
 * 其他容器的时长在 `duration.ts`（MP3 的 Xing 头 / FLAC 的 STREAMINFO / Ogg 的
 * 末页 granule / MP4 的 mvhd），WAV 这一支也从那里调进来。平台在结果里报的
 * `usage.duration` 才是真正计费的秒数，入账用那一个（`run.ts`），这里的估价只是
 * 确认卡上的那一格。
 */
