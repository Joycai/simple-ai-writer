...
/**
 * Replace one segment's summary text, keeping the segment protocol intact:
 * ranges and hashes are untouched (summary wording has no bearing on staleness
 * detection), and the metadata comment is regenerated by the save. This is the
 * only write path agent tools may use — handing them raw file access would let
 * a model mangle the `ai-writer-memory` comment block.
 *
 * Throws with a model-readable message when there is no memory file or the
 * index is out of range; returns the updated memory otherwise.
 */
export async function rewriteMemorySegment(
  projectPath: string,
  absDocPath: string,
  segmentIndex: number,
  summary: string,
): Promise<DocMemory> {
  const mem = await loadMemory(projectPath, absDocPath);
  if (!mem || mem.segments.length === 0) {
    throw new Error("No story memory exists for this document yet.");
  }
  if (!Number.isInteger(segmentIndex) || segmentIndex < 0 || segmentIndex >= mem.segments.length) {
    throw new Error(
      `segment_index ${segmentIndex} is out of range — this memory has segments 0–${mem.segments.length - 1}.`,
    );
  }
  const text = summary.trim();
  if (!text) throw new Error("The replacement summary must not be empty.");

  const segments = mem.segments.slice();
  segments[segmentIndex] = { ...segments[segmentIndex], summary: text };
  const updated: DocMemory = { ...mem, segments, updatedAt: new Date().toISOString() };
  await saveMemory(projectPath, updated);
  return updated;
}
