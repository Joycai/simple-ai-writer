...
const INDEX: LoreIndex = {
  characters: [
    entity("阳菜", ["陽菜", "ひなちゃん"], "主人公，青叶学院的学生会长，成绩优秀"),
    entity("小阳", []),
  ],
  world: [entity("青叶学院", ["青葉学園"], "  故事发生的学校  ")],
};

describe("collectGlossary", () => {
  it("只收本块命中的别名", () => {
    const entries = collectGlossary(INDEX, "陽菜は扉を開けた。");
    expect(entries.map((e) => `${e.src}->${e.dst}`)).toEqual(["陽菜->阳菜"]);
  });
  ...
  it("按源词长度降序 —— 取舍时留下更特指的那个，顺序也和强制替换一致", () => {
    const entries = collectGlossary(INDEX, "陽菜とひなちゃんは青葉学園にいる。");
    // 同长的两个按码元比，不按 locale —— 同一份知识库在两台机器上必须切出同一张表。
    expect(entries.map((e) => e.src)).toEqual(["ひなちゃん", "青葉学園", "陽菜"]);
  });

  it("备注取自条目摘要，压成一行并去掉首尾空白", () => {
    const [e] = collectGlossary(INDEX, "青葉学園。");
    expect(e.note).toBe("故事发生的学校");
  });

  it("长摘要被截断 —— 备注是给模型的一句提示，不是条目正文", () => {
    const long = "主人公，青叶学院的学生会长，成绩优秀运动万能，同时也是图书委员会的顾问";
    const idx: LoreIndex = { characters: [entity("阳菜", ["陽菜"], long)] };
    const [e] = collectGlossary(idx, "陽菜。");
  ...
  it("别名与条目名相同的词对不进表 —— 它什么都不表达，只占一行", () => {
    const idx: LoreIndex = { characters: [entity("陽菜", ["陽菜"])] };
    expect(collectGlossary(idx, "陽菜は。")).toEqual([]);
  });
...
describe("parseDictBody", () => {
  it("一行一条 src->dst #备注，就是 Sakura 的格式", () => {
    expect(parseDictBody("陽菜->阳菜 #主人公\nひなちゃん->小阳")).toEqual([
      { src: "陽菜", dst: "阳菜", note: "主人公" },
      { src: "ひなちゃん", dst: "小阳" },
    ]);
  });
  ...
  it("同一行里箭头优先于 = —— src 懒匹配切在第一个分隔符上", () => {
    // 备注里出现 = 不该把词对切错。
    const [e] = parseDictBody("陽菜->阳菜 #罗马字 fumika=ka");
    expect(e).toEqual({ src: "陽菜", dst: "阳菜", note: "罗马字 fumika=ka" });
  });

  it("标题、说明文字、空行静默跳过 —— 正文允许夹杂说明", () => {
    const body = "# 术语\n\n这是给翻译用的词表。\n\n陽菜->阳菜";
    expect(parseDictBody(body)).toEqual([{ src: "陽菜", dst: "阳菜" }]);
  });

  it("src === dst 的行不进表", () => {
    expect(parseDictBody("陽菜->陽菜")).toEqual([]);
  });

  it("备注走和摘要相同的截断预算", () => {
    const long = "主人公，青叶学院的学生会长，成绩优秀运动万能，同时也是图书委员会的顾问";
    const [e] = parseDictBody(`陽菜->阳菜 #${long}`);
    expect(e.note).toBe(long.slice(0, 24) + "…");
  });

  it("# 后面为空时不留一个空备注", () => {
    expect(parseDictBody("陽菜->阳菜 #")).toEqual([{ src: "陽菜", dst: "阳菜" }]);
  });
});
...
describe("formatDictBody", () => {
  it("一行一条，备注不截断 —— 条目正文是作者的资产，截断属于发给模型的清单", () => {
    ...
    const body = formatDictBody([
      { src: "陽菜", dst: "阳菜", note: long },
      { src: "ひなちゃん", dst: "小阳" },
    ]);
    expect(body).toBe(`陽菜->阳菜 #${long}\nひなちゃん->小阳`);
    // 渲染出的正文必须能被自己的解析器读回来——AI 标准化的闭环就靠这条。
    expect(parseDictBody(body).map((e) => e.src)).toEqual(["陽菜", "ひなちゃん"]);
  });
});

describe("collectGlossary + 翻译词典", () => {
  const dict: GlossaryEntry[] = [
    { src: "陽菜", dst: "文乃", note: "词典指定" },
    { src: "スケルトン", dst: "骷髅" },
  ];
  ...
  it("同一个源词词典赢过别名 —— 那一行是作者专门为翻译写的", () => {
    const entries = collectGlossary(INDEX, "陽菜は扉を開けた。", dict);
    expect(entries).toEqual([{ src: "陽菜", dst: "文乃", note: "词典指定" }]);
  });
...
describe("formatGlossary", () => {
  it("一行一条 src->dst #备注", () => {
    expect(formatGlossary([{ src: "陽菜", dst: "阳菜", note: "主人公" }, { src: "ひなちゃん", dst: "小阳" }])).toBe(
      "陽菜->阳菜 #主人公\nひなちゃん->小阳",
    );
  });
});
...
describe("enforceGlossary", () => {
  ...
  it("补上模型漏替的地方 —— 术语表只是软提示，实测漏了一半", () => {
    expect(enforceGlossary("陽菜拿出了粉饼盒。", entries)).toBe("阳菜拿出了粉饼盒。");
  });

  it("最长词优先", () => {
    // 短词先跑会把 `陽菜さん` 吃成 `阳菜さん`，再也拼不回来。
    expect(enforceGlossary("陽菜さん、こんにちは。", entries)).toBe("阳菜小姐、こんにちは。");
  });

  it("已经译对的地方不动", () => {
    expect(enforceGlossary("阳菜拿出了粉饼盒。", entries)).toBe("阳菜拿出了粉饼盒。");
  });

  it("不碰 URL、链接目标和行内代码 —— 那里面改一个字就是坏链接", () => {
    const g: GlossaryEntry[] = [{ src: "assets", dst: "资源" }, { src: "陽菜", dst: "阳菜" }];
    const src = "![陽菜](assets/陽菜.png) 见 https://x.test/assets/陽菜 与 `assets/陽菜`，陽菜在此。";
    const out = enforceGlossary(src, g);
    expect(out).toContain("](assets/陽菜.png)");
    expect(out).toContain("https://x.test/assets/陽菜");
    expect(out).toContain("`assets/陽菜`");
    // 图片的 alt 文本在保护区之外，是正文，照常替换。
    expect(out).toContain("![阳菜]");
    expect(out.endsWith("阳菜在此。")).toBe(true);
  });
