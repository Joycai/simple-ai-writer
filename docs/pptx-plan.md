...
> 状态：**读取端一期 + 二期已实施**（PR #223）。**生成端已实施，作为 Settings → 通用 → 实验功能 里的 Beta 开关**（§4）。
...
**退化成图片**：内联 SVG（图示类内容基本都走这条，视觉一致但不再是可编辑形状）、`<canvas>`。
...
**未验证**：桌面 app 里没跑过——Settings 开关、审批卡、`export_pptx` 的完整链路只有类型检查和单测担保（这台机器上 Vite dev server 起得来但没有项目和模型，导出要读磁盘上的文件）。

### D18 `read_slides` 也读 .html——同一个工具，不是第二个（2026-08-19）
...
- **切分约定必须与 `harvester.js` 的 `SLIDE_SELECTORS` 一致**，否则「第 7 页」在读和导出时
  ...
  （它 import 不了任何东西），所以这份不变量靠 `htmlSlides.ts` 的模块注释加 `htmlSlides.test.ts` 守。
...
- **`.html` 的分页与导出的分页同源**：`htmlSlides.ts` 的选择器表和 `harvester.js` 的 `SLIDE_SELECTORS` 必须逐条一致——不一致的那天，作者审的「第 7 页」和导出的第 7 页不是同一页。
