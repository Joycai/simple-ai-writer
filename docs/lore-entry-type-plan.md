...
2. 条目本身仍是「主词条 + 特征 + 配图」，只是特定类型下某些特征归属于特定的项目（外貌、人物关系……）。
...
条目 = 主词条 + 特征 + 配图（屏 14/15/16）；详情页三栏 `主词条 320 | 特征 flex | 配图 300`，
...
20. 详情页 · 槽位缺口。同一屏的变体：expected 但为空的槽位显示为**虚线空段**
    ...
    另给主词条栏底部一条「本类型建议的 6 面已有 4」的等宽小结。
