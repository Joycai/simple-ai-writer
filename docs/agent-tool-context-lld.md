...
| 4 briefing 去重 | **zh -1,477 / en -800**（实测，见下） | 同比例 | 中（实测占大头） | 已测：两格 3/3=3/3 无退化 |
...
全部落地后对话助手（中文界面）的每轮固定头部：**~12,300 → ~7,000 token**（-43%），
④ 族端点上这 7,000 里的绝大部分还走缓存价。（原估 ~6,000；PR3 量掉之后改为 ~6,600；
再按 PR4 的实际语言差修正到 ~7,000。估算连错两次，都是没先量。）
