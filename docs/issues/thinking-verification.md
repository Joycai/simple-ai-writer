...
| # | 验什么 | 怎么验 | 影响 |
| --- | --- | --- | --- |
| 3.1 | ~~3.1 Pro 是否拒绝 `MINIMAL`~~ | — | **已不需要**：2026-09-26 起「关闭」发 `LOW`，所有 Gemini 3 都收（3.8 Flash 拒 `MINIMAL`，见上方 OrcaRouter 实测） |
| 3.2 | 档位是否真的生效 | 对比 `LOW` 与 `HIGH` 的 `thoughtsTokenCount` | ✅ 已验（3.8 Flash，193 → 1,348） |
