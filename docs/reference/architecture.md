...
1. `trimHistory` (`runtime.ts`) caps a live history at the newest `MAX_IMAGE_RESULTS` (3) pictures — unconditionally, before the token check, because the token estimate charges a *flat rate* per image while the payload keeps growing.
   Then, still unconditionally, it elides older pictures until what is left fits `MAX_REQUEST_IMAGE_CHARS` (24 MiB of data URL) — the count cap counts *messages*, and one message can carry four. The newest picture-bearing message is never touched; if it alone is over, `streamCompletion` refuses the request with `ImagePayloadError` before uploading anything. The chat composer and the vision subagent fit themselves under the same ceiling when they build a message (image-normalize-plan.md §2.9).
