...
**Three entry points**, all gated on `canSeeImages(model)` (type `multimodal` or `vision` — `lib/ai/configDb`) — a *declaration on the model row* in 设置 → 供应商与模型, not a guess from the model name. A text-only model is told the picture could not travel rather than silently losing it:
