...
| `reference/design-system.md` | ### 设定集设计语言 | ### 知识库设计语言 | 同上；设计稿文件名 `03a 设定集 · 条目与类型 Lore A` / `03b 设定集 · 集合 Lore B` 是外部产物，保留原样 |
...
**这句是错的。** 那些块名不是字面量，是从 i18n 拼出来的（`context.ts` 的 `` `【${label}】` ``，label 取自 `roleplay.section.bound` / `.memory` / `.scene`）。所以**批次 A 就已经把【绑定设定】改成了【绑定条目】**，根本轮不到批次 F。错判的代价是：此后三个月里 `context.ts` 的注释、`run.ts` 的注释和七份 roleplay 文档一直画着一个不存在的块名。
...
`design/PRD.md` 一个文件就有 44 处「设定」——那是应用只做小说、知识库还叫设定集时的原始需求文档。`plans/` 是十一批动效审计的执行记录（README 的总表 + 仍被引用的 16 份方案）。两者都是历史，不是「今天的系统」，校准它们没有收益且会毁掉时间线。
...
清扫结果：`docs/` 132 处 + 源码注释与夹具 20 处。留在原地的 17 处都有名有姓——设计稿文件名 `03a 设定集 · 条目与类型 Lore A` / `03b 设定集 · 集合 Lore B`、配图槽位名「服装设定」（`model.ts` 里的真实标签）、`docx` 里作动词的「能设定样式」、`collections: ["小说A", "共享设定"]` 这种示例数据、`subagent-lld.md` 三处引用 CLAUDE.md 禁令原文，以及 `docs/api/` 的 44 处「思维链」（协议域）。
