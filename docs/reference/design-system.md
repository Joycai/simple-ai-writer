...
- **Tags**: `--color-tag-bg` / `--color-tag-text` for the neutral badge. Model-type tags get one hue each — `--color-type-{text,multimodal,vision,image,video,asr}-{bg,fg}` — so a model list is scannable without reading the labels.
...
Each scheme overrides **both** `--font-serif` (editor body) and `--font-sans` (UI); `hei` points serif at a sans stack to make the whole app sans. To **add a scheme**: append a `[data-font="…"]` block in `tokens.css`, extend the `FontScheme` union + `FONT_SCHEMES` array in `appStore.ts`, add an entry (with a `previewFont` mirroring the serif stack) to `SYSTEM_FONTS` in `settings/panes/AppearancePane.tsx`, and add `systemSettings.appearance.font*` labels to both locales. A **downloaded** scheme differs in two places: its id goes into the `FontPackId` union + `FONT_PACK_IDS` (`lib/theme/fontPacks.ts`) rather than straight into `FontScheme` — `FontScheme` already spreads `FONT_PACK_IDS` — and into `PACK_FONTS` rather than `SYSTEM_FONTS`. It also needs its package pinned in `scripts/gen-font-packs.ts` (run it with `--verify`) and entries in `FONT_PACK_SOURCES` + `FONT_PACK_LICENSE`; its `tokens.css` block and locale labels are the same as a system scheme's.
...
**Typography theme files** sit beside the built-ins (`docs/feature/theme-system-plan.md` §5): a `.css` with `--theme-kind: markdown` in `appDataDir/themes/` or a project's `.ai-writer/themes/`, fenced to `.md-body` by `lib/theme/validate.ts` (every selector starts there; `@font-face` / `@keyframes` allowed; `url()` relative or `data:` only, inlined as `data:` on install and export). The file is installed as a second `<style>` after the generator's, its selectors prefixed `html[data-md-theme]` so it wins over the built-in it extends, and `data-md-theme` names that built-in. The settings samples are sandboxed `<iframe srcdoc>` frames carrying the export's stylesheet (`lib/theme/sample.ts`) — never `allow-same-origin`. **Downloadable examples** live in [`themes/`](../../themes/README.md) at the repo root — 学报 / Journal (numbered sections, booktabs tables) and the 物理猫 Phycat suite (eleven appearance themes, eight light and three dark, plus the two typography themes they pair with: 气泡 bubbles for the light band, 灯带 neon for the dark one) — with the author-facing guide: both kinds' anatomy, the knob table, the core-token table, the two fences, the assets convention. `themeExamples.test.ts` holds every file there to the runtime's own predicates for its kind, so a sample can never teach a rule the validator would drop.
...
- **Select (下拉选择)**: use `components/common/Select`, never a native `<select>` — the browser's option popup can't be themed, so it renders OS furniture regardless of tokens. The trigger reads as a text input (`--color-bg-surface`, so the settings `--stg-*` remap reaches it); the menu portals to `<body>` like ContextMenu — app chrome by design, which also keeps it clear of `overflow` clipping in drawers. Selected option = accent tint + accent text per the list-item rule. The menu is **at least** the trigger's width and grows to its longest option, capped at the viewport edge — a trigger sized to its current value (「未绑定」) must not squeeze every other option to an ellipsis. A Select that is a form field's control spans the field like the text inputs beside it (the trigger is a `<button>`, which otherwise shrinks to its value in block flow). Surfaces with their own input dialect (AI panel inset/mono, lore paper) re-skin the trigger by passing a class **doubled in their own module** (`.x.x { … }`) — 0-2-0 outranks the base class deterministically, where a single class would tie and fall to bundle order.
- **Reusable keyframes** (in `global.css`): `fadeIn`, `scaleIn`, `slideUp`, `slideInRight` — reuse these, don't redefine per component.
- **Combobox (自由文本 + 补全)**: use `components/common/Combobox`, never `<input list>` + `<datalist>` — same reason as Select: the native suggestion popup is OS furniture (a white system pill under a dark-themed field). The field stays a plain `<input>` wearing the caller's own class; only the menu is ours, and it reuses `Select.module.css`'s menu/option classes so the two dropdowns read as one family. Suggestions never constrain the value; a suggestion equal to the current text isn't offered back.
- **Selected/active list items**: `--color-accent-tint` fill + `--color-accent` text.
- **Focus ring**: `global.css` declares it as **`:focus-visible:focus-visible`**, and the doubling is load-bearing — do not "tidy" it back to one. The ring is a `box-shadow`, so it competes with any component rule that sets `box-shadow` on the same element, and the app's usual button reset (`.btn { all: unset }`) ties a single `:focus-visible` at 0-1-0. Ties fall to source order and CSS Modules are injected **after** `global.css`, so the reset used to win: the control focused with nothing on screen. At 0-2-0 no single class can tie it, while a component that shows focus its own way (`.input:focus-visible { box-shadow: none }` in AgentChat, where the whole row lights up via `:focus-within`) still wins on order. `focusRing.test.ts` ratchets the selector.
- **Density tiers inside a resizable panel**: use a **container query** on the panel (`container-type: inline-size`), not the width in `appStore` — while a resize handle is being dragged the width lives in a CSS variable and the store value is the *pre-drag* one, so a JS breakpoint changes tier only on mouse-up. Mind the off-by-one: the query measures the panel, and the sidebar's 1px right border is inside its width (border-box), so a 240px rail is a 239px container — see `components/layout/RecentProjects.module.css`. A container query can swap layout but not words: render both label spellings and let it hide one. **An `@container` rule needs an ancestor that declares `container-type`, and when there is none it silently never matches.** No warning, no test: the theme cards' wide 「详情」 card shipped asking for 2 columns in a 1-column grid that way. Declare the container in the same CSS file as the query, on the nearest box whose width is the one the tier is about (there it is the card `.grid`, not the settings pane).
...
- **One remap, not eighteen restyles**: every settings module consumes the same `--color-*` vocabulary as the rest of the app, so `SettingsPage.module.css` re-points those roles at `--stg-*` **once on `.page`** (custom properties resolve at use time, so the whole subtree — panes, drawers, probe panel — follows). Element-level exceptions that the mapping can't express (kbd 键帽, stat cards `#F7F1E2`, hint blocks `#F8F2E3`) read their `--stg-*` token directly. (`--stg-bar` `#C68B5A` used to paint the usage bar; since 2026-09 that bar is segment-coloured — see 用量计量条的分段色 below. The token stays defined so themes that declare it keep validating, but nothing paints it.)
- **用量计量条的分段色** `--usage-seg-{input,cache,output,count,duration,other}`（`tokens.derive`，**不在 41 个核心契约里**）：用量页明细行那根条，**长度**是调用次数相对同批最大值（没变），**颜色**是这笔钱花在哪一种量上。五个具名段各借一个已有的语义色相（输入←`--color-type-text-fg`、缓存←`--color-success`、输出←`--color-sienna`、张数←`--color-type-image-fg`、时长←`--color-type-video-fg`），第六段是中性的 `color-mix(--color-text-dim, --color-border 25%)`，表示「这笔钱分不出来」——**它弱是因为它没内容**，不是第六个维度。段→类名的映射是 `UsagePane.tsx` 里一张**静态** `Record`（`cssModuleClassRefs` 那条守卫对模板字符串下标失明，动态拼类名打错字不会报错）；段的顺序只在 `lib/ai/usageMeter.ts` 定义一次。外观主题不声明这六个就跟着公式走，想定制直接声明同名 token 覆盖。可辨性量过：六色两两 15 对，paper 最小 ΔEok 0.100、night 0.099；六套内置主题里最弱的是 `ink` 的「张数/时长」0.067。
...
#### v2 · 条目 = 主条目 + 特征 + 配图（设计稿屏 14/15/16）

设计稿后来加的三屏把知识库条目重述成**三段结构，直接对应磁盘上的三样东西**：`index.md`（主条目）、若干带 frontmatter 的 `*.md`（特征）、`images.md`（配图）。数据模型早就是这样（`LoreFacet` 的 mode/keys/group/priority、`LoreImage.desc`），只是 UI 还按旧稿分栏，所以这一轮是把界面搬到模型上，不是加字段。

- **详情页三栏**（屏 15，`LoreDetail`）：`主条目 320px | 特征 flex | 配图 300px`。取代了旧的 `正文+特征+图集 | 关系 | 出场密度`——关系与出场是上一轮"按结构落地的空态"，v2 稿把它们整个移出了详情页，所以连同密度时间线一起删掉了（要接关系图谱时按新稿重开一栏，不要复活旧空态）。左栏底部的 mono 文件树是这套语汇的锚：作者随时能看见条目在磁盘上长什么样。
- **每一栏自己滚**（屏 15 的三栏结构 = 三个 `flex-shrink:0` 栏首 + 三个 `overflow-y:auto` 主体 + 钉在栏底的落款）：主条目栏也照此拆成 `.indexHead` / `.indexScroll` / `.indexFoot`。**别让栏本身当滚动盒**——`.colIndex` 一度直接 `overflow-y:auto` 且给正文块配了 `min-height:0`，于是 flex 把渲染好的 markdown 压到低于内容高度，而它不裁剪，正文段落被画到 mono 文件树和「AI 编辑助手 / 拆分条目」上（视觉重叠 69px，实测方式：把正文块的 rect 裁到滚动祖先的 client rect 再和落款 rect 求交，肉眼数像素靠不住）。栏首独立钉住还有第二个理由：三栏的 head 在设计稿里是齐平的一条基线，跟着正文滚就散了。
...
墙上摊开的一张纸**——纸 820 × 内容量高、正文测量 660 · 16px/1.85（`md-body` 面，`--md-size`
归这块面）、硬阴影升一档 `--lore-card-shadow-xl`、**整墙滚动纸不自滚**；卡片的 22px 装订边放大成
纸左沿 30px（窄态回 22），分类退成纸顶 2px 色带 + 头像底色。

- **命中词**（名称 + 别名，命中任一即注入——这行是把「别名就是激活关键词」写明白的地方）：
  无框 chip，`--color-tag-bg` 底 + 2px `--color-sienna` 下划线——它是「会被命中的词」，画成划线的
  词而不是又一个盒子；别名与名称同形，数据上平权。词汇上叫**命中词**，与特征的「触发词」（keys，
  命中之后的二级开关）分开。
- **注入记号收进节头，不打断行文**：常驻＝28px **3px 实赭石**短线、自动＝**1px 实中性**
  （`--lore-neutral-line`）、手动＝**1px 点线中性**——三种线在两套主题里都不靠颜色深浅区分，靠
  **粗细与断续**；手动特征**绝不降透明**（作者点名才注入是机制，不是内容次等）。元数据是标题下
  一条 11px mono 细行，**恒可见**（hover 才出＝键盘与触屏读者永远读不到）；只有**编辑记号**
  （24×24、纸左页边 -38、真 `<button>` 常驻 tab 序列）按 hover / focus-visible 显形。
- **互斥组是骑缝组边**（9px 端头 + 1px 竖线 + 组头一条 mono 说明），不是虚线盒——盒子会把两节
  完整正文压成引文块。组内按优先级降序，首条挂赭石「优先级 N · 当前采用」。
- **只读不催**：缺口（`+ 补上这一面`）、「无描述—点击添加」这类催办一行不画；降级＝元信息行位置
  换一条 10px mono 注（下沿 1px dashed，分类色带/头像底换中性——分类色不再可信时不假装它还在）；
  空条目＝档案头 + 概要 + 一句邀请，纸短本身就是「还没写」。
- **词典条目**（`dict`）正文渲染成三栏词表（原文 mono 13 / 译文 Spectral 15 / 备注 italic 12
  faint；行分拣 `splitDictBody` 逐行喂 `parseDictBody`，解析不了的行按原样 markdown 落词表下方）。
- **页边目录** ≥1300 容器宽出现在纸右 200px（sticky；刻度＝面 8×1 中性 / 当前 14×3 赭石，缩进一级
  ＝节，只展开当前面的子条目）；**≥1400** 图库 3 列，且特征的 mono 边注**推成真边注**：同一串文字
  按 · 拆行、落在纸右沿外 120px 栏（border-left 细线，`.metaSide`，显隐全在 CSS）——目录不随宽度
  跳边、恒在更外侧右边（1f-1 与 1d-2 的矛盾裁决记在 brief 出入表 #1）；≤900 纸贴满、padding 收窄、
  图库 1 列、编辑记号隐藏。纸宽只有 820 与「贴满」两种，中间不做无级伸缩——行长一跑出 660 的舒适
  区全部判断都不成立。
- **改完回原位的淡染**：`FacetEditModal.onSaved` / 编辑表单保存后，被改那节播一次
  `--color-accent-tint` → 透明 320ms（延迟 160ms 让位收场动画）。**档案头大图**：`index.md` 可选
  `cover:` 字段（同 dict/collections 的缺席不写行纪律），入口是 lightbox 里的「设为档案头图」，
  缺省仍取第一个配图组的第一张。
- 与设计稿的出入（配图描述记号缓做等）记录在
  [`lore-browse-mode-ui-brief.md`](../feature/lore/lore-browse-mode-ui-brief.md) 的实现记录。

#### 集合 · 装订语汇（设计稿屏 24–31）

知识库有了第二根轴（数据侧见 [`lore-collection-plan.md`](../feature/lore/lore-collection-plan.md)）。
这一套的全部前提是一句话：**分类用颜色，集合用装订**。分类六色（`catColor.ts`）已经占满
了墙面，还兼着头像底色与候选徽标，集合再引入一套彩色，两套颜色会在同一张卡片上互相打架。
所以归属由**边、竖排短名、翻面、骑缝线**来说，一律中性色阶——`--lore-bind-*` /
`--lore-flip-*` / `--lore-edge-inner` / `--lore-neutral-line`，逐个 token 在 `tokens.css`。

- **装订边**（`.edge`，22px）：卡片左侧竖带，一个集合一道竖排 mono 标（`bindingLabel`：
  剥掉书名号取前四字，全名进 `title`），多道之间一条 10px 细线。**未归集不写字**——空着的
  边就是答案；给每张未归集的卡挂提示徽标等于把「还没分类」渲染成一种错误。
  卡片因此从「一个带 padding 的盒子」变成 `装订边 + .cardBody`，padding 移到卡身上。
- **围栏 ≠ 筛选，这条分界写在形制上**：筛选让卡片消失（`.filters` 的分类 chips、装订栏
  里点集合名），围栏让卡片**翻面**（`.flip`：`--lore-flip-bg` + 虚线边，只剩名字与所属
  集合）。围栏还带三样筛选**永远不产生**的东西——顶部 `3px double` 骑缝带（`.band`）、
  右页边 26px 竖排标注（`.pageEdge`，`box-shadow: inset` 造第二道线）、装订栏底部的围栏
  说明卡。**不要**把 `3px double` 借给任何别的状态：它此刻是「AI 的视野变了」的唯一记号
  （多选条 `.selectBar` 是唯一的例外，且理由同源——这一刻墙的规则变了）。
- **围栏不是错误态**：没有红、没有警告黄。翻面的卡仍可点开、可编辑、可搜索，文案说的是
  事实（「AI 看不见」），不是警告。同理未归集只在三处出现（装订栏末行、围栏生效时的墙底
  一行、卡片的空装订边），**第四处就是噪音**。
- **置顶越栏**是围栏外唯一保持正面的卡片：赭石描边 + `◆ 你已置顶` 徽标 + mono「显式指定 ·
  越栏生效」。它不是漏网，是规则本身（显式 > 围栏）。
- **分类 chips 从圆点改成 3px 左边**（墙上的筛选 chip；新建条目模态里的分类选择仍用圆点，
  那里没有第二根轴要区分）——卡片左沿此刻已经有一条装订边在讲归属，chip 上的色块换成同一
  种「边」的语汇，两根轴才读得出是两根轴。
- **退化**：集合 > 8 时切换器出 mono 过滤框 + 滚动 + 「其余 N 个」。墙面不退化，因为它
  从不按集合分区。
...
- **分类说明的摘要行**（设计稿 01b TURN 2 屏 2e-2）：筛到某分类时 chip 行下一行——13px `NotebookText`（`--color-text-faint`）+ 12px/1.55 衬线 `--color-text-muted` 正文单行省略（宽度封顶 520px）+ 右端 10px sans faint「右键分类可更新」。它是**只读的一行字，不是控件**：没有 hover 态、没有点击，入口仍是分类 chip 的右键，那句尾注就是指回去的路标。「全部」不显示；没有说明时这一行**不渲染**，不留空高——空态不是「暂无说明」的占位，是没有这一行。
...
- **思考过程 `ThinkingPanel`**：`--color-bg-stream` 折叠带，头行 `思考过程 · 实时流式 · 不入库`；正文斜体衬线 12px/1.8、顶部渐隐、闪烁光标，流式时自动展开+跟滚，结束自动收起（作者手动切换后以作者为准）。**不入库**是承诺：reasoning 只进这块面板，不进任何文件。
- **分工**：走 agent 工具环的两个流（更新条目、特征 AI 助手）的步骤+思考过程仍由 `AgentLog` 承担（工具粒度更细），只补状态行；三个流（提取 / 拆分整理 / 主条目补全）用全套。拆分整理后来改成了工具环（一次 `split_facet` 提交一条特征，见 `lib/agent/splitTools.ts`），但仍留在这一档：作者要看的是"拆到第几条了"，不是工具调用回放——所以第二段步骤的 mono 注记挂**已提交的条数**（`概述已提交 · 已提交 3 个特征`），这也是唯一能让中途停止的运行说清自己留下了什么的地方。数据源是现成的：`generateLore`/`splitLore` 本就跑在 runtime 上，把 `onEvent` 穿透出去即得 reasoning 与 token 总量（此前 `onEvent: () => {}` 白白扔掉）；结构化流用 `runStructuredTask.onReasoning`，token 是 `estimateTextTokens` 估算。
- **各模态对齐**（屏号 → 组件）：08 提取 = 分类范围多选 chips（实底 sienna 选中态，至少留一枚，收窄 `generateLore.allowedCategories` 的枚举）；09 更新条目 = 左栏改为**写入目标列表**（主条目 / 各特征 / 虚线 `+ 生成新特征`），GOAL 预设删除（v2 无此栏，指令框仍在），footer 主键 `应用到 <文件>`；**生成新特征**走 `draft_lore_facet` 结构化单发（标题/触发词/注入方式/正文一次成稿，虚线草稿条可改，Apply → `createFacetFile`）——这正是屏 17 演示的那个任务；10 特征助手 = 快捷动作改单行描边 chips（新增 压缩正文；检查互斥组未做，见下），触发词结果多一排 diff 绿 `+词` 预览；11 拆分整理 = 改名 + 头部统计签 `正文 N 字 · M 特征` + 方案卡补「自动」徽标与「触发词」行标 + footer `应用 · 生成 N 条特征`；12 主条目补全 = 改名 + 全量 i18n（原先整页 `isZh ?` 三元）+ 建议概要 diff 绿；13 AI 中心 = 代码 defaultValue 与 locale 对齐（此前两套文案）；18 生成配图 = 斜纹占位卡（`生成中 · Ns`）+ 补上此前不存在的**图库描述**输入（`note` 状态一直有，只是没有 UI）。
- **术语**：主条目的那句话统一叫 **概要**（屏 15 v3 把「摘要·命中即注入」改成了「概要·命中即注入」；`summary` 字段名与 i18n key 不动）。拆分保留段叫 **正文概述**（屏 11 原文）。
...
- **确认条就地展开，不弹模态**：折叠后的「已折叠 N 个分组 · 选区缩为 N 项」贴在工具行下方
  ...
  就是「删哪几个」的清单，文案因此只说数量。**同一套版式有两个语气**，只换那根 `inset 2px`
  竖条和底色：危险（删除，`--color-danger-*`）与警告（修复失配的 `assets/` 分组，
  `--color-warning-*` + 一排候选文档 chip）—— 修补不是销毁，用红色说它会让作者以为要丢东西。
- **脚线 22px 常驻，三件事共用、永不叠加**：三个计数 / 剪贴板（赭石底，剪切与复制的唯一区别
  是行变不变淡）/「定位当前文档」。顶部省下的 154px 换这 22px。第三件事认**两种**看不见：
  被折叠掉（一次集合查询）和滚出视野（**只给当前那一行**挂一个 `IntersectionObserver`，
  root 是滚动容器）—— 「不按行测量」这条性能保证约束的是**遍历**，一行的观察者不是遍历。
...
- **Sent-message references (2b)**: `@[名称]` tokens in a sent user bubble render in the ref-chip accent (`--color-accent-text`) via `lib/agent/mentionText.splitMentions` — the bubble is the record of the composition the chips described. The token's one definition is `mentionText.mentionToken`: `@[` + a newline-free label + `]`, where brackets inside the label pair up (a paired `潮汐[旧]` stays as it is; a lone one, or a `[` straight after an `@`, is written full-width — so a label never holds `@[`), and readers count bracket depth to find the closing `]`, giving up at a newline or a nested `@[` (that is a new token, and the `@[` before it was typed by the author); an unclosed, cut-off or empty token stays plain text. The chat composer's mention picker anchors **above** the input (`preferAbove` — the composer sits at the panel's bottom edge); lore modals keep below-first.
- **`@` 选择器的作用域 (02i)**: 列表顶上一行 chip——「全部」恒在，其余按候选里有什么画：有条目才有「条目」、有文件就有「文档」（只有图也留着，chip 行不因有没有图变形）、有图才有「图片」——**档位组同一语法**（idle `--color-text-dim` + `--color-border-panel` · hover `--color-bg-elevated` · 选中 `--color-accent-text` / `--color-bg-selected` / 赭石边），固定在列表顶上（行在它下面单独滚，240px），右端 faint mono「Tab 切档」。Tab / Shift+Tab 切档（从空档按 Tab 跳过同样空的档，事实行说的「Tab 切过去」才是一下），**只有 Enter 选中**（Tab 从前也选中；改义与 ⌘K 对齐）；英文下 chip 是首字母大写的复数（All / Entries / Documents / Images），事实行里是小写复数；每次 `@` 新开回到「全部」，不跨次记忆——⌘K 记档是因为它一次只开一回。**空档不消失**：chip 行留着，下面一行 10.5px mono 事实「条目里没有「夜航」· 文档里有 1 篇 · Tab 切过去」（数字 `--color-text-primary`、Tab 赭石），全为 0 写「没有匹配」，此时 Enter 不再被吞（哪儿都没命中的 `@` 多半只是个 `@`）。文档行多一行 10px mono 分组路径副行，靠别名命中的条目副行是那个别名；命中片段名字与副行都可高亮（`--color-mention-bg` 底 + 赭石字，同 ⌘K）。空查询按 条目 · 文档 · 图片 交错，不分段——十行分三段等于没分。设计与被否方案：`docs/feature/agent/mention-scope-ui-brief.md`。
- **写手署名 (12 · 写手 Writer)**: 一条回复是不是写手写的，答案不在回复**上**，在回复**左边那条槽**里 —— 13px 横刻度 (`--writer-tick`) 起头、1px 竖线 (`--writer-rule`) 贯下，**线的长度正好等于写手写的那段文字**：上不越过助手的执行日志，下不越过最后一个字。为此 `.assistantTurn` 的左槽从 8px 加宽到 **13px**（刻度的宽度就是这套记号的字号，收窄它等于裁掉它要承载的那个记号），7px 圆点改为在其中居中；写手那一块是**同一个网格的第二行**而不是 `.turnContent` 的子元素 —— 两者必须落在同一根槽列、并且左边缘完全对齐，嵌套进去必有一个会缩进（实测 `bodyLeft === contentLeft`，`rule.bottom − prose.bottom === 0`）。模型名只在会话**第一轮**常驻，之后靠悬停叫回来（悬停根是整条回复，因为左槽与正文是网格里的两个兄弟单元格，`:hover` 跨不过兄弟——而「悬停任意处」本来就是设计要的）。三个态：正常＝整条实线 · 降级＝顶端 16px 虚线（标的是**接缝**可疑，不是正文可疑）· 写手没跑＝**没有线**（缺席本身就是信号）。被否掉的两个做法记在 `WriterTurn.tsx` 顶部：徽章标不出边界，换底色/字色是隐形的第二强调色。
- **工单卡 (3a/3c)**: 只有**一层**展开，并且挂在交接行上、**不在执行日志里** —— 工单不是子代理的产出，它是这一轮的接缝（`logModel.roundRows` 把它从日志里滤掉）。卡上只有**两处衬线**：目标那一句和文风锚点的原文（它们本来就是给人读的话），其余全部等宽与无衬线 —— 衬线一断，读者就知道下面是清单，不是文章。空的整节**直接不渲染**：没有灰字占位、没有「无」，卡片从 6 节缩到 1 节，高度差本身就是信息。
- **写手不是第七个芯片 (5a/5b)**: 芯片行的每一个都是「这个能力可用」；写手是「从现在起每句话都由它写」——一定生效，且改变每一轮的成本和时长。它从方框里出来，变成**输入框上边那条分隔线本身**（`WriterStrip`，13px 刻度起头 + 发丝线贯通，与左槽是同一个记号系统：上边说「接下来谁写」，回复左边说「这段是谁写的」）。角色扮演面板既没有这条线也没有第七个芯片——那里不经过写手，而一个按下去没反应的开关比没有开关更糟。
...
- **主题覆盖是有测试守着的**, because the failure mode is silent: `tokens.css` is five cascade layers (`tokens.scale` · `derive` · `scheme` · `theme` · `user`) and several section comments appear in more than one block, so a family appended next to "the" anchor can land in the wrong layer or the wrong scheme and override the values it was meant to complement. `themeContract.test.ts` pins the rules — a token is either on `:root` or in **both** `[data-scheme]` blocks (and both `[data-theme]` hand-tune blocks), scales stay theme-independent, and the snippet surfaces (plus the shared `ContextMenu` they borrow) contain **no** raw hex/rgba at all. After adding any `--x-*` family, also read it back in the running app per theme: `getComputedStyle(document.documentElement).getPropertyValue('--x-…')`. Screenshots do not show this.
...
### 模型编辑抽屉 (Model editor — 设置 → 渠道与模型 → 一条模型)

Transcribed from 设计稿 `19 模型编辑 Model Editor`; the brief and the landing record are in [`docs/feature/model-drawer-redesign-brief.md`](../feature/model-drawer-redesign-brief.md). Three rules, each with one visual word:

- **Fold by "has a value", never by importance.** 身份 → 计费 → 限额与实测 → 思考 → 能力声明 → 采样与前置提示词 (an image model swaps the middle four for 出图). A section with nothing set collapses to its eyebrow label plus a right-aligned line that says what *not* being set means (`未填 · 用量页记 $0`); a set section's folded line is its values in mono. Decided **once from the stored row** when the drawer opens (`initialOpen`), so clearing a field mid-edit never snaps the section shut. 身份 never folds; 添加模型 also opens 计费. 「展开全部」 is for the relay author's first pass.
- **Dashed = nothing sent.** The invariant every relay depends on is that an unset field sends nothing, so "unset" must not look like "set to 0". One 1px dashed edge everywhere it applies: an empty input (`.unset`), a *selected* 自动 / 跟随默认 / 普通模型 chip (`.chipAuto` — hollow, dashed, ink text; solid accent is reserved for a value the author chose to send, 「关闭」 included), an off toggle (`.toggleUnset`), a folded empty section (`.dashMark`, 7px) and the empty 「将发送」 line (same square). Temperature is the test case: empty = dashed + 「不发」 placeholder, `0` = solid + a mono 「确定性」 tag.
- **Only what is below the clicked control moves.** Sections, conditional fields and 「为什么」 blocks all open by `grid-template-rows 0fr ↔ 1fr`, 200ms `var(--ease-settle)`, opacity 160ms, same curve both ways; no horizontal motion, no scale; chips swap in place with no enter/exit. Folded content also gets `visibility: hidden` (after the 200ms) so it cannot take focus.

Two lines with a fixed shape:
...
- Reset a control with `all: unset` and assume the focus ring survives — it does now (see 组件模式 above), but check with the keyboard, not the mouse: a control that is invisible or zero-sized at rest and only revealed on `:hover` is still a focus trap for keyboard users. Reveal it on `:focus-visible` too.
