...
- **Density tiers inside a resizable panel**: use a **container query** on the panel (`container-type: inline-size`), not the width in `appStore` — while a resize handle is being dragged the width lives in a CSS variable and the store value is the *pre-drag* one, so a JS breakpoint changes tier only on mouse-up. Mind the off-by-one: the query measures the panel, and the sidebar's 1px right border is inside its width (border-box), so a 240px rail is a 239px container — see `components/layout/RecentProjects.module.css`. A container query can swap layout but not words: render both label spellings and let it hide one.
