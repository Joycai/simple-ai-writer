...
- **Tokens** — `src/styles/tokens.css`, five cascade layers declared once, `@layer tokens.scale, tokens.derive, tokens.scheme, tokens.theme, tokens.user`, and the blocks appear in the file in that same order (LightningCSS rewrites the statement into first-occurrence order when it bundles):
  ...
  - `tokens.theme` — the built-in themes' **hand-tuned** values for derived tokens (139, identical sets for `paper` and `night`). A token is hand-tuned unless its formula reproduces the designer's value exactly in both schemes; that rule, not taste, decides which block a token lives in. A user theme sets a different `data-theme`, so none of these reach it — its derived tokens fall back to `tokens.derive`, which is why a blue accent recolours the settings switches instead of leaving them sienna.
  - `tokens.user` — reserved for a theme file installed at runtime (S2 of `docs/feature/theme-system-plan.md`).
...
- **Theme Modes** — dark, light, system (auto-detect); `app:theme` is the mode, and the built-in theme for the resolved polarity is applied by `applyScheme()`.
...
- Containers carry the global `md-body` class (`MD_BODY_CLASS`); the theme comes from `data-md-theme` on `<html>` (state in `appStore.markdownTheme`, persisted as the `app:markdownTheme` preference in `config.db` — see `lib/prefs`). A container may set the attribute on *itself* to pin one theme regardless of the app setting — the settings picker's live samples do this, which is why the generator emits pinned blocks after inherited ones.
...
### 模型编辑抽屉 (Model editor — 设置 → 供应商与模型 → 一条模型)
