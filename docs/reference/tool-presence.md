...
| `toolTable/shared.ts` `galleryViewer` | 图集抬头三档 here / delegate / none，两条臂都查 `allowedTools`（见 [subagent-lld §6.1.4](../feature/agent/subagent-lld.md)） |
| `chatRefs.ts` 未发送图片段 | 只在识图子代理真的在时，才教模型 `delegate(kind:"vision")` |
| `aiTaskStore` / `stores/agent/chatJob.ts` 的两份清单 | 工作流卡与 docx 格式表跟着 `read_workflow` / `export_docx` 走，不跟档位走 |
