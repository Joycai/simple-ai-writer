...
>
> **2026-09-23 增补**（对照 09-22 版文档：新增 5.0 flash、透明背景两款都支持；套餐 key，22 条零成本探测 +
> 3 张计费）。**每个不支持的参数都在出图前 400**，报错文案点名字段——所以下面的探测都不花钱：
>
> | 请求 | 回应 |
> | --- | --- |
...
> | 同上 + 一张 RGB PNG | 400 `param:"image"`「requires a PNG input with at least one transparent pixel」 |
> | 同上 + RGBA PNG + `output_format:"jpeg"` | 400「must be png when background is transparent」 |
>
> **透明模式锁死原图的 alpha 遮罩**（pro，输入 = 透明底上一个红色圆，计费 2 张）：「把红圆改成蓝色」→ png，
> 圆外依旧全透明 ✅；「给红圆加上蓝天白云背景」→ 圆外**依旧全透明**，天空被画进了圆里（上半圆成了天空）。
> 响应 `data[]` 多了 `output_format:"png"`，`usage` 多了 `input_images:1`。
>
> **对本项目**：`ImageCaps.transparent`（pro / flash 起步行声明）；改图工具的 `keep_transparency` 由 agent 决定
> ——因为「加背景」在透明模式下会被吞掉；「没有透明像素」那条 400 免费，adapter 去掉两个字段重试一次。
> 按量平台加 5.0 flash 起步行（未实测）。取舍见 `docs/feature/image-generation-plan.md` PR7 的「09-23 增补」。
