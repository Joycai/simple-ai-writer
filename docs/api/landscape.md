...
- **未测**：付费模型的任何生成（含 Claude 原生的 thinking / `output_config`、
  ...
  第十八个样本补上**——那里也改了本节两处判断：③④ 回包是上游原样，但请求
  侧会被重新序列化；① 与 ② 的默认线路背后是一层 OpenRouter 形态的翻译。
...
- **thinking + 并行工具**：一轮里两个 `tool_use`，每个带新字段 **`caller: {"type": "direct"}`**（程序化工具调用的来源标记）。
  回灌时：原样回灌 ✓；**丢掉 thinking block 也 200**（旧规则是 400）；去掉 `caller` 200；**改 `signature` → 400**
...
- **`thinkingBudget` 仍收**，但 **`thinkingBudget: 0` 关不掉思考**（仍 312 思考 token）。
...
- **函数调用带 `id`**（`call_1626125`，新）：并行两个调用，签名只挂第一个 `functionCall` part；流以一个**光秃秃的
  ...
  而带签名的 `{text: "", thoughtSignature}` 回灌 200。
...
- Gemini 适配器回灌模型 parts 时跳过光秃秃的 `{text: ""}`（带签名的保留），否则 Vertex 上每个流式工具轮的第二轮都 400
  （`gemini.ts`）。
