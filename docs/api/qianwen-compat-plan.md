...
| `qwen3-asr-flash-filetrans` 只发 `file_urls[]` | `input: { file_urls: ["oss://…"] }` + resolve 头 | 提交 200，任务 `FAILED · InvalidParameter.MalformedURL: A valid file URL is required`——这代**只读 `file_url` 单数**；qwen-audio-3.0 两种都读。`file_url` + `file_urls` **一起发**两代都 SUCCEEDED（含分离 + `speaker_count`） |
...
| F ✅ | §4.4 第 4 片 | `responses-effort` 类目（`off/low/medium/high/xhigh/max`，`off` → `none`）；`reasoning:{effort,summary:"auto"}`；`reasoning_summary_text.delta` → `{reasoning}`。`summary` 只在开启时随行（关闭时只发 `effort:"none"`）；该族的 `defaultCategoryId` 从 D 片的临时 `off` 改成这个类目，所以「自动」也能调；跨族类目（导入的 `openai-generic` 行落到 Responses 供应商下）**原先是被照单全收的**——`resolveThinkingCategory` 只对旧 dialect 做同族守卫，声明的类目直接信任，于是 `reasoning_effort` 会上这条线；这一片把同族守卫扩到类目（`off` 跨族通用），跨族退回本族默认，对 ①/③/④ 族同样生效 | 单测（`responses.test.ts` + `thinkingCategory.test.ts`）：每档的 wire 串、未设置不发、关闭发 `none`、跨族类目退回本族；作者在 gpt-5.4 上选 `max` 应得到端点 400（这是对的，5.4 到 `xhigh`）——**不做**按模型裁剪 menu，同 ① 族"端点 400 是声明"的做法 |
| G ✅ | §4.4 第 5 片 | `jsonMode.ts` ② 族分支 → `text.format:{type:"json_schema",name,schema}`，**不发 `strict`**；千问上按 400 学习退回强制工具。落地细节：schema 照样过 `strictify`——省略 `strict` 在官方端点等于自动升成 strict，端点会按 strict 的规则（全字段必填、`additionalProperties:false`）校验，所以 schema 必须先按那套规则整形，只是不写那个键（显式 `strict:true` 是中转站整个丢掉 format 的触发条件）；`json_object` 同 ① 族的 "json" 前置；该族的自动档同 ① 族按 `KNOWN_JSON_SCHEMA` 抬到 `json_schema`；`isJsonModeRejection` 加认 `text.format` 字样。千问上「报错还是忽略」仍未验（§4.5）——忽略是唯一学不到的情况，那时提示词里的 cue 兜底 | 单测（`aiJsonMode.test.ts`）：`text.format` 无 `strict` 键、无 `json_schema` 包装、无 `response_format`；`json_object` 的 cue 规则；无 schema 降级；ceiling 备忘按本族 standard 计；`aiModelSummary.test.ts` 「将发送」行显示 `text.format`。作者跑一次条目生成 |
| H ✅ | §4.4 第 6 片 | `image_url` → `input_image`（data URL 直传）、`file` → `input_file`。映射在 D 片随 adapter 一起落了（不映射就是静默丢图）；`responses.test.ts` 的 part 用例也是 D 片的。`input_file` 的形状是文档形状，实测在 I 片的 live 文件里（一个程序生成的单词 PDF） | 作者发一张图 |
| I ◐ | §4.4 第 7 片 | `live.openai-responses.test.ts`（`OPENAI_KEY` + 可选 `OPENAI_RESPONSES_BASE` / `OPENAI_RESPONSES_MODELS`；官方 base 走 `openai_responses`，其它走 compat）；结果回填 `responses.md` §9 与 `landscape.md` | 文件已落、无 key 时整组跳过；每款模型 12 条：连接测试、默认不发 `reasoning`、off / medium / max（5.4 应 400）、`max_output_tokens` 截断、工具一轮 + 原条目回传第二轮、思考中强制 `tool_choice` 单次请求、`text.format` 的 json_schema（无 `strict`）与 json_object、`input_image`、`input_file`。**跑与回填待一把新 key**——把 §4.5 剩下的三条定掉 |
