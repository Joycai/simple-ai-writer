...
类目清单：`openai-generic` / `deepseek` / `qwen-budget` / `qwen-effort` / `glm` / `doubao` /
`gemini3` / `claude-adaptive` / `claude-budget` / `minimax` / `doubao-switch`（后两个豆包类目 2026-09-18 加，
理由见 [`landscape.md`](landscape.md) §7 第十二个样本），外加 `auto`（＝按族默认）
...
> 当时发出去的字面量 `extra_body` 键被端点忽略，思考照常。**已修**（该方案第 1 片）：现在发顶层 `thinking:{type:"disabled"}`。第二条与第四条在千问上
...
2. **折叠态也要说话。** 没有做成通用的 `ExpandableRow`，因为"思考过程 · 2.1s"
   ...
   恰恰是它的**第一行**通常就是关键（模型有没有理解选区？有没有找到那条条目？）。
   所以折叠态带一行截断的正文。
