...
- **CC 的强制工具**两面都不写：不带思考时全调用，带 adaptive 思考时 3/8、4/8。判「不发」会丢掉能调用的那部分，
  结构化任务的回退链本来就兜得住「没调」。
- **合并渠道**（`channelMerge.ts`）时两张前缀表取并集，同一前缀以保留的一行为准。两行是同一台中转站，并起来的表比哪一半都全，所以：
  - **手选的上游（含「不按上游」）合并后不变**；同 id 折叠时保留行的手选优先。
  - **按前缀表得到上游的模型，合并后不变**：并表对它说法不同时（同一前缀两种映射，或被吸收表里更长的前缀改写了保留行模型的解析），
    把合并前的结果写成它自己的手选；
  - **其余模型跟随并表**，不冻结成「不按上游」；同 id 折叠时，保留行既没有手选、也没有按前缀表得到上游，才用被吸收行手选的上游。
    **只从 id 认出来的上游**（kiro、bedrock）算在「其余」里：它不是谁的设置，合并前后 id 不变、认出来的也不变，不必钉成手选；
    若算作「有上游」，它会盖过被吸收行专为这个 id 做的手选（`kiro-claude-opus-4-6` 被吸收行手选 CC，合并后退回 Kiro，④ 面 PDF 由发变不发），
    钉成手选还会让抽屉把一个推断显示成「手选」。
