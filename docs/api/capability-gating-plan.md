...
- **合并渠道**（`channelMerge.ts`）时两张前缀表取并集，同一前缀以保留的一行为准。两行是同一台中转站，并起来的表比哪一半都全，所以：
  - **手选的上游（含「不按上游」）合并后不变**。同 id 折叠时先看保留行的手选，再看被吸收行的手选——后者是专为这个 id 做的，
    比保留行表里的一行前缀更细，不能被它静默盖掉（`[x]` → anti 的表盖过被吸收行手选的 CC，① 面 PDF 由发变不发）。
  - **没有手选、按前缀表得到上游的模型，合并后不变**：并表对它说法不同时（同一前缀两种映射，或被吸收表里更长的前缀改写了保留行模型的解析），
    把合并前的结果写成它自己的手选；
