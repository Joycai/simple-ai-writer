...
| 大纲 · 全图（主视图） | `components/outline/OutlineFullView.tsx` | 卷/章排序（book spine）、逐章前情摘要、批量移动 |
...
2. **章节重命名/删除**（右键菜单）：重命名裸名保留原扩展名（.txt 不会悄悄变 .md），并把 spine 位置、在写状态、前情记忆一起迁到新 relPath；删除走 `deleteEntry({backup: true})`（与文件树相同，先快照进 `.ai-writer/backups/`），并清掉 status 残留。
