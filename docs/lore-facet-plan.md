# Lore 侧面（Facet）系统落地方案

> 目标：把 lore 的注入粒度从"实体"降到"侧面（facet）"，配合分层预算注入（调研方向 1+3），并用 AI 自动拆解已有条目降低作者负担。涵盖数据模型、注入引擎、AI 拆解流程、UI 更新、迁移兼容与分期计划。
>
...
1. **facet = 实体目录里的普通 `.md` 文件**。`readEntity` 已经在收集 `mdFiles`，LoreDetail 已把非 index 的 md 显示为"附件"——只需给这些文件加 frontmatter 就升级为可激活侧面，**目录结构零改动，老项目零迁移**。
...
实体目录下除 `index.md`、`images.md` 之外的 `.md` 文件，**带有 `facet` frontmatter 的**被识别为侧面：

```markdown
---
facet: 战甲形象            # 侧面显示名（必填，作为识别标志）
...
- **frontmatter 在扫描期解析，正文注入时按需读取**——避免大项目扫描时把所有侧面正文常驻内存（`charCount` 在扫描时顺带记录）；
...
```
入口：LoreDetail 顶栏 "拆分侧面" 按钮（Scissors/Sparkles 图标，与"AI 改进"并列）
...
- "概要" tab 中现有"附件文件"区改造为 **侧面卡片区**：每个 facet 一张卡（标题、group 徽章、keys chips、~token 数、mode 图标）；无 frontmatter 的 md 显示为灰色"附件"，卡片上给"转为侧面"按钮（弹出 frontmatter 表单预填）；
- "新建侧面" 按钮：表单（标题/keys/group/mode）→ `createFacetFile()`；
...
全部新 UI 文案进 `en.json` / `zh-CN.json`（facet 术语建议：中文"侧面"，英文 "Facet"）。
...
| 目录里已有的无 frontmatter md | 惰性附件，不注入；UI 提供一键"转为侧面" |
| 旧版 app 打开新项目 | facet 文件被当普通附件忽略，index.md 照常工作（核心卡自足性由拆解提示词保证：core 必须独立成立） |
| 旧 pin 数据（纯 dirPath） | 原样有效 = pin 整实体 |

无 schema 迁移、无一次性脚本。`docs/architecture.md`（RAG 节）与 `docs/workflows.md`（新增"添加/拆分侧面"recipe）随 PR 更新。
...
**PR-2 Facet 管理 UI**
LoreDetail 侧面卡片区、新建/编辑/转为侧面、frontmatter 表单条。验收：不写一行 yaml 能完成 facet 全生命周期管理。
