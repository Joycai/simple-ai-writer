...
- **Send stamp needs a hairline**: the chat composer's send button is the mockup's raised block + 11px sienna square (an ink stamp, not an arrow), but on paper `--color-bg-raised` is nearly the composer's own `--color-bg-base`, so the block vanishes and the bare square reads as a floating stop icon. The stamp therefore carries `--color-border-input` (mockup shows none — dark didn't need it); the stop state recolors the same block to `--color-error` with a white square, which is what keeps 发送/停止 distinguishable at a glance.
