...
| [zhipu-plan.md](api/zhipu-plan.md) | `partial` | 智谱 BigModel (2026-09-19 live on glm-4.5-air / 4.7 / 5.3-flash): what the ① adapter already does unchanged, ten gaps against the current code (the four silent ones: an off switch 4.x ignores, forcing ignored or refused without naming `tool_choice`, `json_schema` ignored, mid-stream failure only in `finish_reason`), P1 built (the `zhipu` platform, `glm-switch` / `glm-effort`, a calibration table for all 11 model ids, auto-only tool choice, the three finish reasons, caps, starters) and P2–P5 proposed (web search, the Coding Plan's routes, preserved thinking, a `json_schema` cap) |
