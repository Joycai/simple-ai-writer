...
| [xlsx-export-plan.md](feature/xlsx-export-plan.md) | `shipped` (Beta flag) | agent 产出 .xlsx：一张 markdown 表格 = 一个工作表，**数字必须是数字**（前导零 / 15 位以上 / 带单位一律留成文本，百分数存分数）；生成放 Rust 而 docx 放 TS 的同一条规则（方言不过界，D1）；工具预算 +274 与「为什么不走延迟装载」（§9） |
