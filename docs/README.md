...
| [channel-model-route-plan.md](feature/channel-model-route-plan.md) | `partial` | 「供应商与模型」重构为 **渠道 × 模型 × 线路**：渠道 = 一把 key + 一个平台（每个协议族一条线路），模型挂当前线路，思考 / 最大输出 / 结构化 / 温度按「模型×线路」各一份，服务端工具的授权挂模型、拼写由平台画像按「平台＋协议＋模型 id」决定。§1 记下一个现存缺陷：`openai_compat` 上一律发千问私有的 `enable_search`，DeepSeek 直连 / New API / Ollama 上被静默忽略。含迁移（不自动合并，作者触发合并并在同一事务里改写 id 引用）、五期分期、七条不变量；设计稿是 app 设计项目的 `05k 渠道与线路`。**P0 已实施**（§11）：`lib/ai/platforms.ts` 画像 + `providers.platform`，服务端工具按平台拼写，修掉了上述缺陷 |
