...
| [tool-search.md](api/tool-search.md) | `living` `unverified` | Tool search / deferred tool loading across the families from the vendors' docs (2026-09-14): OpenAI GPT-5.4+ (`tool_search`, client execution, `additional_tools`), Anthropic 4.5+ (regex / bm25, `tool_reference`), xAI (spec only), none on Gemini or Chat Completions — and how that meets this project's own run-state loading and the rejected `load_tools` |
| [gpt56-plan.md](api/gpt56-plan.md) | `shipped` (P2–P4) · `deferred` (P1 / P5–P7) | GPT-5.6 (sol / terra / luna) support audit after the 2026-09-14 relay run: what already works on the Responses family, the gaps (Chat + tools while thinking, the reasoning echo nobody reads, verbosity, OpenAI's hosted `web_search`), and one proposal per gap, each marked by what it still needs an official key for |
| [zhipu-plan.md](api/zhipu-plan.md) | `partial` | 智谱 BigModel (2026-09-19 live on glm-4.5-air / 4.7 / 5.3-flash): what the ① adapter already does unchanged, ten gaps against the current code (the four silent ones: an off switch 4.x ignores, forcing ignored or refused without naming `tool_choice`, `json_schema` ignored, mid-stream failure only in `finish_reason`), P1 built (the `zhipu` platform, `glm-switch` / `glm-effort`, a per-platform model-id calibration table that prefills a hand-added model (G11), auto-only tool choice, the three finish reasons, caps, starters), P6 built (DashScope's hi-res / clip-fps knobs gated by platform, G12) and P2–P5 proposed (web search, the Coding Plan's routes, preserved thinking, a `json_schema` cap) |
| [capability-matrix.md](api/capability-matrix.md) | `living` (generated) | 能力矩阵：`src/lib/ai/capabilities.ts` 两张表的渲染结果（每个能力一张 平台 × 协议族 的表，带原因码与「按模型」标记），`capabilities.test.ts` 保证与代码一致；勿手改，重新生成的命令在该测试的头注里 |
| [capability-gating-plan.md](api/capability-gating-plan.md) | `partial` (C0–C3 built · C4 shelved) | 模型能力判定的系统化：今天「能不能 X」在抽屉、适配器、「将发送」摘要、会话面各算一遍（十种能力、十种签名，`family === "…"` 在画像之外 50 余处），由此而来的三类缺陷（层级放错——G12 与仍按族放行的视频；提问者之间不一致；「没测过」无法表达）。目标：一张能力登记表（`origin: native / private` 让私有字段缺省只在点名的平台出现）、平台画像一个 `capabilities` 槽、一个三值裁决函数加原因码（句子只在语言文件）、抽屉三态一条规则；三道闸（矩阵快照、对登记表遍历的一致性测试、源码扫描棘轮）；C0 已落成（`capabilities.ts`：平台 × 协议族 × 能力、第三轴模型 id，服务端工具也在表里；与旧判据 4480 余格逐格等价）；C1–C3 已落成（调用点直接问表、一致性测试、采样与输出进表、原因码进语言文件、可用性矩阵成为标准组件 `CapabilityMatrix`、源码棘轮），C4 视频按平台搁置；§7 记作者的三条决定，§8 记 C1–C3 |
...
| [orcarouter-probe-plan.md](api/orcarouter-probe-plan.md) | `shipped` | The 2026-09-26 paid probe of OrcaRouter's four surfaces (GPT-6 / GPT-5.6-terra, Claude Sonnet 5 · Opus 5.5 · Fable 5.1, Gemini 3.8 Flash): what was measured, which observations count as vendor behaviour (③④ bodies are Vertex / Anthropic verbatim; ① and ②'s default route are an OpenRouter-shaped layer; requests are re-serialized), the three fixes it forced, and (§8) tuning it as a built-in channel: upstream-reported cost trusted per platform, paid-model calibration and starters, PDF cells, Gemini's built-in tools; and (§9, 2026-09-27) the sweep of all six GPT-6 / GPT-5.6 ids on Chat and Responses — three backends behind one gateway, and the two effort cells it forced. Facts in landscape.md §7 第十八个样本 |
...
| [measurements/search-tools-ab-2026-09.md](feature/agent/measurements/search-tools-ab-2026-09.md) | `research` | `search_tools` A/B on spark-x2.5-4b, 2026-09-17：常驻 39/50 · 延迟第一版 30/50 · 第二版 33/50；不需要延迟工具的两格不掉分，需要的三格掉一到两次。附 qwen3.8-27b 方向性对照（每格 5 次）：常驻 7/15 · 延迟 9/15 |
...
| [chat-image-paste-plan.md](feature/agent/chat-image-paste-plan.md) | `shipped` `unverified` | 助手输入框直接贴图（⌘V，2026-09-23）：**贴图 = 会话暂存区里的一个真文件**（`.ai-writer/tmp/chat/<stashId>/`，内容哈希命名），于是整条 `@` 附图管线原样接住它；每条消息 5 张，与 `@` 共用一个数。暂存区的清理是**一条对账式清扫**而不是三个删除钩子（自动修剪是一句 SQL，没有回调），24 小时宽限防多开误删。上下文里的图按**轮**计租期（发送轮 + 之后 1 轮），只在新一轮开头淘汰；【附图】块带路径，所以退场后模型能按路径读回来（`read_image`，或委派看图子代理）。含「描述后丢弃」等四个否决方案的理由，以及待实测回填的 WebView 剪贴板行为（`VITE_PASTE_PROBE` 探针已在代码里）。落地时租期改挂在 `chatJob` 的新问题入历史处，§9 记了五处改动的理由；§10：只贴图不打字也能发（图本身就是问题），气泡显示作者附的全部图、取消排队连芯片一起还；回退也把提问的图还成芯片（编号不变） |
...
| [mention-scope-ui-brief.md](feature/agent/mention-scope-ui-brief.md) | `shipped` | `@` 引用选择器的作用域档位与排序（设计稿 `02i`，2026-09-26）：顶部一行 chip 全部 / 条目 / 文档 / 图片（候选里没有的类不画，「文档」有文件就在），语法照 ⌘K 的档位组，Tab 切档、只有 Enter 选中，每次打开回「全部」；空档不消失，一行 mono 事实报别处的数；排序复用 ⌘K 的 `matchText`（别名、分组路径与整条路径参与，一个词可跨 `/`），空查询按类交错。含六个被否方案（前缀 `@/`、恢复三个「+」、分组标题、记忆档位……）的理由 |
| [mention-fixes-retro.md](feature/agent/mention-fixes-retro.md) | `research` | `@` 引用三轮修复（#708、#709 两段）的复盘：为什么重复多轮、review 为什么总有新问题、修一轮为什么又冒一轮——根因是做法：需求变了（多实例、多方写入、异步读取）却一直在旧结构上打补丁，没在模块层面重构（草稿在 store、claim 在组件里的归属错位从未收拢）；review 总冒新问题、修法只冲反例、测试与修复同源都是它的表现。附错过的重构点、当时就有的信号、下次的做法与两个重构候选（未评估）。再碰 `MentionPicker` 的 claim / 落字之前读 |
...
| [mcp-tool-plan.md](feature/agent/mcp-tool-plan.md) | `proposal` | MCP 工具：应用作为 MCP 客户端（Rust `rmcp`，stdio + Streamable HTTP），每个服务是一个 `search_tools` 可搜组，工具按需展开成真实 wire 工具（`mcp__<服务>__<工具>`，schema 规整后上线）；调用默认过卡，免审只有作者按工具、按定义指纹给的信任清单与本次运行内连批；服务给的说明、注解、结果都按不可信文本处理，定义漂移即缺席。配置装机级、机密进 keyring；Beta + Tauri + 能渲染卡的 surface 缺一即缺席。实施前先过装载台架与四族 schema 探测 |
...
| [folder-note-plan.md](feature/lore/folder-note-plan.md) | `implemented` | 目录说明文件 `index.md`（2026-09-22）：任意目录（含知识库分类文件夹）可放一份，`status: draft/stable/deprecated` 是唯一机读键、首段是摘要；读侧三处（`list_files` 挂摘要且 `deprecated` 不展开只报数、`search_text` 跳过并报数、当前文件简报多一行），书脊不读它；写侧工作区不加工具、分类走 `manage_category` 的 `describe`；入口是文件树目录右键与知识库墙分类右键，提示词作为一条消息发出、写入仍过卡；墙筛到某分类时 chip 行下显示说明首段（`loreStore.categoryNotes`，`describe` 写盘后逐出） |
...
| [billing/01-fee-groups.md](feature/billing/01-fee-groups.md) | `shipped` | 计费组（价格从模型行搬成可复用的组，三种计价模式）与两本用量账（项目 `.ai-writer/project.db` + 总账 `appDataDir/config.db`）：九条不变量、迁移为什么用 `fee_migrated` 而不是清零旧列、写模型行为什么必须自己盖这个章（1.76.1 修的 REPLACE 清标记）、老的 `price_cached_in` 的 0 为什么要迁成 null、备份 v3、厂商与搜索（自由文本、段序按首现、搜索连模型一起搜、版本号为什么不涨）、上游报价（信任边界在平台上、多次请求全报才加）、以及设计稿 05l 上的几个界面判断 |
...
| [video-input.md](feature/video-input.md) | `shipped` | 对话里 `@` 一段视频给声明了「视频输入」的模型（仅 Chat Completions，千问实测）：不加工具、芯片上标 ≈token、模型行声明 fps；每条消息 1 段、历史里只留最新 1 段、读字节前先查 15MB 上限；实测的时长下限 / data URI 上限 / token 与 fps 表和估算公式 |
...
| [file-tree-folder-note-brief.md](feature/file-tree-folder-note-brief.md) | `shipped` | 目录说明 `index.md` 的界面侧（2026-09-22）：设计稿 `17 文件面板` TURN 2 屏 2a/2b/2c/2e/2f/2z。主干：**说明行与章节行只差一枚图标（`NotebookText`）和右列一个词（说明 / NOTE）**，作者那一档灰、行上不显示 status；分组菜单「造」格第四项**一槽两标签**（`NotebookPen`，新建目录说明并交给助手 / 让助手更新目录说明）；点击链 = 写模板 → 树 → 编辑器 → 抽屉对话页里一条普通用户消息 → 整篇重写批准卡；知识库墙分类头「让助手写分类说明」一词覆盖两义，另提案筛到分类时显示说明摘要（唯一要新读盘的一处）。含数据边界清单、模板与提示词中英两套、落点表 |
...
| [channel-model-route-plan.md](feature/channel-model-route-plan.md) | `shipped` | 「供应商与模型」重构为 **渠道 × 模型 × 线路**（界面词随之改为「渠道」）：渠道 = 一把 key + 一个平台（每个协议族一条线路），模型挂当前线路，思考 / 最大输出 / 结构化 / 温度按「模型×线路」各一份，服务端工具的授权挂模型、拼写由平台画像按「平台＋协议＋模型 id」决定。§11 P0（平台画像，修掉 `openai_compat` 一律发千问字段的缺陷）；§12 P1–P4（线路表与先选平台、模型切线路与差异卡、合并同一渠道 + 配置备份 v2、选择器线路签与同名提示），含与方案的出入：线路存成两个 JSON 列而不是两张表、旧列永远存当前线路、`providerFor` 是新接缝；§5.1.2 专用接口未做 |
...
| [downloadable-fonts-plan.md](feature/downloadable-fonts-plan.md) | `shipped` `unverified`（Windows / Linux） | 可下载字体方案 **鸿蒙黑体 / MiSans**（设计稿 `05n`）：字体栈在构建期写进 `tokens.css`，只有字节是运行期的，没下载时自然落到黑体栈；字节经窄协议 `ai-writer-font:` 按 `unicode-range` 懒加载（先按字面判定路径再碰磁盘、ACAO `*`、异步）；开发期锁定表钉住 420 个文件的大小与 sha256，源只送字节（鸿蒙在 npmmirror 上 403）；`.part` 改名、`faces.css` 存相对名、完成标记最后写；「选了」（偏好）与「有了」（磁盘）分开，焦点同步只看不下载；导出 .html 不带字节 |
...
| [code-structure-plan.md](feature/code-structure-plan.md) | `shipped` | 源码结构整改（2026-09-19 审查）：4 组循环依赖（24 个文件）→ 0、`lib → stores` 7 个文件 → 0、store 间 61 处动态导入 → 17（理由写在守卫里）、三个 3000+ 行的文件拆到 1500 行以下——P0–P7，§3 前后对照，§7 记偏离，`layering.test.ts` 守着。**§9 是整改前 → 整改后的位置对照表**：写在整改之前的设计文档不重写（重写会把设计记录改成假的），只在开头挂一行「位置注记」指回这张表 |
