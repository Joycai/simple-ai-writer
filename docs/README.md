...
| [docx-export-plan.md](feature/docx-export-plan.md) | `proposal` | 导出 Word（.docx）与排版预设：为什么「难的是读 docx，不是写 docx」，RTF / HTML-塞进-.doc / pandoc sidecar / Rust `docx-rs` 四条弃用理由，`DocFormat` 为什么不从 markdown 主题的 CSS 推导，以及严格格式规格（公文级）的实测表达力与「校对规格表而不是校对产出」（§7） |
