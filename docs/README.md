...
| [import-images-plan.md](feature/import-images-plan.md) | `planned` | 导入 PDF/docx 时抽取内嵌图片：`ConvertResult` 接缝、pdfjs opList 抽取 + y 坐标定位、落 `assets/<文档名>/`；去重/装饰过滤/扫描件三个决策，矢量图明确不做 |
