...
| [chat-image-paste-plan.md](feature/agent/chat-image-paste-plan.md) | `planned` | 助手输入框直接贴图（⌘V）：**贴图 = 会话暂存区里的一个真文件**（`.ai-writer/tmp/chat/<stashId>/`，内容哈希命名），于是整条 `@` 附图管线原样接住它；每条消息 5 张，与 `@` 共用一个数。暂存区的清理是**一条对账式清扫**而不是三个删除钩子（自动修剪是一句 SQL，没有回调），24 小时宽限防多开误删。上下文里的图按**轮**计租期（发送轮 + 之后 1 轮），只在新一轮开头淘汰；【附图】块带路径，所以退场后模型能 `read_image` 找回来。含「描述后丢弃」等四个否决方案的理由，以及动手前要实测的 WebView 剪贴板行为 |
