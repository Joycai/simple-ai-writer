...
| [shell-command-plan.md](feature/agent/shell-command-plan.md) | `shipped` (#561 · #563；2026-09-15 审批策略升级；可选流式未做) | `run_command`：Windows 走 PowerShell，macOS / Linux 走系统 shell。POSIX / PowerShell 封闭只读白名单免审批，未知、复合和带写入/执行参数的命令默认审批；普通写命令可按 1–5 条连批并绑定当前 run，危险命令永远逐条看。stdin 关死、杀整个进程组、输出头尾截断 + 日志落 `tmp/cmd/`、`cwd` 双重围栏；Beta + Tauri + 能渲染卡的 surface 三者缺一即缺席。 |
