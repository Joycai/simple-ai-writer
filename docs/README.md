...
| [compact-threshold-plan.md](feature/agent/compact-threshold-plan.md) | `proposal` | 归纳阈值：「上下文与记忆」页加「自动归纳」开关 + 两条滑块（8k–512k 对数刻度 · 50–80% 窗口比例，先到者生效）。给 Claude Design 的任务书（应用的第一个滑块，请求新开 `19 归纳阈值`）+ 三片 PR 的计划。要点：第三条线（0.7 × 消息上限）保留且默认赢，比例滑块在默认窗口占用 50% 下整段不生效，必须靠读数说清；折叠目标随触发线同比缩放；扮演页今天**没有**手动归纳按钮，要补 |
