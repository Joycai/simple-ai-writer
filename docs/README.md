...
| [mention-scope-ui-brief.md](feature/agent/mention-scope-ui-brief.md) | `shipped` | `@` 引用选择器的作用域档位与排序（设计稿 `02i`，2026-09-26）：顶部一行 chip 全部 / 条目 / 文档（有图才有「图片」），语法照 ⌘K 的档位组，Tab 切档、只有 Enter 选中，每次打开回「全部」；空档不消失，一行 mono 事实报别处的数；排序复用 ⌘K 的 `matchText`（别名与分组路径参与，逐词跨字段），空查询按类交错。含六个被否方案（前缀 `@/`、恢复三个「+」、分组标题、记忆档位……）的理由 |
