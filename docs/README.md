...
| [docx-export-plan.md](feature/docx-export-plan.md) | `proposal` | 导出 Word（.docx）与排版预设：为什么「难的是读 docx，不是写 docx」，RTF / HTML-塞进-.doc / pandoc sidecar / Rust `docx-rs` 四条弃用理由，以及 `DocFormat` 为什么不从 markdown 主题的 CSS 推导 |
