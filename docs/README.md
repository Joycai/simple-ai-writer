...
| [billing/01-fee-groups.md](feature/billing/01-fee-groups.md) | `shipped` | 计费组（价格从模型行搬成可复用的组，三种计价模式）与两本用量账（项目 `.ai-writer/project.db` + 总账 `appDataDir/config.db`）：九条不变量、迁移为什么用 `fee_migrated` 而不是清零旧列、老的 `price_cached_in` 的 0 为什么要迁成 null、备份 v3、厂商与搜索（自由文本、段序按首现、搜索连模型一起搜、版本号为什么不涨）、以及设计稿 05l 上的几个界面判断 |
