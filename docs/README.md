...
| [capability-gating-plan.md](api/capability-gating-plan.md) | `proposal` | 模型能力判定的系统化：今天「能不能 X」在抽屉、适配器、「将发送」摘要、会话面各算一遍（十种能力、十种签名，`family === "…"` 在画像之外 50 余处），由此而来的三类缺陷（层级放错——G12 与仍按族放行的视频；提问者之间不一致；「没测过」无法表达）。目标：一张能力登记表（`origin: native / private` 让私有字段缺省只在点名的平台出现）、平台画像一个 `capabilities` 槽、一个三值裁决函数加原因码（句子只在语言文件）、抽屉三态一条规则；三道闸（矩阵快照、对登记表遍历的一致性测试、源码扫描棘轮）；C0–C3 纯重构，C4 视频按平台需实测 |
