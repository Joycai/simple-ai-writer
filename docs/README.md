...
| [lore-category-manage-plan.md](feature/lore/lore-category-manage-plan.md) | `partial` | 分类的管理面：墙上多选批量改分类（已实施，含置顶重指）；「删除分类」为什么今天不删任何东西、该怎么补；agent 那侧的 target 轴缺口。为什么分类和集合的管理面天生不对称 |
