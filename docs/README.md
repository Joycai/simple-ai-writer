...
| [mention-fixes-retro.md](feature/agent/mention-fixes-retro.md) | `research` | `@` 引用三轮修复（#708、#709 两段）的复盘：为什么重复多轮、review 为什么总有新问题、修一轮为什么又冒一轮——根子是 claim 用偏移量 + 单段 `editRange` 启发式、写入方没先列全、修法只冲反例、测试与修复同源；附下次的做法与两个换形状的候选（未评估）。再碰 `MentionPicker` 的 claim / 落字之前读 |
