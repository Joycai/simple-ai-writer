...
| [volcengine-files-api.md](issues/volcengine-files-api.md) | `open` | 火山方舟 PDF 只走 base64 内联（<50MB），厂商推荐的 Files API / `file_id` 与 ② 的 `file_url` 未支持；套餐前缀没有 `/files`（404），只对按量 key 有意义——而按量线路整条都未实测。前提是一把按量 key |
