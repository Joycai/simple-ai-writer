...
| [approval-card-ui-brief.md](feature/agent/approval-card-ui-brief.md) | `shipped` `unverified` | 设计稿 02h 已按切片 A–F 落地（改动窗、重写 / 删除 / 插入卡、方案账本与破坏性步骤暂停、撤回、自动批准后的「本轮写入」、窄栏）；「最后改于」因文件层没有 mtime 没做。原任务书：编辑类的卡今天说得出「少了 812 字」却说不出少的是哪 812 字，知识库那一侧连一个字的将写入内容都不在卡上（L1 调用即落盘，方案卡只有模型自己写的一句承诺）。含发稿前查到的底稿（数据都已在手、待批准提案不落盘、仓库里没有 diff 算法）与要稿子回答的五个问题 |
