...
| 历史（`trimHistory` 的 `elideImagesOverBudget`） | 从最老的带图消息开始去掉图片（保留文字），直到放得下；**最新一条带图消息不动** | 最新那条是模型马上要看的；它自己就超了，说明是构造时的问题，交给下一行报出来，不在这里悄悄吞掉 |
| 发送前（`streamCompletion`） | 抛 `ImagePayloadError`，不发送 | 兜住上面三条之外的路径（知识库的生成 / 改进元信息 / 标定等），和 `ContextSizeError` 同一个位置、同一种处理 |
