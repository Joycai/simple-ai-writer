...
| 端点长边 `MAX_IMAGE_EDGE` | 8192 px | 否 | 端点的硬拒绝，不是偏好（2026-09-17 补，见下） |
| 一次请求的图片合计 `MAX_REQUEST_IMAGE_CHARS` | 24 MiB | 否 | 见 §2.9 |

`0` = 不主动缩长边，但仍守 `MAX_IMAGE_EDGE`。上界钳在 8192，照
[`DEFAULT_MAX_OUTPUT_MAX`](../../src/lib/ai/modelLimits.ts) 的做法：自由数字
输入框，但不接受胡来的数字。

**上界为什么从 16384 降到 8192，`0` 为什么不再是「原样发」**（2026-09-17）。
DeepSeek 文档写明单边超过 8192px 的图直接 400，同日实测坐实：8192 收、8193 拒，
而且报错说的是"格式不支持"，作者照着改格式也没用
（[`landscape.md` §2.1](../api/landscape.md)）。原来的 16384 上界和「0 = 不限」
都会把一张长截图原样送过去，作者拿到的是一个他没法处理的端点报错。这和 §2.8 的
10px 下限是同一类事：**端点的硬拒绝不归作者的偏好管**。所以 `imageLimits()` 在作者
选 `0` 时用 8192 兜底，设置项上界也钳到 8192；早先存下的 16384 读出来就是 8192，
不用迁移。代价只落在长边超过 8192 的图上，而 8192 以上的细节，各家端点进模型前
本来就会缩掉（DeepSeek 缩到约 1300² 总像素）。仍然不按 provider 分档，理由同上。
没有管到的：动图（§2.5）照旧原样发，一张长边超过 8192 的动图仍会被拒——这种图
现实里罕见，不值得为它把动图拆帧。
...
### 2.8 下限：短边不足 10px 就放大（2026-09-14 补）

阶梯原本只管上限。实测百炼 qwen3-vl-plus 对一张 9×9 PNG 直接 400
（`height:9 or width:9 must be larger than 10`），10×10 与 200×10 都收
（[`landscape.md` 第六个样本](../api/landscape.md)）。图标、色块、裁出来的单个字
都是作者真会发的图，「端点拒收」不是他们能处理的报错，所以 `MIN_IMAGE_EDGE = 10`
进了 `downscalePlan.ts`，和上限走同一个循环：

- **短边 < 10 → 等比放大到短边恰好 10**，长边向上取整（10/3×3 在浮点里不一定等于
  10，所以短边直接赋值，不走倍率）。放大不丢任何像素，所以**不写
  `downscaled` 说明**——两处调用方的措辞都是「已缩小」，写了就是错话。
- **下限压过长边上限。** 一张 2×1000 的图在 256 的上限下两头保不全：要么长边超，
  要么把宽高比压扁。上限是作者对 token 的偏好，下限是端点的硬拒绝，而压扁的图会让
  模型认真描述一张变形的画面——所以长边按宽高比超出上限（这里是 10×5000），
  `fitsLimits` 也把它算作达标，否则阶梯下一轮会把它缩回 10 以下。
- **缩小不穿过下限。** 超长边时的目标长边、超字节时的 ×0.75，都截在短边 10；
  已经贴着下限还超字节的 PNG 直接 give-up。
- **不变的：** 动图照旧原样；文件头读不出尺寸、字节又不超的格式照旧不解码（不知道
  尺寸，也就不测下限）；解码或编码失败照旧原样发出，让端点的报错说话。
- `normalize.ts` 的候选比较改为直接调 `fitsLimits`，不再手写一份——手写的那份还按
  旧上限判断，放大后的候选会输给原图。

不按 provider 分档，理由同 §2.2：中转背后是谁探测不出来，而 10px 对其他端点毫无代价。
...
（`MAX_IMAGE_RESULTS`，按**消息**数，一条消息可以带 4 张，现在是 5 张）——合起来没有任何上限。
四张 12 MB 的 PNG 截图 base64 后约 64 MB，超过 Anthropic 的 32 MB 和 DeepSeek 的
48 MiB 请求体上限。失败方式和 §2.1 当初要修的一样：上传几分钟，然后 413 或者
干脆没有响应。

**数值：24 MiB，按 data URL 字符数计**（`lib/ai/imagePart.ts`
`MAX_REQUEST_IMAGE_CHARS`）。按字符数计是因为请求体里实际占地方的就是 base64
字符串。取 24 是因为最紧的已知上限是 Anthropic 的 32 MB，要给正文、工具 schema、
JSON 外壳留出余量；一张封顶的单图（12 MiB，编码后 16 MiB）一定放得下，所以这道
上限只决定几张能一起走，从不拒掉单张图。同样不按 provider 分档（§2.2）。

**五个落点，按「谁造这次请求」分：**

| 路径 | 超了怎么办 | 为什么这样处理 |
| --- | --- | --- |
| 对话 `@`（`chatRefs.buildChatMessage`） | 按顺序发得下的先发，其余按路径列给模型（`ai.chat.imagesOverCap`） | 和超张数同一种处理；模型拿着路径还能用 `read_image` / 看图子代理去读。顺手修了一个旧错：模型能看图、只是超了张数时，原来告诉它的是「当前模型读不了图」 |
| 看图子代理（`subagent.ts`） | 整个委派失败，要求拆开 | 和超张数一样拒绝。任务点名了这些图，少带几张它也会照样作答，而且不会说少看了 |
| 历史（`trimHistory` 的 `elideImagesOverBudget`） | 从最老的带图消息开始去掉图片（保留文字），直到放得下；**进行中的一轮和最新一条带图消息都不动**，放不下就照发 | 这两处都是模型马上要看的图。第一版只保护最新一条，结果同一轮并行读两张大图时，第一张还没被看到就被去掉，模型重读它又挤掉第二张，一直循环到轮数上限——这正是 `trimHistory` 的 M1 规则（进行中的一轮不裁，window-edge-plan.md）要防的。放不下交给最后一行报错，不在这里悄悄吞掉。**被拒之后，这些图要从历史里拿掉**（`elideUnsendableImages`，文字留下）：聊天的下一次提问接在同一份历史后面，而助手的最后一条消息仍是那次读图请求，裁剪还会把这一轮当成进行中继续保护，于是这个对话之后的每次请求都会被拒；报错里建议的「少带几张 / 调小长边」对已经在历史里的图也没用 |
| 改进元信息（`LoreMetaImproveModal`） | 头像和图库按顺序发放得下的，其余在文字清单里标注「未附上」 | 整个图库自动带上，作者在这个弹窗里选不了带哪几张；如果只靠最后一行兜底，图库大的条目每次都会失败 |
| 发送前（`streamCompletion`） | 抛 `ImagePayloadError`，不发送 | 兜住上面几条之外的路径（知识库条目生成、AI 任务附图——图是作者自己选的，报错里的「少带几张」对他们有用），以及历史裁剪放不下时照发的请求；和 `ContextSizeError` 同一个位置、同一种处理 |

视频不计入这道上限：视频只走 DashScope，每条消息和历史里各只留 1 段，另有自己的
单段上限（`docs/feature/video-input.md`）。
...
三条「只有真机能答」的，**2026-08-25 在 `pnpm tauri dev` 里验过了，全部成立**：

- [x] **EXIF 方向**（§2.4 唯一没有退路的假设）。一张 iPhone 15 Pro 的
      5712×4284、`Orientation = 6` 照片：解码后是 **4284×5712**（转置过了），
      出来 **3072×4096** 竖图。像素比对给的是同一个答案 —— 与「EXIF 已应用」的
      参考图平均差 2.38（JPEG 量化噪声的量级），与「EXIF 被忽略」的参考图平均差
      **64.79**。对照组同一张照片把 tag 改回 `1`，出来 4096×3072 横图。
      所以 `<img>` + `decode()` 两件事都对：像素转正了，`naturalWidth/Height`
      报的也是转正后的尺寸。§2.6 的提示里那对数字因此是**作者看到的**尺寸
      （`4284×5712 → 3072×4096`）而不是文件头里的存储尺寸 —— 这是对的，作者
      从没见过后者。
- [x] **alpha 存活**。两条路都试了，因为触发它们的阈值不是同一个：
      5000×3000 带透明的 PNG（**长边**超）→ 4096×2458 PNG，全透明像素占比
      33.3% → 33.2%；4000×4000 带透明边框的 PNG 52.6MB（**字节**超、长边不超，
      走 §2.3 阶梯的 ×0.75 那条）→ 2250×2250 PNG 9.5MB，36.0% → 35.6%。
      两张的 alpha 最小值都仍是 0，没有一张被编成 JPEG。
- [x] **动图原样**。5000×900 的 5 帧 GIF（长边远超 4096）：出来 **sha256
      与原文件相同**、5 帧都在、`downscaled` 为空、耗时 2ms —— 它根本没进
      canvas，`readImageHeader` 认出 `animated` 就在 `planImageStep` 第一问上
      返回了 `as-is`。

§2.6 那两句提示也一并核了，是设计里写的那两句：聊天附件 chip 的 tooltip 渲染成
`已缩小以适应发送上限（4284×5712 → 3072×4096, 4.3MB → 2.2MB）`（后面接
`· 移除引用`），`read_image` 的工具结果带
`Downscaled to fit the size limit (4284×5712 → 3072×4096, 4.3MB → 2.2MB).`

### 7.1 怎么验的（下次照这个来）

**GUI 自动化点不动这个应用**（点击会被 Dock 抢走 hit-test），所以没有靠「打开
项目、拖张图进聊天框」。改成一个临时的启动期探针：`main.tsx` 的 `boot()` 里加
一段 `import.meta.env.VITE_IMG_VERIFY` 守卫的动态 import，模块拿到路径后
`invoke("project_register_root")` 拿 fs scope（目录里放个空 `.ai-writer` 就够，
见 `scope.rs`），对每张图调**真的** `imageForModel`，把返回的字节
`writeBinaryFile` 落盘、连同 header / `downscaled` / 渲染出来的 i18n 文案写一份
`report.json`。然后在外面用 PIL 对着落盘的结果核。跑完把探针和 `main.tsx` 的
改动删掉 —— 它不进仓库。

这条路的价值不在省事，在于**它验的是 WKWebView**：`report.json` 里记下的 UA 是
`AppleWebKit/605.1.15`，而不是浏览器里开 Vite dev server 那条路（那是 Chromium）。§2.4 赌的正是这个
引擎的默认行为。

一处取巧要记下来：机器上找不到长边超 4096 且 `Orientation ≠ 1` 的照片（相册导出
的那批 iPhone 照片方向都已经被规范化成 1 了）。所以拿一张真的 iPhone 15 Pro 原
片，**只改 EXIF IFD0 里 Orientation 那一个字节**（1 → 6），文件其余 4515563 个
字节逐字节相同、熵编码数据一点没动。手机为竖拍写下的就是这一个字节，所以这与
「原片」在这条验证上没有区别。

### 7.2 顺手观测到的两件事

- **缩像素可能让字节变大。** 那张 5000×3000 的渐变 PNG 76KB 进、288KB 出 ——
  PIL 的 PNG 编码器对渐变用足了 filter，WebKit 的 canvas 编码器没有。这不是
  bug，长边才是作者设的那个上限；而且 `shrink` 的择优规则（达标优先于更小）
  已经把这件事的坏情况兜住了：一个既不达标又更大的候选不会取代原图，最坏是原样
  发出去。
- **一张 24MP 照片在发送路径上要 3~4 秒**（exif6 3.8s / exif1 3.1s，WKWebView，
  debug 构建 —— 但这几秒是 webview 侧的解码+编码，release 不会差多少）。§5 第 5
  条「不上 worker」的理由仍然成立（只有超标的图付这笔钱），但这个数比上表那轮
  （Vite dev server）的 1.4s 高一倍多，真要有人抱怨卡，先看的是这里。
