...
| 历史（`trimHistory` 的 `elideImagesOverBudget`） | 从最老的带图消息开始去掉图片（保留文字），直到放得下；**进行中的一轮和最新一条带图消息都不动**，放不下就照发 | 这两处都是模型马上要看的图。第一版只保护最新一条，结果同一轮并行读两张大图时，第一张还没被看到就被去掉，模型重读它又挤掉第二张，一直循环到轮数上限——这正是 `trimHistory` 的 M1 规则（进行中的一轮不裁，window-edge-plan.md）要防的。放不下交给最后一行报错，不在这里悄悄吞掉 |
