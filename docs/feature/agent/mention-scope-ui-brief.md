...
**验证**：`lib/search/__tests__/mentionSearch.test.ts` 钉住别名命中、路径命中且排在名字命中后、一个词跨 `/` 走整条路径档且高亮拆回两行、`分组/标题中间一个词` 按最后一个 `/` 拆开也能命中而左半不认子序列、分组与路径不认子序列（`小李` 不落 `小镇/李家.md`）、库层多词跨字段（⌘K 一致性）、取最优字段、空查询交错、作用域硬过滤、media 归文档、计数、limit、`cycleScope` 环绕与空档跳过空档。选择器在 vite 预览里用假候选独立渲染核过四态：全部档交错列表（含「已常驻」与已附带的暗行）、文档档 + 命中高亮（名字与副行）、条目档空档的事实行、纸面浅色（第一版还核过「跨字段查询 `潮汐门篇 归途`」，那是绕过 `findMention` 直接喂 query 的结果，宿主打不出来——见决定 8）。状态迁移与键盘协议抽成纯函数之后有了单测（`lib/agent/__tests__/chatMentions.test.ts` → `syncMention`：新开回全部与第 0 行、继续保留档位、改 query 回第 0 行、关掉再重开也回第 0 行、开着时另起一个 `@` 算新开；`spliceMention`：原位落字、`@query` 已不在原位或后面紧接 query 字符则不动、刚落下的 `@[A]` 不再被当成空 query 的 `@`；`nextLive`：关掉保留最后一次打开的位置；`mentionKeyDown`：不在屏上不接管、Esc 组字期间也关、组字期间其余键放过、Tab / Shift+Tab 切档、空列表上 Tab 带计数跳过空档、Enter 选中高亮行、越界退回第 0 行、Shift+Enter 放过、空档 Enter 只在别处有命中时吞掉、没算计数时放行；`EmptyLine` 与 `Highlighted` 用 `renderToString` 核输出（三种情形、哨兵拆分、只点在册的档、英文复数）；`useMentionSearch` 同法：零候选不开、关着不算、开着给可用档与计数）。宿主里从 keydown 到这两个函数的那几行，以及真实输入法下的时序，仍只有代码路径与手动核——预览里对话助手要有模型、扮演与弹窗要有项目。
