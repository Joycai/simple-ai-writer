...
- **选择器** `components/common/MentionPicker.tsx`：`useMentionState` 增 `scope` / `setScope` / `cycleScope`；状态收成一个对象，`sync` 是纯函数 `syncMention(prev, value, caret)` 在函数式 `setState` 里跑——从提交过的状态出发分辨「新开还是继续」：新开回「全部」**与第 0 行**（第一版只重置档位不重置高亮，Tab 到条目、↓↓、打「，」再退格，高亮会落在全部档另一项上），继续保留档位、改 query 回第 0 行；`close` 用 `useCallback` 稳住，点外关闭的监听不再随对话助手每次流式刷新重挂。空档那一行是独立的 `EmptyLine` 组件（测试不用绕 portal）；英文下 chip 是首字母大写的复数（Entries / Documents），句子里是小写复数——`appTerms` 的单数「entry」放进「3 in entry」读不通；从空档按 Tab 跳过同样空的档，「Tab 切过去」才是一下。chip 行固定在顶上、行列表在它下面单独滚动（第一版把 chip 行 sticky 在滚动区里，`scrollIntoView({block:"nearest"})` 不认 sticky 头，↑↓ 绕回第 0 行时它藏在 chip 下面）；空档渲染 chip 行 + 一行事实；名字与副行按区间高亮，用的是和 ⌘K 同一个 `common/Highlighted`（`--color-mention-bg` 底 + `--color-sienna` 字；第一版各抄一份，两份的色 token 已经不一样了，review 时合并）。chip 走 `onMouseDown` + `preventDefault`，和行项一样不让 textarea 失焦。同文件还出两样给宿主共用：`useMentionSearch(candidates, mention, projectPath)` 跑作用域 / 打分 / 计数（选择器不在屏上时不算——候选每次条目写入、文件树刷新都在变，为没人看的列表重打分是纯浪费），`mentionKeyDown(e, mention, search, composing, onPick)` 是键盘协议的**唯一**一份，返回「这一键是不是选择器的」。
...
**验证**：`lib/search/__tests__/mentionSearch.test.ts` 钉住别名命中、路径命中且排在名字命中后、一个词跨 `/` 走整条路径档且高亮拆回两行、分组与路径不认子序列（`小李` 不落 `小镇/李家.md`）、库层多词跨字段（⌘K 一致性）、取最优字段、空查询交错、作用域硬过滤、media 归文档、计数、limit、`cycleScope` 环绕与空档跳过空档。选择器在 vite 预览里用假候选独立渲染核过四态：全部档交错列表（含「已常驻」与已附带的暗行）、文档档 + 命中高亮（名字与副行）、条目档空档的事实行、纸面浅色（第一版还核过「跨字段查询 `潮汐门篇 归途`」，那是绕过 `findMention` 直接喂 query 的结果，宿主打不出来——见决定 8）。状态迁移与键盘协议抽成纯函数之后有了单测（`lib/agent/__tests__/chatMentions.test.ts` → `syncMention`：新开回全部与第 0 行、继续保留档位、改 query 回第 0 行、关掉再重开也回第 0 行；`mentionKeyDown`：不在屏上不接管、Esc 组字期间也关、组字期间其余键放过、Tab / Shift+Tab 切档、空列表上 Tab 带计数跳过空档、Enter 选中高亮行、越界退回第 0 行、Shift+Enter 放过、空档 Enter 只在别处有命中时吞掉、没算计数时放行；`EmptyLine` 与 `Highlighted` 用 `renderToString` 核输出（三种情形、哨兵拆分、只点在册的档、英文复数）；`useMentionSearch` 同法：零候选不开、关着不算、开着给可用档与计数）。宿主里从 keydown 到这两个函数的那几行，以及真实输入法下的时序，仍只有代码路径与手动核——预览里对话助手要有模型、扮演与弹窗要有项目。
