...
台架：`scripts/prompt-ab.ts`（多轮、喂假工具结果、判据成立即收手），
gemma4:12b-mlx via ollama，**每格 3 次**（不是原计划的 4 次——按实测每轮 ~65s
折算，4 次要多花近一半时间，而三格的分歧程度不需要那第 4 个样本）。

**结果**：

| 判据 | 现行 briefing（3,763 字符 system） | 精简版（2,346 字符） |
| --- | --- | --- |
| plan-first：动{{kb}}前必须提 `propose_lore_plan` | **3/3** | **3/3** |
| search-before-read：用 `search_text` 定位而不是逐个 `read_file` | **3/3** | **3/3** |
| segmented-write：`create_file` 骨架 + `append_file` 分段 | **0/3** | **0/3** |

前两格两个变体的调用序列几乎逐字相同
（`list_lore_entities → read_lore_entity → propose_lore_plan`）。

**第三格记为「判据失效」，不是「不达标」**，这个区分是要紧的：

- 两个变体都是 0/3，失败形态一样——要么在目录里原地打转
  （`list_files → list_files → list_files`），要么用 184s 一次性灌完一个
  `create_file`，**正是那条规则警告的事**。完整版 briefing 并没有买到任何东西。
- 而这一格管辖的「大文件必须分段写」整节，在精简版里是**逐字保留**的。所以
  它不是这次改动引入的风险。
- 4.2 的验收线原话是"任何一格不达标就退回保守版"。那条线的用意是**检测精简
  是否造成退化**；基线就是 0 的格子检测不了退化，它只说明这个模型做不了这件事。
  照字面执行就是让一个失效的测量否决一个测量支持的改动。

**因此按全量精简落地**，并如实记下三条局限：n=3；只在一个 12B 本地模型上测过；
分段写这条纪律仍然**未经测量验证**（只是按构造未被触碰）。

> **第三条暂缓（作者 2026-08-21）**：实际使用中没有暴露成问题，所以不专门去验。
> 这不是"忘了"，是排序——真要验也不必动台架：用平时的模型在对话助手里要一个足够大的
> 交付物，展开执行日志看是 `create_file` → `append_file × N`（生效）还是一个巨大的
> `create_file`（没生效）；日志里那行红色的「输出被上限截断，回答未写完」就是这条规则
> 存在的理由，出现即说明这段 briefing 对该模型无效，届时该考虑的是把长度上限写进
...
> 非流式请求下 Node fetch 的 300s headers 超时会把一次 357 秒的生成变成一句
> 毫无信息量的 `TypeError: fetch failed`；而改成流式**仍然不够**——ollama 要把
> 整个提示 prefill 完才发第一个 SSE 字节，这套 28KB 的工具载荷实测 **84 秒**才
> 到首字节，忙的时候超过 300s。最终走 `node:http`（没有那个默认超时）。
> 讽刺的是，让这个台架慢下来的正是工具载荷的体积——也就是这整套改动要治的东西。
...
`AgentChat.tsx` 的上下文条改用**常驻集**估算，并在 tooltip 里说明"批准知识库方案后会增加"。
...
## 6. PR5b —— `load_tools` 元工具（**决定不做**）

前置条件其实满足了：5a 已上线、§5.5 的测试稳定，PR4 实测两格 3/3=3/3。
但 PR4 顺带产出的证据指向相反方向，所以不做。

先把净收益算清楚：`file_ops`（4,979 字符）+ `whole_file`（1,478）合计 6,457 字符
≈ 1,600 token。减去它自己的常驻成本——`load_tools` 的 schema 加一份分组目录
（~400 字符 ≈ 100 token），加 briefing 里那句「需要建/移/删文件时先
`load_tools(["file_ops"])`」（中文 ~60 字 ≈ 60 token/轮）——净剩 ~1,440，
**且只在用不到这些工具的那些轮才省得到**。

代价那一侧，§4.3 的 segmented-write 一格是直接证据：gemma4:12b 面对
「写一个介绍页」，在**工具就摆在眼前**的情况下 3 次里 0 次走通
`create_file` → `append_file`——要么在目录里原地打转，要么一次性灌完。在这样一个
模型面前再加一层"你得先开口要工具"的间接，只会让它更做不成：省下的 1,440 token
换来的是一类任务从"做得差"变成"做不了"。

这不是"以后再说"，是**这条路对本项目支持的模型档位不成立**。要重开这个话题，需要
的新证据是：一个能稳定完成 §4.3 第三格的模型，在同样的间接下仍然稳定。在那之前，
工具集继续变大时的正确答案是把更多组挂到 5a 那种**由运行状态自动装载**的机制上
（不需要模型配合），而不是让模型自己去要。

<details>
<summary>原设计概要（留档）</summary>
...
### 6.1 重开：`search_tools`（2026-09-17，作者拍板）

**为什么重开。** 上面那条结论针对的是「让模型先开口要工具」这个间接本身。一年里常驻半边又长到了
10,828 token，其中两组的形状恰好是这条路要的：**没有门、只是很少用**——`file_ops`
（改名/移动、复制、删除文件与目录、新建章节与目录，1,404）和 `image`（三个画图工具，1,940）。
它们挂不到 5a 那种「由运行状态自动装载」的机制上：没有任何运行状态说得出「这一轮要改名了」。
作者看过三种装法（作者在输入框勾选 / 模型自己搜 / 按作者这句话的关键词判断）之后选了模型自己搜，
理由是行业里这条路已经走通（Anthropic 的 tool search、Claude Code 的延迟工具），而作者勾选
把一件模型该判断的事推给了人。

**和 5b 原设计的不同**，每一处都是为了让「没要到」不至于变成死路：

| | 5b 原设计 | 落地的 `search_tools` |
| --- | --- | --- |
| 入参 | `groups: ToolGroup[]`，必须写对组名 | `query` 一个字符串：组名、工具名、中英关键词都算命中（`toolSearch.matchGroups`） |
| 模型不搜、直接点名一个未装载工具 | `Unknown tool` | 当场装上，报「这次没执行，下一步再调」（`autoLoadMessage`） |
| 多轮对话 | 每次运行从头要 | 这段对话用过的组，下一次运行开跑就装上（`groupsUsedIn`） |
| 查的是知识库写入 | —— | 改口指向 `propose_lore_plan`（那两组由方案装，不归这里） |
| 以此为本职的预设 | —— | `TaskPreset.residentGroups` 照旧常驻：文件 pack、旁白（`create_chapter` 是它保存一场戏的方式） |

目录只列这次运行**非空**的组（没绑图像模型时 `image` 被 routing 摘空，目录里就没有画图），
而且装载之后不变——`search_tools` 排在常驻的最后一个，它的 description 一变，Anthropic 的缓存
前缀就断在常驻半边的末尾。`read_image` / `read_lore_image` **不进** `image` 组：作者 @ 的图、
条目配图的查看都经由它们（`chatRefs` / `imageHistory` / `loreSelect`），延迟它们省 365 token，
换来的是「看一眼这张图」也要先搜。

**收益**：助手常驻 10,828 → **7,726**（−3,102，`search_tools` 自身 241），棘轮上限
12,000 → 9,000。编排层常驻 4,789 → 3,264（画图三件套在那里也延迟了）。

**实测**（[`measurements/search-tools-ab-2026-09.md`](./measurements/search-tools-ab-2026-09.md)，
spark-x2.5-4b，每格 10 次、至多 6 轮）：

| 判据 | 常驻 | 延迟·第一版 | 延迟·第二版（落地） |
| --- | --- | --- | --- |
| plan-first | 7/10 | 5/10 | 6/10 |
| search-before-read | 10/10 | 10/10 | 10/10 |
| rename-file（`move_chapter`） | 6/10 | 2/10 | 4/10 |
| delete-folder（`delete_directory`） | 6/10 | 6/10 | 4/10 |
| draw（`generate_image`） | 10/10 | 7/10 | 9/10 |
| 合计 | 39/50 | 30/50 | 33/50 |

读法：

- **不需要延迟工具的格子不掉分。** search-before-read 三组都是 10/10；plan-first 第二版的四次失败
  全是「读完条目就停」，常驻组里同样的失败有三次——这是这个模型的底子，不是这次改动。
- **第一版 → 第二版的两处修正都是看了调用序列才加的。** plan-first 第一版有四次去
  `search_tools("update lore meta alias")`，拿到「没有匹配、用你手上的」就收手；改口之后三次
  搜了的全部接着提了方案。rename-file 的失败形状是「用 `propose_edit` 改标题行代替改名」——
  常驻组里也有四次，说明任务本身有歧义；briefing 里点名「改名不是改正文」之后 2→4。
- **需要延迟工具的三格仍然各掉一到两次**，合计 −6/30。这是这条间接在 4B 模型上的真实代价，
  没有被修正抹平；失败形状是「一直在读、没想到去搜」，不是「搜了没用上」——第二版这三格
  搜了的 19 次里 17 次随后用上了工具（一次撞到 6 轮上限，一次搜完就收手）。
- 「直接点名未装载工具」这条兜底在 150 次里**一次都没触发**：这个模型要么搜，要么根本不想这件事。
  它留着是为别的工具的结果文本（`update_lore_image` 的「画新图用 generate_image」之类）点名的时候。

**更大的模型上差距收窄**（同一份记录的第二节，qwen3.8-27b-uncensored，每格只跑 5 次、
至多 8 轮，只求方向）：
...
| plan-first | 4/5 | 5/5 |
| rename-file | 2/5 | **4/5** |
| delete-folder | 1/5 | 0/5 |
| draw | 0/5 | （作者叫停：用得少） |
| 合计（不含 draw） | 7/15 | 9/15 |

27B 在 rename-file 上反而是延迟组更好：五次里四次先 `search_tools("rename a chapter file")`，
而常驻组有两次照旧用 `propose_edit` 改标题行——**去搜这个动作本身让它把「改名」想成了文件操作**。
delete-folder 两组都几乎全败，是夹具的问题（假 `list_files` 里没有「旧稿」，27B 会一直找，
4B 不会），说明不了延迟的代价。样本太小，只能读作：代价随模型变大而变小，至少不再是负的。

**局限**：4B 每格 10 次、27B 每格 5 次，假工具结果，轮数上限；延迟格的失败里有几次是轮数不够
而不是方向错。前沿模型上这条间接不该有可测的代价（Anthropic 自己的 tool search 就是按这个
前提设计的），没有测。

**以后要调这里**：先跑 `AB_BASELINE=1` 和不带它的两组对照，看的是**需要延迟工具的格子**；
如果某个作者常用的小模型在那几格掉得比这里多，下一步是再加一个不需要模型配合的装载信号
（作者这一句话命中 `matchGroups` 就预装——候选里被放下的第三种），而不是把两组搬回常驻。
...
| --- | --- | --- | --- |
| 1 计量 | ✅ 已落地 | —（修正，不是节省） | — |
| 2 ④族缓存 | ✅ 已落地（仅官方端点） | 0 | 重复头部 ≈ -90%，首 token 延迟同降 |
| 3 schema 护栏 | ✅ 已落地 | **0**（量过：可删的只有 ~225 token，且有反例，见 §3） | — |
| 4 briefing 去重 | ✅ 已落地 | **zh -1,460 / en -800**（实测，见下） | 同比例 |
| 5a 随方案装载 | ✅ 已落地 | **-2,542**（实测） | 同比例 |
| 5b `load_tools` | ❌ 决定不做 | ~~1,600~~ | — |
| 5d `search_tools`（重开 5b，§6.1） | ✅ 已落地 | **−3,102**（实测，常驻 10,828 → 7,726） | 同比例；小模型上需要这两组的任务成功率 −6/30 |
| 5c 按方案**形状**分组装载 | ✅ 已落地 | —（不是节省，是不浪费） | — |

5c 是 §6 那句结论的第一次兑现——「把更多组挂到由运行状态自动装载的机制上」。集合整理
的三个工具（`lore_organize`，见
[`lore-collection-plan.md`](../lore/lore-collection-plan.md) §8）没有沿用 5a 的全有全无，
而是按**已批准方案的形状**分组装载：批准一份改写正文的方案不装集合工具，批准一份整理
方案不装 `rewrite_lines`。它省不到默认场景的占用（默认场景两组都不装），省的是「批准了
一件事，另一件事的工具也跟着倒出来」——顺带少给模型一格走神的机会。仍然零模型配合，
所以 §6 否掉 5b 的那条理由在这里不适用。

五条里两条的收益是 0，而这**不是失败**：3 是量过之后确认无处可省（留下的棘轮防止
它再涨回去），2 换的是钱和首 token 延迟而不是占用。真正省下上下文的是 4 和 5a。

**briefing 的成本随界面语言差一倍**，这是估算时漏掉的一条：`ai.instructions.agent`
中文 3,418 字符 ≈ 3,400 token（CJK 约 1 token/字），英文 8,125 字符 ≈ 2,030 token
（约 4 字符/token）。所以 §4 原估的"省 ~2,000"只在中文界面下接近：

| 语言 | 现状 | 精简后 | 省 |
| --- | --- | --- | --- |
| zh-CN | 3,418 字符 ≈ 3,400 tok | 1,941 字符 ≈ 1,940 tok | **-1,460** |
| en | 8,125 字符 ≈ 2,030 tok | 4,924 字符 ≈ 1,230 tok | **-800** |

两份 locale 不能机械照搬同一种删法：英文那份把每个工具讲得更细（单是「Tools」一节
就 5,300 字符），删掉的主要是逐工具复述；中文那份更紧，删的是同样的四段但比例不同。
两份**共同逐字保留**的是「知识库流程五步」和「大文件必须分段写」连同它的**理由**。

### 端到端（实测，中文界面、默认子代理全关、尚未批准任何知识库方案）

```
                     改动前              改动后
工具 schema          8,501（36 个）       5,959（常驻 27 个）
briefing (zh)        3,400               1,389
系统提示               379                 379
────────────────────────────────────────────────
每轮固定头部         12,280              7,727      −4,553（−37%）
```

④ 族官方端点上这 7,727 里的绝大部分还走缓存价；批准知识库方案后回到 ~10,269，
仍比改动前低 16%。

估算三次都偏乐观（~6,000 → ~6,600 → ~7,000 → 实测 7,727），原因每次一样：
