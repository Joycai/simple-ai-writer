...
- **按对话开启**。芯片的值是会话的属性（`ChatSessionMeta.stateMode`，随会话落盘、随会话恢复），不像计划模式那样切换会话就归零：历史的形状是它造成的，换回来的会话必须把它显示出来。新对话默认从关开始，见下一条。
- **新会话默认打开**（2026-09-17，`app:skillStateDefaultOn`，默认不勾）。Beta 开着时开关行下多一个勾选框；勾上后每个新对话的状态记忆**起点**是开，作者仍可在那次对话里关掉，能力菜单那一行的小字换成「beta · 新会话默认开」，说明为什么一个新对话已经亮着。
  - **为什么是勾选框、不是第二个开关**：实验室每行右边只有一个开关，这是那一栏的约定；子选项只在父开关打开时有意义，所以挂在说明文字下面、随父开关出现和消失（同「接着去 →」的位置）。
  - **为什么只改起点**：会话的值仍由 `meta.stateMode` 决定并随会话落盘。已存的会话按它存下时的样子恢复，不被偏好改写——历史的形状是它造成的。
  - **为什么还没开始的空白对话也跟着变**（`applyStateMemoryDefault`）：作者在设置里勾上后回到对话框，眼前那个空白标签就是「新会话」；如果它还是关的，看起来就像设置没生效。没有轮次、也没有存档行的对话才算没开始。
  - **为什么 Beta 关着时不起作用**：`newChatStateMemory()` 同时要求两者都开。勾选的值会保留，重新打开 Beta 时照旧生效。
  - 默认关，理由同 §5：两三轮的闲聊反而多付一次更新请求；主要做长任务的作者自己勾上。
...
- 2026-09-17 · 新会话默认打开：`stateFlag.ts` 的 `isSkillStateDefaultOn` / `newChatStateMemory`；`agentStore` 的 `freshChat`（新标签、复用空白标签、换项目）与 `applyStateMemoryDefault`；实验室勾选框；能力菜单的小字；`chatSessions.test.ts` 三条。设计稿：https://claude.ai/artifact/V6vj4jQgs8PR4ujFZZyPTC 。
