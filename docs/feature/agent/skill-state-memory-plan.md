...
- **新会话默认打开**（2026-09-17，`app:skillStateDefaultOn`，默认不勾）。Beta 开着时开关行下多一个勾选框；勾上后每个新对话的状态记忆**起点**是开，作者仍可在那次对话里关掉，新对话里、状态记忆亮着时，能力菜单那一行的小字换成「beta · 新会话默认开」，说明为什么它已经亮着；其余情况（已开始或已恢复的对话、作者手动关掉的）仍显示「发送前折成执行状态」，不丢掉对功能本身的说明。
  ...
  - **为什么只在勾选框变化时同步，切 Beta 不同步**：同步不区分「默认给的值」和「作者在空白对话里亲手点的值」，切 Beta 时同步会冲掉后者（默认不勾、手动打开、去实验室把 Beta 关了又开，回来就被改回关）。Beta 关着时这个值本来不起作用，没有必要为它同步；代价是先勾「默认打开」再开 Beta 时，眼前那个空白对话要等下一次新会话才亮。
...
- 2026-09-17 · 新会话默认打开：`stateFlag.ts` 的 `isSkillStateDefaultOn` / `newChatStateMemory`；`agentStore` 的 `freshChat`（新标签、复用空白标签、换项目）与 `applyStateMemoryDefault`；实验室勾选框；能力菜单的小字；`chatSessions.test.ts` 三条、`stateFlag.test.ts`。设计稿：https://claude.ai/artifact/V6vj4jQgs8PR4ujFZZyPTC 。
