...
> **状态：`shipped` `unverified`**（2026-09-23，一个 PR 四个提交）。租期 N = 1 由作者
> 拍板。落地时和原方案不同的五处见 §9，每处写了理由；`unverified` 是因为 §8 的剪贴板
> 实测还没做——判定规则按原方案实现，探针已经在代码里，等两个平台的实测表回填。
>
...
| 历史里只留最新 **3 条**带图消息，其余去图留字（留下的那句是 `runtime.ts` 的 `ELIDED_IMAGE`） | `runtime.ts` `elideOldImageResults`（`MAX_IMAGE_RESULTS`） | 图**已经不是**永驻的；但单位是「消息」不是「轮」，见 §5.1 |
...
- 现有三道上限（3 条 / 24 MiB / 窗口）**全部保留**，先到者生效。租期是更早的
  一道，不是替代。
- 落点：`stores/agent/chatJob.ts` 里作者的新问题进入历史、记成 turn start 的那
  一行之后，调一次 `lib/agent/imageLease.ts` 的 `elideExpiredTurnImages(history, meta)`
  （不变量 4）。原方案写的是塞进 `trimHistory`、再判断「最后一条是不是新一轮的
  turn start」——那样要把 `meta` 穿过 `runAgent` 的签名，而且每个工具轮次都要跑一次
  判断；新问题入历史那一行本身就是「新一轮开头」，工具轮次、「继续」、恢复都不经过
  它，前缀在一轮之内天然稳定。见 §9。
- 一次性的 agent 运行（AiPanel 的 Agent 模式）不走 `chatJob`，天然跳过。
...
原计划两个 PR（第二片等第一片合并后再切）。落地时改成**一个 PR 四个提交**（作者
2026-09-23 定）：租期只动一处调用，拆成两个 PR 只是多等一轮合并；提交按「逻辑层 /
界面层 / 租期 / 文档」切，每个提交单独能编译、测试通过。下面保留原来的两片划分，
内容没变。
...
**探针（已在代码里）：** 用 `VITE_PASTE_PROBE=1 pnpm tauri dev` 启动，在助手输入框里
粘贴，开发者工具控制台会打出一行 `[paste-probe]`：`items` 的 `kind/type`、`types`、
`files`（名字、类型、字节数）、文字长度。实现时还多了一个要看的格子：同一次复制
会不会以两种格式各给一个 `file` 项（比如同一张图的 PNG 和 TIFF）——那样会出两枚
内容不同的芯片，得在判定里只取一项。

| 来源 | macOS（WKWebView） | Windows（WebView2） |
| --- | --- | --- |
| 系统截图 | 待测 | 待测 |
| 浏览器「复制图片」 | 待测 | 待测 |
| Finder / 资源管理器复制文件 | 待测 | 待测 |
| Word 复制一段带图内容 | 待测 | 待测 |
| 微信截图 | 待测 | 待测 |

## 9. 落地时的改动（2026-09-23）

1. **租期的落点**从 `trimHistory` 移到 `chatJob` 的新问题入历史处（§5.2 已改写）。
   效果相同，运行时签名不动，也不会被一轮里的每个工具轮次反复判断。
2. **`stashId` 用 `nanoid(10)`**，不是 uuid：仓库里会话一类的 id 都这样生成。
3. **芯片缩略图走 `imageToThumbnailDataUrl`**（`useImageThumbnails`），不是
   `imageToDataUrl`：同属渲染读法，一张全尺寸 data URL 塞进 16px 的格子是浪费，
   气泡缩略图已经是这样做的。
4. **芯片不做灯箱预览。** 芯片本身是「点一下移除」的按钮，里面再放一个能点的区域
   是嵌套交互；发出去之后气泡缩略图可以点开看大图。
5. **所有图片芯片都带缩略图**，不止贴进来的——同一行里一半有图一半没有，读起来像
   两种东西。

顺带的几个小决定：贴图被拒的提示用输入区已有的 `refError` 一行（`@` 被拒时用的
就是它，属于输入区而不是会话）；满了的图不落盘；`newChat` 复用一个空标签页时保留
它的 `stashId`（上面可能还挂着已贴未发的芯片）；清扫与显式删除都先确认目录在
`.ai-writer/tmp/chat/` 之内再删——`stashId` 来自会话 blob 与数据库列，两处都能被
手改。`MAX_MESSAGE_IMAGES` 是共享常量，扮演的 `@` 图也随之变成 5 张。
