...
- **粘贴还在变成芯片时不能发送。** 「打字 → ⌘V → 回车」是贴图最常见的节奏，芯片出现前按下回车，消息会不带图发出去，芯片随后落进已清空的输入框、跟着下一条走。所以 `markPasting` 可订阅，发送按钮在这段时间里灰掉。
- **粘贴队列按标签页、放在模块里**，和编号表一样。输入区每切一次标签页就重挂一次，挂在组件实例上的队列切走再切回就是一个新的空队列，两次粘贴会一起越过上限、抽到同一个号。
- **【附图】的路径和暂存标注不进知识库匹配。** `buildChatMessage` 另给一份 `matchText`，图只留名字。自动匹配是对名字和别名的子串匹配：`.ai-writer` 会命中叫「AI」的条目，英文标注里的 "deleted" 会命中角色 "Ted"，`assets/<文档名>/` 会命中文档名里的人名。路径是给模型回头读图用的，对匹配只是噪声。
