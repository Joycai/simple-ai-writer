...
> **状态：`shipped` `unverified`**（2026-09-23，一个 PR：四个分片提交加 review 修复）。租期 N = 1 由作者
...
3. **像素是租的，路径是留下的。** 图在上下文里有租期，到期只去像素；留下的文字里
   永远带着路径，模型需要时按路径自己续租（`read_image`，有看图子代理时委派它——
   留下的那句话本身不点名工具，§5.2）。
4. **淘汰只发生在新一轮的开头，从不在一轮中间。** 两个理由叠在一起：M1
   （[`window-edge-plan.md`](window-edge-plan.md) D2：模型马上要看的东西不裁），以及改写历史前缀会打掉
...
- `stashId` 是一个 uuid（落地用 `nanoid(10)`，见 §9），**第一次真正写盘时**才生成，挂在 chat 状态上，并写进会话
...
- 贴图芯片 = 现有附件芯片 + 左侧 20px 缩略图（`imageToDataUrl`——**渲染给人看**
  ...
  （落地：16px、走 `imageToThumbnailDataUrl`、所有图片芯片都带、芯片不做灯箱——
  见 §9。）
...
- **例外：作者明确删除会话**时直接 `removeDir` 那一个目录，不等宽限——「删掉后也
  清理掉」是作者对这个动作的预期。（原来这里还写了「被删的会话不可能在别的窗口里
  还有效」——不对：多开时另一个窗口可以正开着这一行，它的气泡缩略图会随之失效，下次
  存档重新插出一行、`stash_id` 指着已删的目录。与下面「宽限保护不到的一格」同属多开
  边界，接受。）
- **宽限保护不到的一格（已知，接受）：** 宽限看的是目录的 mtime（最后一次往里贴图
  的时间），不是会话最后活跃的时间。同一个项目开两个窗口，A 窗口存档时的自动修剪
  删掉了 B 窗口里正开着的一个旧会话的行，而那个会话最后一次贴图早于 24 小时——下次
  任一窗口重新打开项目时，它的目录会被清扫掉。要修得在每次存档时刷新目录的 mtime，
  为一个本来就不受支持的多开场景给存档热路径加一次写盘，不值。
- **降级再升级（已知，接受）：** 旧版本不认识 blob 里的 `stashId`，存档时会丢掉它
  （列里还在）；回到新版本后再贴图会生成新的 id 并覆盖列，旧轮次的贴图 24 小时后被
  清扫。降级不保贴图。
...
- **N = 1 的理由**：对一张图的追问（「那第二张呢」「左上角那行字是什么」）几乎
  ...
  工具轮次的图片钱。
- **不分来源，按轮一起到期。**（原方案这条的标题是「只管作者附的图」，与下面的
  结论相反，落地按结论做。）工具循环读进来的图（`Visual reference for read_image: …`
...
- **只数答过的轮。** 失败或被停下、一个字都没答的那一问仍留在历史里；把它算作一轮，
  作者重问那句追问时图已经到期了。所以「之后 N 轮」数的是含 assistant 消息的轮。
- 到期留下的那句话**不点名工具**：有看图子代理时主模型手里没有 `read_image`，知识库
  图集的图要用 `read_lore_image` 读回——点名一个这次运行没有的工具正是
  `tool-presence.md` 说的那种错。
- 回退（`rewind.ts`）不恢复已经到期的像素：到期是原地改写，回退只截断历史。路径
  还在，模型需要时照样能读回来。
...
- 像素去掉之后，这段文字还在（`imageHistory` 的「字留下」规则），模型看到的是
  「这里曾有一张图 + 它的路径 + 到期留下的那句『还要的话再读一次』」（`imageLease.ts`
  的 `EXPIRED_IMAGE`；`runtime.ts` 的 `ELIDED_IMAGE` 是上限淘汰留下的另一句）——
  现在这句话第一次有了可执行的对象。读回用 `read_image`，有看图子代理时则是
  `delegate(vision, refs:[路径])`，所以那句话本身不点名工具（§5.2）。`@` 的图同样受益，所以这一条不分来源都改。
...
原计划两个 PR（第二片等第一片合并后再切）。落地时改成**一个 PR**（四个分片提交，外加各轮 review 的修复提交）（作者
...
就是它，属于输入区而不是会话）；满了的图不落盘；`newChat` 复用一个空标签页时，只有
它**从没存过档**才保留 `stashId`（上面可能还挂着已贴未发的芯片；存过档的——回退到
第一问的会话——id 归它的行，带给新会话会让两行认领同一个目录，删一个就删了另一个
的图；存过档、输入框里又挂着贴图芯片的空标签页干脆不复用），打开历史会话时也不
复用输入框里挂着贴图芯片、或者正有一次粘贴在写盘的空标签页（芯片指着的目录会从此
没人认领）；`stashId` 在第一次
真正写盘前才生成，重复、超限、读不出的粘贴不会给标签页留下一个空目录 id；「粘贴的
图片 N」的号在本窗口内只增不减，移除一枚芯片后再贴不会撞号；剪贴板说有图却给不出
字节时提示「读不出来」而不是把粘贴吞掉；清扫与显式删除都先确认目录在
`.ai-writer/tmp/chat/` 之内再删——`stashId` 来自会话 blob 与数据库列，两处都能被
手改。`MAX_MESSAGE_IMAGES` 是共享常量，扮演的 `@` 图也随之变成 5 张，【附图】同样带
相对路径；发不出去的图（模型读不了图、超出字节预算）那张清单里，暂存图也带「随会话
删除」的标注——读不了图的模型只从那里知道这张图。

整体 review 第 4 轮又补了四处：
- **粘贴还在变成芯片时不能发送。** 「打字 → ⌘V → 回车」是贴图最常见的节奏，芯片出现前按下回车，消息会不带图发出去，芯片随后落进已清空的输入框、跟着下一条走。所以 `markPasting` 可订阅，发送按钮在这段时间里灰掉；运行中按回车排队的那一条，也等粘贴完成再发。
- **粘贴队列按标签页、放在模块里**，和编号表一样。输入区每切一次标签页就重挂一次，挂在组件实例上的队列切走再切回就是一个新的空队列，两次粘贴会一起越过上限、抽到同一个号。
- **【附图】的路径和暂存标注不进知识库匹配。** `buildChatMessage` 另给一份 `matchText`，图只留名字，贴进来的图连名字也不留（「Pasted image N」是应用起的名字，本身就含 "ted"）。自动匹配是对名字和别名的子串匹配：`.ai-writer` 会命中叫「AI」的条目，英文标注里的 "deleted" 会命中角色 "Ted"，`assets/<文档名>/` 会命中文档名里的人名。路径是给模型回头读图用的，对匹配只是噪声。
- **`stashId` 定了形状**（nanoid 的字母表，一段，1–64 位），在反序列化、拼写入路径和删除三处都要校验。原先只有删除有围栏，而贴图是往这个 id 下**写**：手改成 `../../参考` 的 id 会把图写进正文目录，而且删除又会拒绝它，于是永远清不掉。

## 10. 只贴图、不打字也能发送（2026-09-23 作者定）

**原来的问题。** 发送键要求草稿里有字。`@` 选图会往草稿里插 `@名字`，所以从没遇到过；
文件树的「发送到助手」也早就是这样。贴图让「截图、⌘V、回车」成了最常见的路径，回车却
什么都不发生。当时两条路：放开「只有图片芯片也能发」，或者保持现状、在芯片行提示「配一句话
再发」。

**定了放开，理由：** 图本身就是问题。「这张截图哪里不对」「看看这个」这句话作者不写，模型也
会从图里读出来——拿到一张没配字的图，模型的默认反应（描述它、问要做什么）正是作者想要的；
而「配一句话再发」的提示要作者每次多打一句本来不必说的话，最常见的路径每次都多一步。

**规则只有一条，`chatRefs.hasMessage`：有字，或者至少一张图片芯片。** 文件、条目、录音芯片
单独在不算——一份材料没被问任何事，模型只能猜；图不一样，看图本身就是一件事。输入区的发送
键、运行中回车排队、`sendChatTo` 的入口守卫都读这一条（守卫是同一句话手写的一份：`chatRefs`
在那里是懒加载的）。

**跟着改的四处，不改就会出错：**
- **线上只发【附图】块。** 空的正文不再作为最后一段拼进去，否则文字部分会以一个空段结尾。
  知识库匹配读到空串，检索扩写对空意图本来就直接返回，不会白调一次模型。
- **气泡里的缩略图是作者附的全部图，不只是真发出去的。** 原先 `turn.images` 只记上线的那几张；
  只贴图、发给读不了图的模型，这一轮就会是一个空白的提问。转录是「问了什么」的记录，图有没有
  上线，模型已经从【附图】之后那段说明里知道了。`turn.images` 只用于显示（缩略图、`pasteNumber`
  的已知表、失效引用修复），不进线路，所以放宽它不改变模型看到的东西。没字的提问也不再画一个
  空气泡。
- **取消排队把芯片也还回来。** `dequeueChat` 原先只还文字；只贴图的一条排着队被取消，就整条
  消失了。现在还 `{ text, refs }`，芯片插回输入框最前（按 `attachedKey` 去重）。这顺带修了一个
  老问题：带 `@` 芯片的排队消息，取消后芯片也是丢的。
- **会话标签取第一条有字的提问。** 第一问只有图时，原来的标签是空串，读作「未命名」；下一问
  才说清这段对话在干什么。

**回退（回到这里重说）连图一起还（后续 PR）。** 起初回退只把文字放回输入框，这对带图的提问
一直如此；只贴图的一问回退后输入框就是空的，整条消息等于没了。现在 `rewindChat` 返回
`{ text, images }`，输入区把路径重建成芯片：
- **走粘贴的同一条队列**（`usePasteImages` 的 `restore`）。重建要读盘、降采样，要花时间；
  排在一起，它就和粘贴一样挡住发送键、不让 store 把标签页交给别的会话，也不会和同时进来
  的一次粘贴各数各的芯片。
- **贴进来的图回来时还叫原来的「粘贴的图片 N」。** 编号从回退**之前**的全部轮次里算——回退
  已经把那几轮切掉了，切完再算，重启之后同一张图会拿到一个新号，作者就对不上了。
- **不过上限。** 还回来的是作者已经发出去的那条消息，原样还；这时输入框里多出来的，发送时
  照旧由【附图】之后那段说明交代。
- **读不到就说。** `@` 附的图可能发出后被移走或删了，照 `@` 的原因文案报在错误行；贴的图在
  会话暂存区里，会话在它就在。
- 回退时芯片和取消排队一样插在输入框最前、按 `attachedKey` 去重（`chatRefs.refsAhead`，两处共用）。
