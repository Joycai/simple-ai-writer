...
- **折不动的时候不画那条线**（2026-08-27）。竖线的意思是「越过此处，下一轮把
  ...
  单独的分支；设计稿 04c · 2h 给扮演面板撤掉预估态那道线，理由是同一条。
...
§9 的持久化只保最近 5 个会话，那对「刚才那次对话」够用，对**一次值得回来的
对话**不够——它会被之后 5 次随手提问顶掉，而作者事先并不知道哪一次值得留。
所以给行加一个开关：**固定**的会话不参与那 5 个名额，一直在历史菜单里。

- **沿用「固定」，不引入「收藏」。** 最近项目那栏已经把这个概念定死了
  （`lib/recentProjects.ts`：固定项不占名额、不被清空、单列一节），语义完全
  一致。同一件事第二个名字会被作者读成第二个功能——这正是
  `docs/reference/terminology.md` §2 要拦的东西。
- **行上的一个标记，不是第二张表。** `chat_sessions.pinned` 列（旧库走
  `PRAGMA table_info` → `ALTER TABLE`，`lib/project.ts`）。另建一张表就要在
  两处同时正确地删行，而落单的一条「固定记录」指向一个已经被裁掉的会话，
  是没有任何界面能表达的状态。
- **裁剪的两半都要带 `pinned = 0`**（`upsertChatSession`）。只在外层过滤，
  子查询仍会把 5 个名额发给固定会话——那样作者固定满 5 条之后，每次保存都会
  把刚聊完的那次删掉。测试 `chatSessionPin.test.ts` 钉着这条。
- **读取重新施加上限**（`listChatSessions`），因为**取消固定不删任何东西**：
  一次点击就永久删掉一段对话，没有确认、没有撤销，代价和收益完全不成比例；
  取消固定之后它回到「和其他未固定会话同一条规则」，由下一次保存的常规裁剪
  带走。作者当场看到的是它离开「已固定」那一节——那正是他要的那件事。
- **列表仍按时间倒序，固定不置顶。** 分节是**视图**的事（同 recents）：
  `resetChatForProject` 读 `sessions[0]` 当作「上次停在哪」，把固定项排到前面
  会让每次打开项目都恢复成一段旧对话。分节由纯函数 `splitChatSessions` 做。
- **固定数量不设上限。** 上限只能表达成「一次没生效的固定」，而这是作者亲手
  点的（同 `sanitizePinnedProjects` 的理由）；菜单改为可滚动来承接。

## 12. 回到这里重说（2026-09-03）

扮演面板早有这个把手（`roleplayStore.rewind`，05 §2.13）；这一节把它接到对话助手上：
作者轮脚注行悬停出现「回到这里重说」，确认后撤销那一问和它之后的全部轮次，原文回到
输入框。纯逻辑在 [`lib/agent/rewind.ts`](../../../src/lib/agent/rewind.ts)，store 动作是
`agentStore.rewindChat`，测试 `chatRewind.test.ts`。

- **是截断，不是重新播种。** 扮演那边可以把 wire history 整个丢掉，因为 transcript 是真
  相、下一次发送本来就会从它重新播种。chat 没有这样的来源——wire history **就是**对话
  （工具调用、结果、注入的条目、折叠出来的摘要），稿面上的轮次只是它的一种渲染。所以
  这里在目标轮的提问消息处切断历史，再把描述历史的 `meta`（轮起点、注入账本、文档账本）
  拉回同步——和 `buildCompactedHistory` 折叠之后做的是同一件事，用的也是同一个 `pruneLedger`。
- **一条不变量决定哪些轮能回：切口之上作者还看得见的，模型也必须还持有。** 已经折进摘
  要的轮不满足——它只剩摘要里的散文，而散文截不开轮次边界（扮演那边正因为这个才整份
  删掉 summary.md）。回到一个折叠轮意味着作者看着第 1–4 轮、模型却已经忘了它们，两个视
  图各说各话正是这个功能不能制造的东西，所以这些轮**根本不给按钮**（`rewindableTurnIds`）。
  状态记忆模式下每轮只留上一轮逐字，能回的因此只有最后一问——那也正是最常用的那一次。
- **第一问是唯一的反向例外**：它上面没有任何可见内容，回到它等于整段重来——历史归 null，
  下一次发送针对**新的**第一问重新取材，而不是沿用为旧问题取的种子。
- **紧贴目标提问之前的那条注入消息留下。** 每轮注入落在它服务的提问**之前**，按轮起点切
  断时它正好在切口下方。留着是对的：材料仍在 wire 里、账本仍说在，重问的那一句会像原来
  一样紧跟其后；丢掉它则要猜「切口上方最后一条 user 消息是注入还是上一轮的图片续发」。
- **文档账本清零**（`lastDocPath` / `bodyDocPath`）。被撤销的某条消息可能载着一次文件切换
  或一段正文窗口，账本不清，模型就永远不会再被告知一份它已经不知道的文档。代价是下一轮
  多发一次简报；另一头是答错文件。
- **摘要、执行状态、种子块不动**：它们都在序幕里、在每一个轮起点之前，切口够不到；它们描
  述的也都是保留轮之前发生的事。
- **不撤销的**：已批准落盘的修改（回退对话不会把正文改回去——确认文案明说这一点）、任务
  工作区里的笔记（同理，那是文件不是对话）、用量总计（token 已经花了）。
- **回到第一问后 DB 行暂时还是旧的**：`persistChat` 对空会话什么也不写，行 id 保留，下一次
  发送会原地覆盖它，而不是把没回退的那段对话留成第二个条目。在那之前重启会恢复出旧对
  话——一次手滑的回退因此是可挽回的，而不是没了。
- **提问带的图跟文字一起回到输入框**，重建成芯片——提问上只存了路径，所以要重新读盘；
  怎么编号、为什么走粘贴的队列，见 `chat-image-paste-plan.md` §10。
- **确认条长在被点的那一句下面**（沿用扮演面板第八轮的教训）：错误带说的是「刚刚这一
  轮」，天然在底部；回退问的是「你点的那一句」，它可能在几屏之上。之后的轮次调淡——「共
  N 轮」是个数字，调淡是那个数字在稿面上的样子。
