...
> 状态：`partial`（一期「行号契约」PR #396、二期「结构读与搜索上下文」、三期「导出预检 + inspect_html」已实施；还没做的见 §9）
...
- **`htmlArtifact` 专用 preset**（下一片，而且现在有数字了）。该任务跑的是全量 68 工具集，每轮为用不上的知识库写工具、角色扮演、生图、翻译付 ~10k。一个 HTML 专用工具集约 4–5k。要动 `TaskTools` 三值枚举和 pack schema 的校验，不是一行。**这条已经不是省钱而是能不能用**：`contextForecast.test.ts` 钉住了 32k 本地模型上工具表（15.3k）超过整个输入上限（14k），知识库因此分到零。棘轮不该再往上抬，该往下切。
