...
> 状态：`partial`（一至四期已实施：行号契约 PR #396 · 结构读与搜索上下文 · 导出预检 + `inspect_html` PR #398 · `write` 档；还没做的见 §10）
...
- **`plannedToolTokens` 把延迟装载的那组也算进去了**（下一片）。`lore_write` 那 9 个工具在方案批准之前根本不上线，但预估把它们算满，于是助手的每一条计量条都多报约 5.4k（15,337 vs 常驻 9,948）。方向是安全的，但在 14k 的上限上多留 5.4k 是很多。改它会动到每一条条的含义，值得单独一片。
