...
> **状态：`shipped`——五期全部实施完毕（PR1~PR5，见 §8）**，之后仍有零散修订（如 DashScope
> 原生异步路由）。生图现以 `imagegen` 子代理形态存在——见
> [`subagent-lld.md`](./agent/subagent-lld.md)。
>
> 写这份方案时的代码基线是 **v1.3.0**；正文里的「现状」段落都是那个基线的读数，
> 与今天的代码对不上时以代码为准。
>
...
**后续修补（实测日志驱动）：编辑时的画幅不能一律省略。** 上线后一条真实
请求暴露了「gpt-image-2 编辑不发 size」的过度校准：agent 明确要求「重构图
成 2:3 竖版」，而请求里 aspect 没有翻成任何 wire 字段，输出跟着原图画幅走。
修正后的语义是**三态**：明确要求了画幅 → 编辑也发和生成同一套**算出来的
具体尺寸**（档位短边 + 比例 + ÷16，官方 edits 文档虽只列 `auto + 三个预设`，
实际端点普遍照收任意尺寸，收不了的 400 不计费，由 adapter 做一次「最接近
预设」的回退，预设也被拒就去掉 size 重试）；没要求 → 不发 size（= 跟随
原图）。为此 `ImageParamSelection.aspect` 改为可选——`?? "1:1"` 的默认值
在编辑上是错的（会把竖图默认裁成方图），「没人要求」必须与「要求 1:1」
可区分。`edit_image` 工具同时补上 `aspect` 入参（「换成竖版」正是编辑的
常见诉求）。quality 的忽略是方言结构性的：nanobanana 的 `params()` 根本
不产出该字段，agent 传了也只在 GPT-Image 方言下生效。

**同期新增：生图审批卡的连批（批准并连批 N 张）。** 一次任务出多张图时
逐张点卡片太折磨，但配图被刻意排除在 本次都批准 之外（花钱的授权不能变成
模式）。折中是**计数授权**：卡片上选 1~5 张一并批准，之后的配图提案自动
通过并逐张扣减（`AutoApproveState.illustrateLeft`），**本轮 run 结束即作废**
（`illustrateRun` + `rejectAll`，聊天的布尔授权跨轮存活而这个不行——剩余
的花钱额度不能武装到作者还没读过的下一轮）。自动通过的结果给模型的回执带
「Applied under a standing grant — nobody read it.」，与写工具同一措辞；
指示 chip 显示剩余张数，点掉即恢复逐张审批。
...
**后续修补：改图拆成两个工具——`edit_image` 与 `redraw_lore_image`。** 作者给
...
审批卡片对 dest 无感。缺的只有工具这一层。

**被否掉的第一版：把 `edit_image` 一个工具拓宽成两者通吃**（`source` 同时收
项目路径和图集文件名，落点排成 `entity` → `path` → 源图所属图集 → 源图所在
文件夹的阶梯）。它能跑通，但方向反了：作者要的是**把「编辑知识库配图」和
「编辑图片」分清楚**，而通吃的那一版要从 source 长什么样去**推断**作者指的是
哪件事——推断是第二种出错方式，不是对第一种的修复。落点阶梯本身就是这个毛病
的症状：一个工具需要四级 fallback 才知道结果该放哪，说明它在替作者做两件事的
决定。

**最终形态是一对工具，各自拒收对方的输入、并在报错里点名对方**：

| 工具 | 收什么 | 结果落在 |
| --- | --- | --- |
| `edit_image` | `source` = 图片**文件路径**（`list_files` 的写法） | 原图旁边；给了 `path` 就落到那篇文档的 `assets/` 并回传 markdown |
| `redraw_lore_image` | `entity` + `file`（`read_lore_entity` 列出的**裸文件名**） | 存回同一图集，继承原图槽位 |

分界线是**可核对的**而不是要猜的：图集配图有条目、有裸文件名，图片文件有路径。
两边的错误路径才是这次修补真正的产物——`edit_image` 的 `source` 解析不出来时
先按文件名查一次图集，命中就回
「这是 X 的配图，改调 `redraw_lore_image(entity: "X", file: "…")`」；
`redraw_lore_image` 的 `file` 里带了分隔符就回「这是路径，改调 `edit_image`」。
选错的代价因此是一轮，而不是一句读起来像「这张图不存在」的死胡同。

命名上没有叫 `edit_lore_image`：`update_lore_image` 已经占着「改配图」的字面
意思（它改的是描述和槽位、不画图也不花钱），两个名字并排会比原来的混淆更糟。
`redraw_` 说的正是它和前者的差别。它也**没有**放进 `lore_write` 延迟组，尽管
其余写条目的工具都在里面：那个组的门是「lore 方案已批准」，管的是条目**内容**
的改动；这个工具花的是钱，所以门是生图审批卡——与 `generate_image` 往同一个
图集里存图完全一致。

`edit_image` 的落点新增第三种 dest `{kind:"file", dir}`（`saveImageInFolder`，
复用 `uniqueAssetName`，永不覆盖包括源图在内的任何既有文件）。**没有**走
「从图片路径反推它属于哪个文档」：`assetDirFor` 经 `safeAssetName` 做过转义、
长名还加哈希，是有损的，反推只能靠猜——落点宁可由 `path` 明说，不给就放在
原图旁边。
...
`optimize_prompt_options` 与 `output_format`（后者只有 5.0 收，不发则全系默认 jpeg，mime 按字节嗅探；09-23 起透明模式
下会同发 `png`，见下）。
...
- **透明背景做了，但开关在 agent 手里。** 改一张透明 PNG 不发 `background:"transparent"`，回来的是不透明 jpeg——
  立绘、贴纸类素材改一次就丢了透明。这个模式保的是**结果的背景透明**：主体可以改色、变形、换姿势（实测箭头从向右
  改成向上照样是抠图），但「加上蓝天背景」与它的承诺矛盾——实测天空被画进了主体里，照样计费。所以「输入透明就
  自动开」是错的，看提示词关键词判断是两种语言里的猜测；结果要不要一个填满的背景，只有写指令的 agent 知道。于是
  `edit_image` / `redraw_lore_image` 各带一个 `keep_transparency`（默认保留，要填满背景时设 `false`；助手预设
  +105 token，全在延迟加载的 `image` 组里，常驻部分不动），应用层再要求：**是改图**（`generate_image` 没有这个参数可关，参考图的透明背景也不是结果要保留的）、
  **恰好一张输入**、**头部带 alpha 的 PNG**、模型**能**保留透明。头部只说「可能透明」，不做解码：全不透明的 PNG
  上游出图前免费 400，adapter 去掉两个字段重试一次，画出来的东西与不带这两个字段时一样。agent 要求保留、而这次
  不会去要（绑定的模型不能，或带了引用图——该模式只收一张输入）时，工具结果说明被忽略及原因（与 `negative` 同一
  做法），免得 agent 对作者说「透明保住了」；措辞是「没有请求保留」而不是「结果不透明」——带抠图节点的 ComfyUI
  工作流仍可能回透明图。上面「`output_format` 不做」的例外也在这里：透明模式下必须同发 `output_format:"png"`。
- **「能不能保留透明」从方言推导，不做声明。** 最初是 `caps.transparent` 声明，review 发现两处漏：模型抽屉保存时
  按表单重建 caps，没有控件的声明会被丢掉；09-23 之前加过起步行的作者库里也没有它，又没有 UI 能补。
  `seedream-5-pro` 方言恰好只对应 5.0 pro / flash——正好是收这个字段的两款（lite 与 4.x 各有自己的方言），所以
  「ark 线路 + 这个方言」就是答案，老行、手加并选了方言的行、保存过的行都对（`keepsTransparentBackground`）。
  模型设置里**不加开关**（作者决定）。交互式出图会话不覆盖（没有 agent 参数）。同一处抽屉保存也会丢掉非 comfy
  模型的 `maxRefs`（起步行的 10 / 14），一并修了：抽屉没有这个控件，保存时沿用原值——**但只在原行不是 comfyui、
  线路与方言都没变时**。comfyui 的 `maxRefs` 是工作流的 LoadImage 数，lite 的 14 放到改成 pro 方言的行上多了 4 张；
  错的上限比没有更糟（没有上限时，超了是上游出图前的免费 400；错的上限会拒掉合法请求，或放过非法的）。
