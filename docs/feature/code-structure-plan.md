...
P1–P7 只搬文件、不改行为（唯一一处可观察的例外记在 §7 的 P4 那行：批处理两条子句之间从
快捷键另起面板任务，原先不出卡片、现在会出），但仓库里几十份**写在整改之前**的设计文档
留下了当时的文件名。
...
判据是**打开那个文件还能不能找到它**，而且要按符号问、不能按文件问——同一个老文件的两半
可能一半还在、一半没了：`registry.ts` 的**类型**仍从 `toolTypes` 转出（多一跳），它的**工具
定义**却没有（`REGISTRY` 是模块私有的，只能去 `toolTable/`）；`writeTools.ts` 的五个处理器模块
整份转出，`write/shared.ts` 不在其中。真的找不到的那些（`subagent.ts` 的纯查询、`agentStore`
的 `applyProposal`）在注记里逐个点名。

| 整改前文档里写的位置 | 现位于 | 阶段 |
|---|---|---|
| `lib/agent/subagent.ts` 的纯查询：`SubAgentKind` · `SUBAGENT_KINDS` · `DELEGATE_KINDS` · `SUB_PRESETS`（含 `longread` 档） · `subAgentModel` · `searchReadsPages` · `visionSubAgentModel` · `withSessionOverrides` · `chainCanSeeImages` · `resolveSubAgentConn` · `resolveVisionConn` · `MAX_PDF_BYTES` / `MAX_PDF_FILES`——即 `subagent.ts` 除 `executeDelegate` 外的**全部**导出 | `lib/agent/subagentModel.ts`。**`subagent.ts` 不转出它们**，按老路径去找会扑空 | P1 |
...
| `lib/agent/writeTools.ts` 的处理器 | `lib/agent/write/` 的 `planGate` · `loreFiles` · `loreAssets` · `memory` · `manuscript`——这五个 `writeTools.ts` 整份重新导出，老 import 照样编译。**`write/shared.ts` 是例外**（`nextProposalSeq` · `relocateInSnapshot` · `withAlias`）：它不在转出清单里，只能直接 import | P6 |
...
| `stores/agentStore.ts` 的 `runChatJob`、多会话辅助、聊天简报、自动归纳的触发、`inputCeilingFor` 那一路取上限 | `stores/agent/chatJob.ts`（`sendChat` / `sendChatTo` 仍在 `agentStore`，只是把活交出去）。这一半**不是**转出关系：`agentStore` 只转出 `buildResumeSeed` / `chatSurface` / `emptyChat`，`runChatJob` 本身模块私有 | P5 |
