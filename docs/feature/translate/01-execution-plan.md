...
- `enforceGlossary`：`陽菜→阳菜` 与 `陽菜さん→阳菜小姐` 同时存在时走最长优先。
...
**2026-09-19 补**：审查发现逐块的 `max_tokens` 在 ① 族上**从没发出去过**——`run.ts` 把它放进了
`ConnOptions.maxOutput`，而那个字段在 ① 上只用于规划（对 OpenAI 自家推理模型主动发 `max_tokens` 会 400，
所以不能改成一律发）。退化的块因此一直生成到本地服务自己的上限，「被截断」这个信号实际不存在。改为
`StreamOptions.maxTokens`：和 `topP` / `frequencyPenalty` 同一类「属于任务、不属于模型」的逐请求参数，
① 族照发 `max_tokens`；`maxOutput` 同值保留，给 ④ 族当必填的 `max_tokens`。

**实机验证**：用你给的一部日文轻小说第一话跑完整条管线（155 行 → 3 块 50/50/45），三块全部通过判定，19.3 / 23.4 / 17.4 tok/行，共 47 秒；空行、URL 原样穿透，成品行数与原文一致。这组数字也确认了 `TOK_PER_LINE_LIMIT = 35` 的裕度：真实值最高 23.4，离阈值还有很远。
...
用你给的日文轻小说第一话（148 行）跑一遍：3 块、约 60–70 秒、产出 `.zh.md`、审批卡片可见、执行日志有逐块进度、用量页有一行记录。
...
**实机验证**：整文件路径端到端跑了你给的第一话——155 行 → 155 行，3/3 块全部译出，47.5 秒，逐块进度事件（`1/3 块 → 2/3 块 → 3/3 块`）就地更新同一行日志，审批卡片带着正确的目标路径和 `3/3 块译出` 的说明。空行、作者署名行、来源链接全部原位不动。

---

## 6.5 追加 — 「翻译词典」条目（2026-08-23）

别名通道一个条目只能表达一个译名（条目名 = 译文，别名 = 原文写法），而作者手里往往有一份现成的对照词表——几十对词一条条建实体不现实。追加一条**整批**通道：

- **识别**：条目编辑表单里作者**显式勾选**的「翻译词典」开关（frontmatter `dict: true` → `LoreEntity.dict`，`glossary.isDictEntity` 只看这一个字段），任何分类下都行，可以有多本，条目名随便起。第一版用过名字/别名精确匹配「翻译词典」，按作者要求改成显式开关——约定终究可能误判，开关不会。复选框只在翻译 Beta 打开时露面，但隐藏 ≠ 丢标记（`dDict` 仍随 `entity.dict` 初始化并原样保存）。**每个保存 `EntityMeta` 的调用点必须把 `dict` 带过去**（LoreDetail 编辑、LoreMetaImproveModal、writeTools 两处）——漏一处，下次保存就把词典标记安静地抹掉，和 `FacetMeta.slot` 是同一个坑；未标记的条目 frontmatter 里**没有** `dict` 行，字节级不变。
- **格式**：正文一行一条 `原文->译文 #备注`，就是 Sakura 自己的术语表格式，作者写下的即模型看到的。宽容 `→`、markdown 列表符号、省略备注；标题/说明文字/空行静默跳过（`parseDictBody`）。
- **读取时机**：正文不在 `LoreIndex` 里（懒加载），所以 `tool.loadTranslateDict` 在一次运行开始读一遍，经 `RunChunkOptions.dict` 传入；**逐块命中筛选不变**，仍在 `collectGlossary` 里做（词表属于整份文档，注入只给命中的行）。
- **合并规则**：同一个源词同时出现在词典和别名上时**词典赢**——那一行是作者专门为翻译写下的对应关系。词典条目自身不进别名通道（它的名字/别名是给作者找条目用的）。上限 30 条对两个通道合并计数，长词优先不变，强制替换（`enforceGlossary`）对两个来源一视同仁。
- **开关**：跟随现有的 `ai:translate:useLore`——词典也是知识库的一部分，不另设开关。
...
## 6.6 追加 — 实机反馈两修（2026-08-24）

作者实机报告两个问题；先核对了 HF 模型卡（Sakura-14B-Qwen2.5-v1.0-GGUF），官方词典格式
**就是**本实现发的 `原文->译文 #注释` 一行一条，system/user 模板逐字一致——问题不在
发给模型的格式，而在两处别的地方：

1. **`=` 词表被静默跳过**。GalTransl 等工具的现成词表惯用 `原文=译文`，而
   `parseDictBody` 只认 `->`/`→`——一份 `=` 词表每一行都被静默跳过，词典等于不存在
   且无任何报警，作者看到的就是「不给词典」。两修：分隔符加 `=`/`＝`（发给模型的
   永远重排成官方 `->`）；词典条目存在却解析出 0 条时，工具结果里**报警**
   （`loadTranslateDict` 的 `warning`——passage 路径附在译文后并用方括号括开，
   避免模型把警告抄进文档）。
2. **块大小写死**。50 行是 14B/LM Studio 上的实测安全区，换个部署（更小的模型/
   上下文窗口）就会当场退化甚至挂掉——而挂掉是重试阶梯救不回来的（阶梯的前提是
   请求能回来）。改为设置项：子代理卡片上的「每块行数」（pref
   `ai:translate:linesPerChunk`，1–100 夹取，`clampChunkLines` 纯函数可测），
   两条路径（passage / 整文件）同用。1 = 逐行翻，给最脆的部署留了底。

同批追加：**词典标准化**（条目 AI 中心 → 词典标准化，仅词典条目可见）。自由格式
（表格/散文）的词表由模型**一次性**抢救成规范格式：模型只做搬运（结构化输出
`submit_dict_entries`，提示词明令逐字复制），格式由代码渲染（`formatDictBody`——
与 `formatGlossary` 一字之差：备注不截断，因为条目正文是作者的资产，24 字预算属于
发给模型的清单），产物必然能被 `parseDictBody` 读回。防改写用启发式核查：原文/译文
在原正文里逐字找不到的条数以黄字报给作者。分工与内置工作流卡第 5 步同一条原则——
每次运行的格式归一在 `parseDictBody`，一次性的格式抢救才属于模型，且作者过目后落盘。

---

## 6.7 追加 — 运行中的进度（2026-08-30）

作者实机报告「等的心里没底」。查下来是两件事叠在一起：

1. **进度事件从来没有渲染过。** `progressReporter` 发的是 `onNestedEvent` + `parentStep`，
   而 `buildLogModel` 第一行就是 `log.filter(e => !e.parentStep)`——带 `parentStep` 的事件
   只有 `delegate` 那一条路能重新露面（它有自己的子代理卡）。translate **不是** delegate
   （不变量 1 的直接后果），所以那一行「N/M 块」自上线起就被静默丢掉。日志上只剩一行不动的
   「日中翻译 · foo.md」，而它和一个卡死的端点长得一模一样——那两件事作者的反应完全不同，
   这是这个日志能省略的最误导的东西。
2. **只报块数不够。** 块是我们的切法（默认 50 行），作者对「还剩多久」的感觉是按字来的。

改动：

- **`ToolContext.onProgress`（新的通用缝）**——运行时**用它自己那一步**重发一次
  （`runOneToolCall` 把 running step 提成常量，回调只往上面挂 `progress`）。工具因此不用
  重建 step 的身份（round / call id / arguments），而重建错任何一个都会在第一行下面**再印
  一行**，而不是把第一行推着走。`ToolStep.progress` 只在 `running` 时有意义，settled 的那
  一步带回结果、把它顶掉。
- **`DocProgress` 加行/字**——分母是**送进模型的那些行**，不是文档行数：空行、URL、分隔线
  根本不进请求，算进分母，一份对话体小说的进度条会停在六成不动。失败的块照样计入 `done`，
  它的等待确实结束了，而没译出来这件事由 `failed` 说。
- **开跑前先报一次 0**——第一块要几十秒才回来，「这份稿子有多大」正是那几十秒里作者唯一想
  知道的事；等待里最难熬的那一段，原本恰好一个字都没有。
- **ETA 两块之后才给**——第一块的耗时含本地模型加载与显存预热，拿它外推六十块会报出一个
  荒唐的数，而作者只会记住报出来的第一个数。
- **UI**：运行中的工具行右侧显示进度而不是它还没有的结果，行底压一根 1px 进度线（与
  `SyncPresence` 同一套语汇：行不变形、不加高，只有一根线在走）；折叠着的标题行也带上同一
  串数字——长时间运行时那是作者唯一看得见的一行。

`onProgress` 是通用的，但目前只有 translate 用：别的工具都以秒计，给一个三秒的调用画进度条
只是噪音。
...
端点 LM Studio @ `192.168.2.206:11234`，`sakura-14b-qwen2.5-v1.0`，ctx 33536，素材为一部日文轻小说第一话（148 非空行）。
