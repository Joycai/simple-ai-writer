...
5. **PDF / 视频输入的声明不再按当前线路清掉。** 它们是模型的声明（§3 表），切到 ④ 再切回来不该重填。能不能用改在用的时候
   ...
   中转站上游丢 PDF 说是上游（capability-gating-plan §8.11）；勾了但线路 / 平台传不了，点名当前线路（`warnPdfNotSent`）——
   这时说「未声明」会让作者去勾一个已经勾上的开关（2026-09-23 补上，此前第三种落在「未声明」里）。翻译格式仍按当前线路清——它把模型**移出**所有选择器，留着一个
   看不见的声明代价太大。
