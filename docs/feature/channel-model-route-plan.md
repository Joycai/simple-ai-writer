...
5. **PDF / 视频输入的声明不再按当前线路清掉。** 它们是模型的声明（§3 表），切到 ④ 再切回来不该重填。能不能用改在用的时候
   ...
   按 capability-gating-plan §8.7「`refuses` 只收实测」，文档一句话不够判「不发」。所以 Chat 线路被关掉或排在 Responses 之后时，
   这条提示会点名 Responses；默认线路顺序下点名的是 Chat。补一次 DashScope Responses + PDF 的实测后再定这一格。翻译格式仍按当前线路清——它把模型**移出**所有选择器，留着一个
   看不见的声明代价太大。
