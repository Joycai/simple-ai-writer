...
5. **PDF / 视频输入的声明不再按当前线路清掉。** 它们是模型的声明（§3 表），切到 ④ 再切回来不该重填。能不能用改在用的时候
   ...
   这时说「未声明」会让作者去勾一个已经勾上的开关（2026-09-23 补上，此前第三种落在「未声明」里）。建议换线路时只点名渠道上
   **真能传**的那条（`pdfRouteFor`，`warnPdfOtherRoute`），没有就只建议换模型（`warnPdfNotSent`）：中转站上一条线路有 PDF 的拼法，
   文件仍可能被它背后的上游丢掉（Kiro 模型从 ④ 换到 ① 只会换来「上游丢 PDF」）。翻译格式仍按当前线路清——它把模型**移出**所有选择器，留着一个
   看不见的声明代价太大。
