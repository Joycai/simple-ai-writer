...
11. **两个平台都带 Seedream 出图起步模型，钉在 Chat 线路。** 出图走渠道同一把 key 的 `{Chat base}/images/generations`（出图接口
    ...
    **2026-09-24 补记**：手动选择也收紧了。对话类选择器（助手头部、角色扮演、各知识库模态、摘要模型、子代理设置页的对话档位、出图面板的「写提示词」模型）改走 `chatModels`——
    `conversationalModels` 再去掉出图 / 视频行；只有出图面板的选择器（`purpose="image"`）列出图模型。理由：「手动选什么都行」
    没有换来任何能用的选项——出图模型拿来对话第一轮就报错，而列在对话列表里只会让人以为它能对话（Nano Banana 这类名字不带
    「image」的尤其如此）。已经绑在对话 / 摘要上的出图 / 视频模型按选中项失效处理（`loadConfig` 读配置时、`updateModel` 改类型时）：对话模型回落到第一个能对话的模型，摘要模型清空（即跟随对话模型）。子代理的对话档位在运行时同样只认 `chatModels`，与设置页的候选一致——这也顺带补上了翻译计划不变量 2 当时有意留下的运行时缺口（Sakura 绑在 longread 上会被解析出来）。`conversationalModels` 本身不变，因为子代理设置页要从它里面分出 `imagegen` 档位的候选。
