...
> 状态：`partial`（2026-09-18 提出；P0 已实施，见 §11；P1–P4 未开始）。设计稿在 claude.ai/design 项目（本 app 的设计项目）的
...
1. **推断在读时做，不写迁移。** `platform` 为 `NULL` 的行每次读都由地址推断（`resolvePlatform`），抽屉下一次保存时写入。
   §5.2 说的「迁移日志」因此在 P0 没有落点——没有一步迁移可以记日志；受影响的声明改为在模型抽屉里**常驻**一行
   「{平台} 没有这个工具的拼写：开关保留，但请求里不发送」，搜索子代理的绑定处也有对应警告。理由：推断是纯函数、结果稳定，
   写一步迁移只多一个会半途失败的地方。
...
3. **声明保存时不再按线路裁剪。** 模型抽屉以前保存时把「本线路拼不出」的工具从声明里删掉；现在保留（不变量 4），
   开关照亮、下面一行说「不发送」，作者可以自己关掉。能拼不能拼只在两处算：请求（`effectiveServerTools`，adapter 各自再防一道）
   与任何「承诺能力」的地方——搜索子代理是否在线、能不能读网页（`serverToolsSent`，不读原始行）。
   `subAgentModel` 因此多了一个可选的 `providers` 参数：`routeTools`、子代理执行、设置页用它；手里没有供应商列表的界面
   （输入框的子代理小签）退回读声明，最坏是多显示一个签，执行时会被 `executeDelegate` 拦下并说明原因。
4. **协议原生工具在没列出的平台上是「未实测」，不是「没有」。** ④ 族的 `web_search_*` 与 ② 族的 `{type:"web_search"}`
   在 OrcaRouter、New API、自定义平台上照旧可开，开关下写「未实测」——这是 ④ 族今天在中转站上的既有行为，
   §10 第 2 条的待决问题在实测前按这个倾向执行。① 族（Chat Completions）没有任何协议原生的服务端工具，所以非千问平台上一律没有。
...
7. **抽屉里改地址会带着平台走**（`platformForAddress`）：新地址能认出平台就换过去；认不出时，原平台若是按主机识别的（千问、DeepSeek……）
   就退回 `custom`，原平台若是作者自己选的（New API / 自定义）就保留。

行为变化只有 §4 规则 1 那一条，外加一处同源的：xAI（`openai_responses_compat`）以前会收到 `web_extractor` 与两个图片搜索，
那是千问的名字，xAI 会拒绝（landscape §7 第十一个样本）；现在只发 `web_search`。
