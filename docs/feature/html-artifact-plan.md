...
只 flush 脏缓冲区的前提是 `isDirty` 说真话，这一点顺手补上了：`saveNow` 的写盘是异步的，作者在写盘途中再敲一个字，旧实现在写完后仍把 `isDirty` 置假、把 `saveTimer` 置空——那一字没落盘却被标成干净，新布下的定时器也成了没人能取消的孤儿。现在写完时按「写出去的那份」对比当前缓冲区，只清掉自己那个定时器（`editorStoreSaveNow.test.ts`）。顶栏与说明页的入口**不 flush**：它们只出现在不可编辑的文件上，缓冲区里装的不是这个文件；主题文件从不进编辑器，同样无需 flush。
