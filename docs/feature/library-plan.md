...
> 状态：四期全部实施完毕（本文档随实现推进更新）。第四期（2026-09-24）把文库从「自动铺满每个文件夹」改成「作者挑进来的东西」。
...
- 备选名（弃用理由）：「书架」过于小说专属，与多领域 pack 定位冲突；「合集/集合」指单个分组而非整个视图；「文库」与「知识库」都带"库"字但一个装文稿一个装条目，语义正交。
...
3. **资源展示**：`Volume` 新增 `resources`（该文件夹直属的非章节文件）。每列在章节卡下方渲染紧凑的资源区：图片带缩略图（`imageToDataUrl`，与 ImagePreview 同路），其他文件用图标 + 文件名；单击在编辑区打开（与 FileTree 行为一致）。仅有资源、没有章节的文件夹（含根目录）也成列。`assets/` 仍整体跳过——它是文档的配图目录，归属各文档自己的预览。
...
### 第四期（已实施，2026-09-24）：文库改为作者挑选

**起因**：打开一个 76 篇文档、36 个文件夹的知识库目录，文库把每个文件夹都铺成一列（`groupVolumes` 递归整棵树），列是 `flex:1; min-width:0` 放在 `overflow:hidden` 的容器里，几十列被压到几像素宽，中文一行只放得下一个字，看起来像竖排。同一份分组还喂给续写：随手放在工作区里的一篇 md 会被当成某个文件夹的「第 N 章」。这是文库里最后一处「自动发现」（codemap 早就写着「书脊与续写是作者的显式行为」）。

**设计稿**：Claude Design 项目 → `01f 文库 · 挑选 Library Curation`（屏 1a–1g，1z 是用户拍板的三项）。

1. **成员表**写进 `.ai-writer/outline.json` 的 `members: { folders, docs, exclude }`（`lib/context/library.ts`）。
   - `folders`：整夹加入，其直属文档与资源都在，**之后新建的也算**；不递归，子文件夹要单独勾（选择器的「连同子分组加入」是一次性逐个勾上，不是新的成员类型）。
   - `docs`：单挑的文档（所在文件夹不是整夹成员）。这一列标「已挑 N 篇」、不显示资源，新建的文档不会自动加入。
   - `exclude`：整夹成员里被单独移出的文档。没有它，从整夹里移出一篇就得把整夹拆成单挑，资源和「以后新建的也算」一起丢掉。
   - 只列章节文件（md / markdown / txt）；资源不能单挑，只随整夹出现。
   - **为什么不另开 library.json**：书脊本来就是「这本书由什么组成、按什么顺序」的家，续写、AI 面板、文库都已经在读它；分两处放，改名改写要改两处，续写要读两个文件。
2. **文库即书脊**：`resolveVolumes` = `groupVolumes → applyMembers → applySpine`，续写的【全书前情】/上一篇结尾与 AI 面板的桥接候选**只在文库里找**。当前文档是章节但不在文库时，AI 面板续写下方一条提示 +「把它所在的分组加入文库」（设计稿 1g 方案 A）。
   - **弃用的方案 B**（没整理过的项目续写仍按文件夹自动分组，只让文库显示从空开始）：老项目零感知，但「这本书由什么组成」有了两个答案，作者第一次挑选之后续写行为会突然改变。用户选了 A：只有一个真相。
3. **迁移**：没有 outline.json → 文库为空（空态说明规则）；有 outline.json 但无 `members` → `order` 里**章节列表非空**的卷算整夹成员（作者当时真正在排的卷；空卷与只有资源的文件夹不迁）。任何一次写盘都会把 `members` 显式落下，此后不再推断——显式的空表也不会被重新推断（`outlineSpineFile.test.ts` 钉住）。降级：旧版本 app 重写 outline.json 会丢掉 `members`，回到新版时按 order 重新推断一次。
4. **路径变更**：侧栏、文库、agent 的移动都经过 `projectStore.moveEntry`，改名成功后 `moveInSpineOnDisk` 把顺序、在写、成员一起改写到新路径，并递增 `spineRev`（文库与 AI 面板据此重读）。规则：
   - 移动的目标路径不会被占用，所以书脊里落在目标路径上的条目都是侧栏删除留下的残留——先清掉，否则改名到一个删过的卷名上会继承它的整夹身份、在写标记和排序。
   - 一篇**章节**移动前是成员、移动后仍是成员（从整夹移到非成员文件夹 → 补成单挑）；资源和子文件夹没有逐篇身份，不补。
   - 被排除的章节移到哪里都仍被排除（「原来不在，现在仍不在」），包括移进另一个整夹成员。
   - 书脊没变化的移动（图片、杂项）不写盘、不递增 `spineRev`。
   - 删除不挂钩：成员表读时自愈（指向不存在的条目被忽略），选择器确定时 `pruneMembers` 剪枝。
   - 同时修掉了一个老漂移：以前侧栏改名会让排序和在写标记悄悄丢失。
5. **移出文库**不删文件，不弹确认；分组移出时它的**排序记录一并清掉**（在写标记保留）——外部记录只跟着「在文库里的东西」走（设计稿 1z ③）。
6. **布局**：列 `flex: 1 0 260px`，列少时均分铺满，列多时保持 260px 横向滚动。章节卡的四个移动按钮平时宽度为 0、悬停或键盘聚焦时展开（仍在文档流里——做成浮层会盖住摘要按钮），否则 260px 的列里标题只剩一两个字。
   - **删除分组**只在文件夹里什么都没有时出现（按文件树判断，含子文件夹、`assets/`、被移出文库的文档）：后端 `removeDir` 递归且不备份，而「连同子分组加入」会让只装着子卷的父分组显示成空列。
7. **选择器**：居中模态（设计稿 1z ②：挑选是一次性的整理动作，做完回到文库；抽屉适合边看边改，这里用不上）；勾选框是 13px 零圆角自绘方块（原生 checkbox 是平台画的圆角框，改不成本应用的方块语言），`role="checkbox"` + `aria-checked="mixed"`。

## 4. 不变式

- spine 仍是 overlay：文件系统是存在性唯一真相，`outline.json` 只记**顺序、在写与文库成员**；指向不存在路径的条目读时忽略。
- 文库只含作者挑进来的东西；续写与 AI 面板看到的书就是文库（`resolveVolumes` 是唯一入口）。
- 一次移动之后，章节的文库身份不变；目标路径上的残留条目不会被继承。
