...
> 状态：**已实现**（三个 commit，按 §7 的顺序）。实现记录见 05 §10；与本设计的
> 唯一偏差：§3.3 预估 runJob 收到 ~170 行，实际 ~290——ToolContext 装配和
> 收尾段本来就没打算动（§8 方案 B/C），预估漏算了它们的注释密度。历史准备
> 三分支确实缩成了「一行重试 + 两次调用 + 各自的状态 patch」。
