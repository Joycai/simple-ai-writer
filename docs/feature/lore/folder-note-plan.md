...
| 落点 | 做什么 | 为什么在这里 |
|---|---|---|
| `listWritingFiles` | 目录行下第一行挂 `(index.md: 摘要)`；`draft` 标一个词；`deprecated` 目录只留目录行与那一句，不列文件、不进子目录，并在句尾报「N 个文件未列出，read_file 给路径照样能读」 | 规则在生效那一刻到达（`lineEcho.ts` 的先例），不进 schema、不吃每轮常驻的工具预算 |
| `searchWritingFiles` | 收集文件时跳过 `deprecated` 子树，结尾报「另有 N 个文件在 M 个标为 deprecated 的目录里未搜」 | 实际收益最大的一处：三版废稿会吃掉一半的 40 条命中 |
| `documentBrief` | 【当前文件】块多一行：最近一层祖先的说明摘要；祖先链上任何一层 `deprecated` 时再多一句「这个文件在标为不再参考的目录里」 | 作者正在编辑「废稿/第三章.md」时助手应该知道这是废稿 |
| `isChapterFile` | `index.md` 不是章节 | 否则它成了书脊上的「第零章」，进续写上下文 |
| `search_text` 的可搜文件 | `index.md` **仍可搜** | 作者在说明里写的话（「人设以 v2 为准」）正是 search_text 该找到的 |
| `rowMeta.ts` | 第八种行 `note`：`index.md` 一枚说明图标，右列一个「说明」 | 作者在树上要一眼认出它不是一章 |
