...
> 状态：调研背景。方向 1+3 已被 [`lore-facet-plan.md`](lore-facet-plan.md) 落地；
> 方向 2（状态跟踪）与方向 4/6（递归激活 / 向量兜底）尚未实现，仍是未来方向参考。
>
...
把一个实体拆成"**核心卡 + 多个特征（facet）**"，激活单位从实体降到特征：
...
服装本质上不是静态资料，而是**随剧情演进的可变状态**。关键词匹配永远有失灵的时候（正文里没提"战甲"两个字，但角色确实穿着战甲）。根治方案是给故事维护一个显式的状态层：
