...
- **写完标记之后再核对一遍**（分片 + `faces.css`），有缺的就撤掉标记、报 `disk`。上面那次核对和标记改名之间隔着几次 IPC，而 `fs_write_text_file` / `fs_write_binary_file` / `fs_rename` 都会先 `create_dir_all` 父目录：另一个窗口的删除要是正好落在这段空隙里，标记就会写进一个重建出来的空目录，下面既没有分片也没有 `faces.css`。这个后果不会自愈：`readInstalled` 说装好了，`downloadFontPack` 因此不再下载；`packFacesCss` 读不到 `faces.css`，`syncPackFaces` 只记一条警告、什么都不注入，卡片却一直显示「已就绪」，字形一直是回退的，只有作者手动删掉再下才恢复。
  - **为什么事后核对就够**：删除要么落在第二次核对之前（核对看到缺口，撤掉标记），要么落在它之后（删目录连标记一起删）。两种情况最后都和磁盘一致，不需要跨窗口的锁。
  - **代价是偶尔误判「没装」**：撤标记的那一刻，第三个窗口可能已经把包完整重下、写好了自己的标记，这边会把它撤掉。这只让包显示为没装，下次选中时续传只比大小、不重新下载，补一个标记就好。误判「已装」不会自愈，误判「没装」会，所以宁可错在这一边。
  - **第一次核对仍然保留**：它让常见的情况根本不会出现一个盖在缺口上的标记，别的窗口也就读不到这样的标记。
...
- **单元测试**：
  ...
  - 「分片中途消失就不写标记」那条：模拟另一个窗口在**最后一个**分片落盘时删掉整包目录。「最后一个」是**数出来的**，不是按文件名指定的：三个 worker 并发，每个都要等一次真实的 `crypto.subtle.digest`，完成顺序不固定。原先写死「`c.2` 落盘时删 `a.0`」，`c.2` 不是最后一个时，删的那一刻 `a.0` 还没落盘，删了个空；之后 `a.0` 照常落盘，安装本来就该成功，测试约 1/30 的概率失败。这是测试的问题，不是代码的竞态。改成按计数触发后，去掉写标记前的核对，这条测试每次都失败。
  - 「核对和标记之间删包」那条：`faces.css` 落盘时（分片全部核对过、标记还没写）模拟另一个窗口删掉整包，断言标记确实写过、又被撤回。触发点挂在 `faces.css` 上，它在所有分片 worker 回来之后才写，所以不受分片完成顺序影响。去掉写标记后的核对，这条测试每次都失败。
