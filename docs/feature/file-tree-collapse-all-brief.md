...
> 状态：`partial`。**非 UI 部分已实现**（§5.1 纯函数 + §5.2 store action + §5.5 测试）；
> 按钮、右键菜单项、i18n 文案等着 [`17 文件面板 Files Panel`](file-panel-redesign-brief.md)
> 的设计稿一起落。范围是侧栏「文件」标签页的文件树加一个 **全部折叠**动作；
> 不改展开/折叠本身的默认值、不加快捷键、不做「全部展开」。
