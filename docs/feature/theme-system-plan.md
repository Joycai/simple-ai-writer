...
- `appStore.setTheme(mode)` 语义不变。新增 `setThemeFor(scheme, id)`。三者任一变化 → 解出 `(id, scheme)` → `install.applyResolvedTheme(scheme, id)`：注册表解出真正生效的那张（文件缺席 / 读不出 / 极性不对就是内置基底）、经 `scheme.applyThemeId` 写 `data-theme` / `data-scheme`（`color-scheme` 由 `tokens.scheme` 跟着 `data-scheme` 走）。仍包在 `startViewTransition` 里，交叉淡化照旧。**每一份可用的用户主题都装在同一个 `<style>` 里**（`@layer tokens.user { [data-theme="id"] {…} … }`），切换只是换属性，不读文件。用 `<style>` 而不是 `adoptedStyleSheets`：构造样式表 WebKit 16.4 才有，在地板之上；这张 `<style>` **自己先写一遍五层顺序语句**（`install.ts` 的 `LAYER_ORDER`，与 `tokens.css` 开头那句逐字相同，`themeInstall.test.ts` 钉着），注入位置才真正无关紧要。

  **更正（2026-09-23）**：这里原来写的是「层让注入位置无关紧要」，只写了 `@layer tokens.user {…}`——错的。层的优先级在层名**第一次出现**的地方定死，而启动时 `preloadSelectedThemes` 先于 `import("./App")`（`tokens.css` 由它经 `global.css` 带进来），于是这张 sheet 先进文档，`tokens.user` 成了第一个声明的层 = **最低**优先级，文件写的每个令牌都输给 `tokens.scheme` 的内置核心。症状：设置页里当场挑是好的（那时 sheet 才第一次建、排在应用 CSS 之后），重启就失效，从 S2 起一直如此。S2 的核对之所以没抓到，是因为它在 dev server 页面**加载完之后**才注入，顺序天然对；vitest 在 node 里没有级联。实机定位：tauri dev 的 WKWebView 启动探针读到 `data-theme` 已是用户主题、sheet 已装，`--color-bg-base` 却是纸的值，把 sheet 挪到 `<head>` 末尾后变成文件里的值。选择自带语句而不是在 `main.tsx` 里先 import `tokens.css`：后者把正确性押在另一个文件的 import 顺序上，下次改 `boot()` 会悄悄再坏。
...
| 片 | 内容 | 可见变化 |
|---|---|---|
| **S1 令牌分层** — **已落地 2026-09-06** | `@layer` 五层；`data-scheme` + `color-scheme`（`lib/theme/scheme.ts`：`applyThemeId` / `currentScheme` / `useScheme`）；核心 37 / 推导 176 / 手调 139；`[data-theme="light"]` → `paper` / `night`；Preview 的 Mermaid 读 scheme 并在切换时重渲染；CodeMirror 高亮改成 `lib/editor/highlight.ts`——只发 `.tok-*` 类名，颜色在模块 CSS 里读令牌（原来的 `.tok-*` 规则一直是死代码：`defaultHighlightStyle` 生成的是哈希类名）；16 处主题盲的字面色换令牌（图片灯箱那一族的白字黑底是刻意的，保留）；`themeContract.test.ts` 替换奇偶测试并带悬空引用守卫（首轮抓到 `--color-bg-panel` / `--color-red` / `--spring-open` 三处）；`webviewCaps` 加 `@layer` 探针 | 内置主题**零像素变化**（脚本 + 浏览器双重核对）；夜间编辑器高亮、原生控件 / 滚动条、Mermaid 跟主题了 |
...
| **S6 第三套外观主题 霜 / 靛** — **已落地 2026-09-07** | 同一份设计稿 `05j` 的第二选「靛 Indigo」（深色为主角：底是靛墨而不是黑；浅色那张是同一套色相调亮后的白天）落成第三对**内置**卡：`[data-theme="frost"]` / `[data-theme="indigo"]`，与石 / 墨 一样**只写那 37 个核心**。稿子在这个方向上自留了一处待盯——「靛青×金褐是互补色，oklab 混出来的事件色接近灰紫，与势力（灰蓝）要靠色相勉强分开；深色侧更明显」——**照 S5 那套测法量过：不用改**。六色最近的一对，霜是 势力–概念 ΔE **0.081**、靛是 势力–概念 **0.057**，而各自最近的一对是 纸 0.068（地点–概念）/ 夜 0.025 / 石 0.069 / 墨 0.047：稿子担心的 事件–势力 在深色侧是 0.068，不是最近的那一对，浅色侧连前三都进不去。所以又一次**没有开任何手调**。其余测量：正文 14.1:1（两侧）、次级 6.9 / 8.9、muted 3.6 / 5.5，靛青在两张底上 5.73 / 6.73，四个类型色 fg-on-bg 全部 ≥ 4.5，设置页 页/输入框 分离 1.052（纸 1.033 · 石 1.044）。稿子给 14 个令牌，其余 23 个照 S5 的补法：类型色的 **L 与彩度照石、色相各自保住**（直接把锚色掺进蓝底会把橄榄掺成中性灰，四个芯片就不能靠色相扫读了）稿子的第三个方向「苔 Moss」**作者定了弃**（2026-09-07）：它的硬伤是六色里三个落在绿系，而其中三个由推导决定，要拆开只能给它单开手调——而这一批内置存在的意义正是不带手调地站在推导层上，手调一开它就不证明任何东西了。`05j` 三个方向至此走完 | 作者开箱有三对主题、六张卡；推导层第二次在没有手调兜底的情况下被看见，而这次是在一个**互补色**的强调色上 |
| **S7 随仓库发的样例主题 物理猫 Phycat** — **已落地 2026-09-20** | 不是代码片，是**数据片**：`themes/` 从一份样例长到十四份——`journal.css` 之外，物理猫一整套（配色取自 Typora 主题 `sumruler/typora-theme-phycat`，MIT）：**十一份外观**（亮色系 樱桃红 / 焦糖橙 / 森绿 / 薄荷青 / 天蓝 / 普鲁士蓝 / 樱花粉 / 淡紫，暗色系 吸血鬼 / 辐射 / 深渊）+ **两份排版**（`phycat.css` 气泡＝亮色系那套版式，`phycat-neon.css` 灯带＝暗色系那套）。**为什么是 11 + 2 而不是 11 对**：排版主题里一个颜色都没有写死，全读 `var(--color-*)`，颜色是外观主题的活儿——按颜色各抄一份排版，就是同一份规则抄十一遍。原主题自己也是这么分的（`phycat.light.css` 一份版式 + 每个颜色一份调色板），这里只是把它的「调色板」落在外观主题上。**强调色重算，不照抄**：原主题的 `--element-color` 是给彩色标题和图标用的，而这里 `--color-sienna` 同时是 1px 细线、填色按钮和焦点环，所以逐个压到在自己的底上 ≥ 4.5:1 才收手（亮色系落在 4.60–4.65，照 石 5.10 / 霜 5.73 略低是因为色相被钉死了）；暗色系反过来，霓虹本身就够亮，**原值原样留着**（辐射 9.22 / 深渊 13.68；只有吸血鬼的 `#FF5555` 贴着线，4.53）。**明度台阶照 石 / 霜，彩度约 1.6×**：原主题是白纸加底纹，这里的底是一张染了色的纸（`#E5F5F6` 薄荷 / `#FCEDEE` 樱桃 / `#FBEEE8` 焦糖…），台阶一个不改，只把中性的彩度提上去——1.0× 会读成灰，那就不是物理猫了。测量：亮色系正文 14.2–14.4:1、次级 6.1–6.6、muted 3.3–3.4；暗色系正文 13.4–13.9、次级 8.8–8.9、muted 5.7–5.8；四个类型色 fg-on-bg 全部 ≥ 4.5。**字体不随主题走**：原主题用霞鹜文楷，一份 25 MB，`phycat.css` 末尾给了 `@font-face` 的写法让作者自己放。`themeExamples.test.ts` 因此按 `--theme-kind` 分成两路，外观那一路多两条**只对样例成立**的检查——写满整份核心契约（写一半会静悄悄继承基底的色相，在一张换了颜色的底上读起来就是 bug），以及强调色 4.5:1 / 正文 7:1。实机之外的核对：把每一份外观主题喂进 `exportPaletteCss`（推导层因此是真的走了一遍），配它那套排版渲染一整屏应用外壳 + 一篇样文，十一套逐个在浏览器里看过 | 作者开箱能下十一套颜色 + 两套版式；同时这是第一次由**外部主题**来证推导层——石 / 墨 / 霜 / 靛 是我们自己挑的色相，物理猫的十一个不是 |
| **S8 简写丢失：说出来，而不是静悄悄** — **已落地 2026-09-20** | 实机报障：「物理猫 · 气泡」的二级标题药丸只剩反白字。根因不在主题的颜色，在**引擎**：`validateMarkdownRules` 用 `style.cssText` 重新生成规则（为的是 `background: #fff` 回来是一条而不是八条），而 WebKit 序列化不了「带 `var()` 的简写 + 后面同族的长写」——`background: linear-gradient(var…); background-size: 200% auto` 回来是 `background-image: ; background-color: ; …`，`getPropertyValue("background")` 也是空。实测（WKWebView，2026-09-20）：`border`+`border-left`、`margin`+`margin-top`、`font`+`font-size` 同样丢；长写在前、或 `border`+`border-radius`（不同族）不丢；Blink 全部能序列化，所以浏览器里的预览看不出来。**为什么不从源文本恢复**：CSSOM 规则与源文本块的对应，正是 §14 弃掉的「自己切 CSS」；值已经不在 CSSOM 里，恢复就得要第二个解析器。**所以做的是「说出来」**：`cssText` 里的空声明（`name: ;`）被剔掉，并按族记一条 `mdShorthandLost`（中英句子在 locale），卡片上看得到是哪条规则的哪个简写、该怎么改。顺带修掉一个同源的误标：待替换的长写枚举出来是空值，旧判定 `!/var\(/.test("")` 为真，于是任何用了 `background: var(…)` 的主题都被标「自带颜色 · 不随外观变」——现在空值不算字面色。`themes/phycat.css` 的药丸改写成 `background-image`；`make-theme` skill 的检查器在 node 侧按同一条规律（var 简写之后出现 `<简写>-*`）报 `✗`，因为它的预览跑在 Chromium 上，看不到这种丢失 | 丢了规则的主题在卡片上有理由；「自带颜色」不再误标 |
