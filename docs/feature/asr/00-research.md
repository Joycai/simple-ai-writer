...
落地：`lib/asr/sync.ts`（请求体 / 错误改口 / 调用）、`formats.ts`（`looksLikeSyncAsrModel`、`asrIdMismatch`、六个扩展名、`SYNC_MAX_BYTES` / `SYNC_MAX_SECONDS`、`syncRefusal`）、`result.ts`（`parseSyncTranscript`、`Transcript.timed: false`、计费秒数缺席时按 audio_tokens / 25 向上取整）、`run.ts` 按接口分支、缓存键带接口（`ASR_CACHE_VERSION` 3）。不变量见 `01-execution-plan.md` §1 第 8 条。
