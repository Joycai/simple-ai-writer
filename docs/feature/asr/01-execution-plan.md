...
## 5.6 已付费任务的检查点与轮询错误分类（模型端点审查，2026-09-19）

提交是付费的一步。原来提交之后的任何中断——停止、轮询时限、崩溃、密钥轮换后的 401——都把一个已付费的任务丢在平台上，
重跑则从头上传、再付一次。轮询还把 401 / 403 / 404 / 429 一律当网络抖动，连续三次就放弃。

- **检查点**：提交一返回就写 `<缓存键>.pending.json`（taskId、模型、提交时间），结果进缓存后删掉。同一文件、同一模型、同一组参数重跑时，
  直接轮询这个 taskId，不再上传、提交。放在缓存目录**旁边**而不是里面：目录要等结果落地才整体改名就位。超过 24 小时
  （平台保留任务的时长）的检查点视为无效，清扫时删掉（读不出来的也删）；24 小时内的留着等重跑来接。
  清扫要单独处理这些文件，因为缓存那一轮只看目录：换了文件或参数、再没重跑过的检查点，没有别的时机会被删。
- **轮询按状态分类**（知识库 16 §8、坑 119 / 121）：429 按 `Retry-After` 等（缺省 10 秒、封顶 60 秒），**不计入**失败次数；
  401 / 403 立刻抛，保留检查点，修好密钥后重跑接着轮询；404（任务不在了）和任务 FAILED 删掉检查点。
  如果是从检查点恢复时遇到 404，就在同一次运行里重新提交一次：作者批准的是一次转写，旧任务已不存在，这是第一次真正的提交。
  其余错误仍按网络抖动处理，最多容忍三次。
