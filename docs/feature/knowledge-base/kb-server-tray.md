...
- **菜单**：`启动服务器`/`停止服务器`（一项，label 随状态换）· `打开管理后台`
  ...
  配置文件路径 / 版本）· `数据目录…`（见下条）· `开机自启`（勾选项）·
  `退出`（先优雅停机再退出）。
- **数据目录可改，默认在系统数据目录**：首次生成的配置文件把 `data_dir`
  写成**配置文件旁边的绝对路径**（Windows 上即 `%APPDATA%\aiw-kb\data`——
  配置默认就落在 `%APPDATA%\aiw-kb\config.toml`），而不是 CWD 相对的
  `./data`：把数据的位置绑在「进程从哪里被启动」上，正是 CWD 钉住那个
  workaround 在治的病，生成时就写绝对路径把病根去掉。运行时默认（文件里
  **省略** `data_dir` 时）保持 `./data` 不变——已有的配置文件一个字节都不受
  影响。`数据目录…` 菜单项：选目录 → （旧目录有数据且新目录为空时）问
  「搬运并切换 / 只切换 / 取消」→ 停服 → 复制（**只复制，从不删原目录**）→
  经 `confedit::apply` 写回（和管理后台同一条保留注释的路）→ 重启。所有
  问题都在停服**之前**问完，任何一步取消都留下一台还在跑的服务器；
  `AIW_KB_DATA_DIR` 环境变量在管时直接拒绝——写了文件也不生效的改动，
  provenance 模型存在的意义就是提前抓住这种谎。
...
2026-08-28 二片：`数据目录…` 菜单项 + 生成的配置默认把数据放进系统数据目录
（`starter_data_dir`：配置文件旁的绝对路径，Windows = `%APPDATA%\aiw-kb\data`）。
注意两条：迁移是**复制而非移动**，原目录留作后悔药；tracing 在本进程里已经
