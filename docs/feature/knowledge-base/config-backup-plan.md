...
**钥匙串没写进去不算恢复失败，重读照跑。** `applyConfigImport` 先在一个事务里提交供应商 / 模型 / Prompt / 计费组，再写偏好和排版格式，最后才把内嵌的 API Key 逐个存进钥匙串——钥匙串是另一个存储，进不了那个事务。它原先在有 Key 没存进去时 `throw`，两个调用方（文件导入 `ConfigBackupSection.handleImport`、服务端恢复 `configSyncStore.confirmRestore`）都把这当成「导入失败」，于是跳过了 `refreshAfterConfigImport()`：库里已经是恢复后的配置和计费组绑定，`aiStore` / `appStore` / `docFormatStore` 却一直停在恢复前，直到重启；这期间从一个还开着旧数据的抽屉点保存，会把刚恢复的行写回去。服务端那条还多一个坑：模态停在预览上，等于请作者再合并一次同一个包。

现在的约定：**`applyConfigImport` 抛错 = 行没落地**（事务失败），调用方照旧报失败、不重读；事务之后的部分失败走**返回值** `{ failedKeys }`，由 `keyFailureMessage()` 生成原来那句提示（文案不变）。两个调用方拿到结果后**先重读、再报**：文件导入显示那句提示（红字，不再在前面加「导入失败」——配置确实进去了），服务端恢复进「完成」并在下面列出没存进去的 Key，「记住密码」照常生效（信封是用这个密码打开的，Key 的失败是钥匙串的事）。没选「抛一个带类型的错误让调用方认」这条路：那样「成功了但有警告」仍然走 `catch`，每个调用方都得记得在 `catch` 里先分辨一次再重读，漏一个就是这个 bug 原样回来；返回值让「重读」待在成功路径上，不需要谁记得。`configImportTx.test.ts` 与 `configSyncStore.test.ts` 钉住这两点。
