...
> 状态：`shipped`（一期 + 二期 PR #325，三期在后续 PR）。整条链闭合：作者建/改/读格式
> → 模型在 briefing 里看得见清单 → `export_docx` 出稿 → 审批卡逐项核对 → 回读断言钉住
> 产出；预设跟着应用配置备份走到下一台机器。
> **仍未做**：目录（TOC 域）、脚注、页眉的奇偶页各写各的文字。可行性与实测在 [00-feasibility.md](00-feasibility.md)，UI 任务书在 [02-ui-brief.md](02-ui-brief.md)。
...
| **三期 · 已实施** | 页眉页脚与页码（奇偶页不同 · 首页不同 · 页眉横线 · 每章页码重来）· 标题多级自动编号 · `parseDocFormat` 归一层 · 预设进应用配置备份 |
...
2026-09-06（第二笔）：按 #493 核对表把 05e / 05f 剩下的出入补齐——字号改成「一个框两种写法」
（1e：输入号数或磅，框内右侧回显另一种，号数表折在框尾）；删除改成居中确认框，删的正好是默认
那套时给「默认交给 ▾」转交（1l，`removeFormat(id, handoffTo)`）；读取模态的文件芯片带
大小与耗时（1h，页数读不到不编），全是 Word 默认值时主动作改成「改用『素雅』」（1i）；纸样
补行号刻度与「N 字位」（1c）；自建空态段里再给两个入口、`customEmpty` 那句「二期会开放」
删掉（1l）；≤720 纸样收成可展开的条、列表摘要折两行、行内动作常显（1m——稿上是收成一个点按钮，
这里先让它们一直在）。抽屉里「页眉页脚」「标题自动编号」两组的稿（`05h`）已回并对齐——编号进了标题表当第七列、依赖字段虚线不消失、纸样画页码与页眉横线、三处摘要有值才追加、读取模态里两行「没读」，出入见 [03-header-numbering-ui-brief.md](03-header-numbering-ui-brief.md) 的落地记录；
1d 内置行的「编辑」与 1l「内置改不了」稿内矛盾，码取 1l。

2026-09-06（第三笔）：又照 05e / 05f 核了一遍，补齐六处，其中两处是**卡上把同一件事说了两遍**：

1. **审批卡的「格式来源」一格改成短名**（1j）。`describeOrigin` 写的是「默认格式：手稿」，
   而它右边那句安静的话写的正是「手稿」，改过时左边还带着「（改了 2 项）」、右边又挂一枚
   「改了 2 项」徽标——同一张卡上一件事说两到三遍。拆成 `originName`（左格，短）+ 三种
   来历各自的 note（`内置 · 未改动` / `自建 · 未改动` / `未存为预设`）+ 徽标，稿上四种来源
   于是各自只说一次。`describeOrigin` 随之删掉，回给模型的那句改由 agentStore 拼——模型没有
   徽标可看，它那句该说全，包括每一处 `from → to`。
2. **模仿来的格式补上那行括注**（1j ③）：「未在文件里出现的项按 Word 默认值补（N 项）」。
   数字来自 `DocFormatPreset.filledDefaults`，**只活在内存里**：作者一点「存为预设」，那套
   格式就是他自己核对过留下的资产，来历只剩「读自 X.docx」那一句。
3. **字体缺失的三档强度原本是反的**（1e）。稿上是列表一枚小标 → 抽屉一整句 → 审批卡一行
   括注，递减；码里审批卡那句是三处里最长的。审批卡改回「本机未装（不影响导出）」。
   抽屉那句同时**改成真话**：原文写「预览与编辑器里会用替代字体显示」，而这个实现的纸样
   一个字都不画（1c 的灰条），编辑器也不吃这套格式的字体——现在写的是导出仍然合规、纸样
   只画灰条所以这台机器上看不出差别。稿上「用 宋体 替代显示」那半句是对一个所见即所得预览
   说的，照抄进来就是一句 UI 兑现不了的承诺。
4. **导航项补 BETA 小标**（1a / 1n）。它是一个开关开出来的项，导航里不说，作者只有点进去
   才看得见。
5. **≤720 的行内动作收成一个点按钮 + 菜单**（1m），复用 `common/ContextMenu`。上一笔留的
   「先让它们一直在」到此结束——那个宽度多半是触屏，而触屏没有悬停。同一个宽度下编辑抽屉
   铺满整屏，于是它的出口从右上的 × 换成左上的返回箭头：两个按钮都渲染、各自在自己的宽度
   下显示，宽度是 CSS 知道的事，让 JS 去量只会多出一个会和媒体查询对不上的真值。
6. **导出结果卡**（1k），上两笔都没做。它挂在**对话的那一轮**上（`ChatTurn.exports`），和
   图片同一条约定：由批准那一步填，不由模型复述——降级是转换器数出来的，模型历来说不准。
   因此也和图片一样，**不绑轮次的运行（AI 面板任务、扮演）不留卡**，那里模型自己那句话就是
   全部。两处与稿不同：页数写成块数（页数要 Word 自己排版才知道，同审批卡那条），降级那一行
   数的是「几类」而不是「几处」——每条降级字符串里本来就写着「（N 处）」，把它们加起来要重新
   解析自己刚拼好的人话。

2026-09-06（第四笔，改回上一笔的一处）：**自建空态里不再重复那两个按钮。** 稿上 1l 是列表区的
裁切，从「内置 · 不可删」直接开始、没有画页头，所以稿里那一对「从 Word 文件读取格式」「+ 新建
预设」和页头那一对从来没有同屏出现过——照着叠上去，一台还没建过自建预设的机器上这两个按钮就
一屏两份。留下的是那两行字：它说的正是这两个按钮该怎么用，而按钮本身在页头常驻，任何宽度下
都在（1a / 1b / 1m 的页头都有）。
...
### 13.1 页眉页脚：没设过就一个分片都不发

最容易犯的错是给每份文稿都发一个空页眉。Word 里那一行会占着位置，作者打开一看
「我什么时候设过页眉」——而他没有。所以 `headersAndFooters` 在页眉文字为空、页码为
`none` 时**返回一个空对象**，`word/header*.xml` / `footer*.xml` 根本不会出现在包里。
`roundtrip.test.ts` 有一条断言就钉这个。

页码是**域**（`PAGE`）不是死字：写成死字第二页还是 1，而这种错要翻到第二页才看得见。

「奇偶页不同」有一条**必须在文档层开**：`evenAndOddHeaderAndFooters` 是 `Document` 的
选项，不是节属性。只设节属性的话 Word 会拿单页的页脚排满全篇，偶数页那半安静地不
出现——不报错，只是少了一半。

偶数页的处理是**一条规则而不是两套字段**：左右对齐互换，居中不动。装订成册后页码就
落在订口外侧，而作者只需要设一次「靠右」。

### 13.2 标题编号：写法是 `none` 的那一级不挂空编号

给一级挂一个空编号，Word 仍会为它留出一个制表位，标题就莫名其妙地缩进了——所以
`renderBlock` 判断的是「这一级的写法不是 none」，而不是「编号功能开着」。

序号和标题之间用 `LevelSuffix.NOTHING`：Word 默认插一个制表位，会把「一、总体要求」的
标题部分推到一个和正文对不齐的位置上。缩进则整个交给标题样式的 `firstLineChars`，
编号自己的 `indent` 归零，否则两层缩进叠加。

一条只会静默出错的事，因此单独有测试：**三份 numbering 定义共处一份 `numbering.xml`，
而 `numId` 由 docx 自己分配**。标题若指到列表那份定义上，表现只是序号变成了圆点，不会
报任何错。测试因此不查 `numId` 的字面值，而是顺着 `numId → abstractNumId → lvlText`
走一遍，断言它落在 `%1、` 上。

### 13.3 `parseDocFormat`：归一，不是拒绝

三期新增了两块字段，而作者的自建预设早已存在 `config.db` 里。没有写数据迁移——加了一层
归一：读回来的 JSON 缺什么就从底座补什么。

它的立场很明确：**一份预设里有一个字段坏了不该让整套消失**。对作者来说那是「我的格式
不见了」，而缺的那一项本来就该落回默认。同一个函数因此同时服务三个入口——从
`config.db` 读、从配置备份导入、以及以后任何别的来路。

### 13.4 预设进应用配置备份

`ConfigBackup.docFormats` 是可选字段，和 `prefs` 当初一样：旧备份没有它就还原不出预设，
而不是打不开。两条只属于排版格式的规矩：

- **备份里的一律当自建**（`builtin: false`）。内置那几套随版本走，不该被一份旧备份改写成
  上个版本的样子。
- **写在事务外面**，和偏好一样。它没有指向 providers/models 的外键，放进那个事务只会
  扩大「数据库被锁」能撤销一次已经成功的还原的窗口。


### 13.5 收尾的三条

**首页不同＝首页什么都不写**，不是「首页写别的」。后者是两套内容，而作者要的从来是
「封面这一页空着」。所以 `first` 那一份是**空的 Header/Footer，但必须发**——不发的话
Word 会拿 `default` 顶上，`titlePg` 就等于没设。

**页眉横线可以单独存在**：有些模板只要一条线不要字。所以判断是「有文字**或**有横线」
才发页眉，而不是「有文字」。

**每章页码重来**要处理一个会多出白纸的交互：分节符本身就分页，而一级标题的样式上还挂
着 `pageBreakBefore`，两个叠起来每章前面多一张白纸。让开的是标题那一个——节正是按它切
的。`splitChapters` 是纯函数，两个边界单独测：第一个一级标题**之前**的内容自成一节
（并进第一章的话页码就从封面开始数），以及没有一级标题时返回**一整节而不是零节**
（返回空数组会让整份文稿安静地消失）。
